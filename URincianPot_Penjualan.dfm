object FRincianPot_Penjualan: TFRincianPot_Penjualan
  Left = 0
  Top = 0
  Caption = 'Rincian Potongan'
  ClientHeight = 443
  ClientWidth = 1258
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  Position = poDesktopCenter
  TextHeight = 15
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1258
    Height = 81
    Align = alTop
    Color = clGradientInactiveCaption
    ParentBackground = False
    TabOrder = 0
    ExplicitWidth = 1194
    object Label39: TLabel
      Left = 19
      Top = 43
      Width = 91
      Height = 15
      Caption = 'Nama Pelanggan'
    end
    object Label38: TLabel
      Left = 116
      Top = 43
      Width = 3
      Height = 15
      Caption = ':'
    end
    object Label13: TLabel
      Left = 118
      Top = 17
      Width = 3
      Height = 15
      Caption = ':'
    end
    object Label12: TLabel
      Left = 20
      Top = 17
      Width = 69
      Height = 15
      Caption = 'No. Transaksi'
    end
    object edNama_Pelanggan: TRzButtonEdit
      Left = 131
      Top = 40
      Width = 344
      Height = 23
      Text = ''
      TabOrder = 0
      AltBtnNumGlyphs = 1
      ButtonNumGlyphs = 1
    end
    object edKode_Pelanggan: TEdit
      Left = 481
      Top = 40
      Width = 200
      Height = 23
      TabOrder = 1
      Visible = False
    end
    object edNomorTrans: TEdit
      Left = 132
      Top = 14
      Width = 250
      Height = 23
      CharCase = ecUpperCase
      TabOrder = 2
    end
  end
  object DBGridCustomer: TDBGridEh
    Left = 0
    Top = 81
    Width = 1258
    Height = 330
    Align = alClient
    DataSource = dsMasterData
    DynProps = <>
    ReadOnly = True
    SearchPanel.Enabled = True
    TabOrder = 1
    TitleParams.MultiTitle = True
    Columns = <
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'kd_brg'
        Footers = <>
        Visible = False
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'nm_brg'
        Footers = <>
        Title.Caption = 'Nama Barang'
        Width = 200
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'satuan'
        Footers = <>
        Title.Caption = 'Satuan'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'jumlah'
        Footers = <>
        Title.Caption = 'Jumlah'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'harga_satuan'
        Footers = <>
        Title.Caption = 'Harga Satuan'
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'nilai_ppn'
        Footers = <>
        Title.Caption = 'Nilai PPN'
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pot_value_1'
        Footers = <>
        Title.Caption = 'Potongan | Pertama'
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pot_value_2'
        Footers = <>
        Title.Caption = 'Potongan | Kedua'
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pot_value_3'
        Footers = <>
        Title.Caption = 'Potongan | Ketiga'
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pot_value_4'
        Footers = <>
        Title.Caption = 'Potongan | Keempat'
        Visible = False
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pot_persen_1'
        Footers = <>
        Visible = False
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pot_persen_2'
        Footers = <>
        Visible = False
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pot_persen_3'
        Footers = <>
        Visible = False
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pot_persen_4'
        Footers = <>
        Visible = False
        Width = 125
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'total'
        Footers = <>
        Title.Caption = 'Total'
        Width = 125
      end>
    object RowDetailData: TRowDetailPanelControlEh
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 411
    Width = 1258
    Height = 32
    Align = alBottom
    TabOrder = 2
    ExplicitTop = 401
    ExplicitWidth = 1194
    object BBatal: TRzBitBtn
      Left = 1182
      Top = 1
      Height = 30
      Align = alRight
      Caption = 'Cancel'
      TabOrder = 1
      OnClick = BBatalClick
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 1118
    end
    object BSave: TRzBitBtn
      Left = 1107
      Top = 1
      Height = 30
      Align = alRight
      Caption = 'Save'
      TabOrder = 0
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 1043
    end
  end
  object dsMasterData: TDataSource
    DataSet = MemMasterData
    Left = 384
    Top = 256
  end
  object MemMasterData: TMemTableEh
    Active = True
    Params = <>
    Left = 488
    Top = 256
    object MemMasterDatakd_brg: TStringField
      FieldName = 'kd_brg'
      Size = 50
    end
    object MemMasterDatanm_brg: TStringField
      FieldName = 'nm_brg'
      Size = 255
    end
    object MemMasterDataharga_satuan: TCurrencyField
      FieldName = 'harga_satuan'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatasatuan: TStringField
      FieldName = 'satuan'
    end
    object MemMasterDatapot_value_1: TCurrencyField
      FieldName = 'pot_value_1'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatapot_value_2: TCurrencyField
      FieldName = 'pot_value_2'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatapot_value_3: TCurrencyField
      FieldName = 'pot_value_3'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatapot_value_4: TCurrencyField
      FieldName = 'pot_value_4'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatapot_persen_1: TFloatField
      FieldName = 'pot_persen_1'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatapot_persen_2: TFloatField
      FieldName = 'pot_persen_2'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatapot_persen_3: TFloatField
      FieldName = 'pot_persen_3'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatapot_persen_4: TFloatField
      FieldName = 'pot_persen_4'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatanilai_ppn: TFloatField
      FieldName = 'nilai_ppn'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatajumlah: TFloatField
      FieldName = 'jumlah'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemMasterDatatotal: TFloatField
      FieldName = 'total'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object MemTableData: TMemTableDataEh
      object DataStruct: TMTDataStructEh
        object kd_brg: TMTStringDataFieldEh
          FieldName = 'kd_brg'
          StringDataType = fdtStringEh
          DisplayWidth = 50
          Size = 50
        end
        object nm_brg: TMTStringDataFieldEh
          FieldName = 'nm_brg'
          StringDataType = fdtStringEh
          DisplayWidth = 100
          Size = 255
        end
        object harga_satuan: TMTNumericDataFieldEh
          FieldName = 'harga_satuan'
          NumericDataType = fdtCurrencyEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object satuan: TMTStringDataFieldEh
          FieldName = 'satuan'
          StringDataType = fdtStringEh
          DisplayWidth = 20
        end
        object pot_value_1: TMTNumericDataFieldEh
          FieldName = 'pot_value_1'
          NumericDataType = fdtCurrencyEh
          AutoIncrement = False
          DisplayWidth = 100
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object pot_value_2: TMTNumericDataFieldEh
          FieldName = 'pot_value_2'
          NumericDataType = fdtCurrencyEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object pot_value_3: TMTNumericDataFieldEh
          FieldName = 'pot_value_3'
          NumericDataType = fdtCurrencyEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object pot_value_4: TMTNumericDataFieldEh
          FieldName = 'pot_value_4'
          NumericDataType = fdtCurrencyEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object pot_persen_1: TMTNumericDataFieldEh
          FieldName = 'pot_persen_1'
          NumericDataType = fdtFloatEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object pot_persen_2: TMTNumericDataFieldEh
          FieldName = 'pot_persen_2'
          NumericDataType = fdtFloatEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object pot_persen_3: TMTNumericDataFieldEh
          FieldName = 'pot_persen_3'
          NumericDataType = fdtFloatEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object pot_persen_4: TMTNumericDataFieldEh
          FieldName = 'pot_persen_4'
          NumericDataType = fdtFloatEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object nilai_ppn: TMTNumericDataFieldEh
          FieldName = 'nilai_ppn'
          NumericDataType = fdtFloatEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object jumlah: TMTNumericDataFieldEh
          FieldName = 'jumlah'
          NumericDataType = fdtFloatEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
        object total: TMTNumericDataFieldEh
          FieldName = 'total'
          NumericDataType = fdtFloatEh
          AutoIncrement = False
          DisplayWidth = 20
          DisplayFormat = '#,##0.##'
          EditFormat = '#,##0.##'
          currency = False
          Precision = 15
        end
      end
      object RecordsList: TRecordsListEh
      end
    end
  end
  object query2: TUniQuery
    Connection = dm.Koneksi
    Left = 720
    Top = 8
  end
end
