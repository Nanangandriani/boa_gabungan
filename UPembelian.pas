...
begin
    ...
    begin
      ...
      sql.Text:=' Select (case WHEN a."approval_status"=0 THEN ''PENGAJUAN'' WHEN a."approval_status"=1 THEN ''APPROVE'' else ''REJECT'' '+
                ' END) AS status_app,a.*,b.supplier_name,c.account_name,d.account_name as nm_perk ,to_char(receive_date,''dd'') tgl '+
                ' ,to_char(receive_date,''mm'') bln from purchase.t_item_receive a Left join t_supplier b on a.supplier_code=b.supplier_code '+
...
begin
    ...
    begin
      ...
      begin
        ...
        if Memterima_material['sj_status']=0 then Cksj.Checked:=False else Cksj.Checked:=True;
        if Memterima_material['fk_status']=0 then CkFk.Checked:=False else CkFk.Checked:=True;
        if Memterima_material['invoice_status']=0 then CkInv.Checked:=False else CkInv.Checked:=True;
        ...
        if DBGridTerima1.Fields[2].value='PENGAJUAN' then GBDok.Enabled:=False else GBDok.Enabled:=True;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          MemterimaDet['gudang']:=QTerimaDet.FieldByName('warehouse').AsString;
