...
uses
  ...
  Vcl.ComCtrls, RzButton, Vcl.ExtCtrls, Vcl.StdCtrls, RzLine;

type
  TFdata_uang_cash_besar = class(TForm)
    Label29: TLabel;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
...
//var
  //Fuang_cash_besar: TFuang_cash_besar;
Function Fdata_uang_cash_besar: TFdata_uang_cash_besar;
...
uses Ustock_opname_kas_besar, UDataModule, UHomeLogin;


var
  RealFdata_uang_cash_besar : TFdata_uang_cash_besar;

function Fdata_uang_cash_besar : TFdata_uang_cash_besar;
begin
  if RealFdata_uang_cash_besar <> nil then
     Fdata_uang_cash_besar := RealFdata_uang_cash_besar
  else
    Application.CreateForm(TFdata_uang_cash_besar, Result);
end;
...
procedure TFdata_uang_cash_besar.btnnewClick(Sender: TObject);
begin
   self.Clear;
end;

procedure TFdata_uang_cash_besar.BTutupClick(Sender: TObject);
begin
  Close;
end;

procedure TFdata_uang_cash_besar.Button1Click(Sender: TObject);
begin
  with dm.Qtemp2 do
  begin
    Close;
    Sql.Clear;
    Sql.Text:='select * from t_cash_money_data '+
...
procedure TFdata_uang_cash_besar.clear;
begin
  edlbr100rb.Text:='';
  edlbr75rb.Text:='';
  edlbr50rb.Text:='';
...
procedure TFdata_uang_cash_besar.DTtanggalChange(Sender: TObject);
begin
    with dm.QTemp2 do
    begin
      Close;
...
procedure TFdata_uang_cash_besar.edlbr1000Change(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr100Change(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr100rbChange(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr10rbChange(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr200Change(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr20rbChange(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr2rbChange(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr500Change(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr50Change(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr50rbChange(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr5rbChange(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.edlbr75rbChange(Sender: TObject);
begin
...
procedure TFdata_uang_cash_besar.ednilaitotChange(Sender: TObject);
begin
   Ribuan(ednilaitot);
end;

procedure TFdata_uang_cash_besar.FormShow(Sender: TObject);
begin
   DTtanggal.Date:=now();
end;

procedure TFdata_uang_cash_besar.Simpan;
begin
  with DM.Qtemp do
  begin
    close;
    sql.Clear;
    sql.Text:='Insert Into t_cash_money_data(trans_date,lbr100000,lbr75000,lbr50000,lbr20000,'+
              ...
              'Values (:partgl_trans,:parlbr100000,:parlbr75000,:parlbr50000,:parlbr20000,'+
...
procedure TFdata_uang_cash_besar.Update;
begin
    with DM.Qtemp do
  begin
    close;
    sql.Clear;
    sql.Text:='Update t_cash_money_data set trans_date=:partgl_trans,lbr100000=:parlbr100000,lbr75000=:parlbr75000,'+
              ...
              'updated_by=:parupdated_by,updated_at=:parupdated_at '+
              'where trans_date='+QuotedStr(FormatDateTime('yyyy-mm-dd',DTtanggal.date)) ;
    ...
    parambyname('parupdate_at').Value:=NOW;
    execsql;
  end;
...
procedure TFdata_uang_cash_besar.Total_nilai;
begin
  if ednilai100rb.Text='' then
  a:=0
  else
  ...
  if ednilai75rb.Text='' then  //14-06-2022
  m:=0
  else
