...
var
   thn,bln,status_pos,kd_akppn,kd_akpph,kd_akbea,Nofk,nopo,nourut,jenis_pembelian:String;
...
uses UDataModule, UMainMenu, UMy_Function, USearch_TerimaBarang,
  UAkun_Perkiraan_TerimaMat, UPembelian;
...
begin
      ...
      begin
        ...
        begin
          close;
          sql.Clear;
          sql.Text:='SELECT * FROM purchase.t_item_receive  where faktur_no='+quotedstr(Edno_Faktur.text);
          ExecSQL;
        end;
        if dm.Qtemp.RecordCount > 0 then
        begin
        ...
        begin
           ...
           begin
              ...
              begin
                  close;
                  sql.Clear;
                  sql.Text:=' insert into purchase.t_item_receive(receive_no,receive_date,remark,spb_no,sj_no,faktur_no,'+
          ...
          begin
              ...
              begin
                ...
                sql.Text:=' insert into purchase.t_item_receive(receive_no,receive_date,remark,spb_no,sj_no,faktur_no,'+
                          ' supplier_code,faktur_date,due_date,purchase_type,debt_amount,payment_amount,'+
                          ' debt_remaining,ppn_rp,pic,status,valas,valas_value,account_code,trans_month,trans_year,import_duty,'+
                          ' sbu_code,pib_no,po_type,faktur2_date,account_um_code,um_value,trans_day,order_no)values(:parno_terima,'+
                          ' :partgl_terima,:parket,:parnospb,:parnosj,:parnofaktur,:parkd_supplier,'+
                          ' :partgl_faktur,:parjatuh_tempo,:parjenis_pembelian,:parjmlh_hutang,:parjmlh_bayar,'+
                          ' :parsisa_hutang,:parppn_rp,:parpic,:parstatus,:parvalas,:parnilai_valas,:parkd_akun,:parbln,'+
                          ' :parthn,:parbea,:parsbu,:parpib,:parjenispo,:partgl_faktur2,:parkd_akunum,:parum,:partgl_no,:parorder_no)';
                          ParamByName('parno_terima').Value:=EdNo.Text;
                          ParamByName('partgl_terima').Value:=FormatDateTime('yyyy-mm-dd',Dtterima.Date);
                          ParamByName('parket').Value:=EdKet.Text;
                          ParamByName('parnospb').Value:=EdNoSPB.Text;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SQL.Text:=' select a.item_stock_code,a.item_code, b.item_name, a.order_no,b.price,b.remaining_qty as qty, '+
                    ' b.unit,b.warehouse,b.ppn,b.pph,b.po_no, c.supplier_code, e.account_code, c.due_date'+
      ...
      begin
        ...
        SQL.Text:='select a.item_stock_code,a.item_code,a.item_name, a.order_no,b.price,d.qty, '+
                  ' b.unit,b.warehouse,b.ppn,b.pph,b.po_no, c.supplier_code, d.spb_no,e.account_code'+
                  ' , c.due_date ,c.valas,c.valas_value,f.wh_code,c."type",b.pemb_dpp from warehouse.t_item_stock a '+
                  ' inner join purchase.t_podetail b on a.item_stock_code=b.item_stock_code '+
                  ' inner join purchase.t_po c on b.po_no=c.po_no '+
                  ...
                  ' b.unit,b.warehouse,b.ppn,b.pph,b.po_no, c.supplier_code, d.spb_no,e.account_code, c.due_date '+
                  ' ,c.valas,c.valas_value,f.wh_code,c."type",b.pemb_dpp';
        ExecSQL;
      end;
      Show;
        ...
        begin
          ...
          begin
            ...
            sql.Text:='	INSERT INTO warehouse.t_item_stock_det(item_stock_code,stock_code,qty,'+
                      ...
                      ParamByName('parwh').Value:=MemterimaDet['gudang'];
...
begin
    ...
    begin
        ...
        begin
          Close;
          sql.Clear;
          sql.Text:='insert into purhase.t_item_reeive_det(material_stock_code,stock_code,qty,unit,warehouse,'+
...
begin
    ...
    if Dm.Qtemp.FieldByName('um').Asfloat > 0 then EdNilai_um.Value:=Dm.Qtemp.FieldByName('um').value else
    EdNilai_um.Value:=0;
    if Dm.Qtemp.FieldByName('umpo').Asfloat > 0 then Edum.Value:=Dm.Qtemp.FieldByName('umpo').value else
    Edum.Value:=0;
    if Dm.Qtemp['account_code']= null then   Edkd_akunum.Text:='' else   Edkd_akunum.Text:=Dm.Qtemp['account_code'];
...
begin
    ...
    begin
        ...
        begin
          Close;
          sql.Clear;
          sql.Text:='insert into purchase.t_item_receive_det(item_stock_code,stock_code,qty,unit,warehouse,'+
                    ' year,po_no,receive_no,qty_po,unit_po,qty_difference,qty_per_conversion,qty_conversion,'+
                    ' unit_conversion,subtotal,ppn,ppn_rp,pph,pph_rp,grandtotal,price,account_code,'+
