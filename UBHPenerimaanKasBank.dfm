object FBHPenerimaanKasBank: TFBHPenerimaanKasBank
  Left = 0
  Top = 0
  Caption = 'Buku Harian Penerimaan Kas Dam Bank'
  ClientHeight = 558
  ClientWidth = 1136
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  TextHeight = 15
  object dxRibbon1: TdxRibbon
    Left = 0
    Top = 0
    Width = 1136
    Height = 127
    BarManager = dxBarManager1
    Style = rs2010
    ColorSchemeName = 'Blue'
    Contexts = <>
    TabOrder = 0
    TabStop = False
    object dxRibbon1Tab1: TdxRibbonTab
      Active = True
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar1'
        end
        item
          ToolbarName = 'dxBarManager1Bar2'
        end>
      Index = 0
    end
  end
  object DBGrid: TDBGridEh
    Left = 0
    Top = 127
    Width = 1136
    Height = 431
    Align = alClient
    DataSource = DsBHPenerimaanKasBank
    DrawMemoText = True
    DynProps = <>
    SearchPanel.Enabled = True
    TabOrder = 1
    TitleParams.MultiTitle = True
    Columns = <
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'voucher_no'
        Footers = <>
        Title.Caption = 'Nomor Voucher'
        Width = 200
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'trans_date'
        Footers = <>
        Title.Caption = 'Tanggal'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'module_id'
        Footers = <>
        Visible = False
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'code_cust'
        Footers = <>
        Visible = False
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'name_cust'
        Footers = <>
        Title.Caption = 'Nama Pelanggan'
        Width = 250
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'code_region'
        Footers = <>
        Visible = False
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'jum_kas'
        Footers = <>
        Title.Caption = 'Perkiraan Yang Di Debet | Kas'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'account_name_bank'
        Footers = <>
        Title.Caption = 'Perkiraan Yang Di Debet | Bank | Nama Bank'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'account_number_bank'
        Footers = <>
        Title.Caption = 'Perkiraan Yang Di Debet | Bank | Nomor Rekening'
        Width = 200
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'jum_bank'
        Footers = <>
        Title.Caption = 'Perkiraan Yang Di Debet | Bank | Jumlah'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'jum_piutang'
        Footers = <>
        Title.Caption = 'Perkiraan Yang Di Kredit | Piutang'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'ak_lain_kre'
        Footers = <>
        Title.Caption = 'Perkiraan Yang Di Kredit | Lain - Lain | Kode Akun'
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'ak_nm_lain_kre'
        Footers = <>
        Title.Caption = 'Perkiraan Yang Di Kredit | Lain - Lain | Nama Akun'
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'jum_lain_kre'
        Footers = <>
        Title.Caption = 'Perkiraan Yang Di Kredit | Lain - Lain | Jumlah'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'ket_faktur'
        Footers = <>
        Title.Caption = 'Keterangan'
        Width = 300
      end>
    object RowDetailData: TRowDetailPanelControlEh
    end
  end
  object ActMenu: TActionManager
    Left = 1016
    Top = 432
    StyleName = 'Platform Default'
    object ActBaru: TAction
      Caption = 'Baru  '
    end
    object ActUpdate: TAction
      Caption = 'Update  '
    end
    object ActRO: TAction
      Caption = 'Refresh  '
    end
    object ActDel: TAction
      Caption = 'Delete'
    end
    object ActPrint: TAction
      Caption = 'Print  '
    end
    object ActApp: TAction
      Caption = 'Approve  '
      Enabled = False
    end
    object ActReject: TAction
      Caption = 'Reject  '
      Enabled = False
    end
    object ActClose: TAction
      Caption = 'CLose PO    '
      Enabled = False
    end
  end
  object dxBarManager1: TdxBarManager
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    Categories.Strings = (
      'Default')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    PopupMenuLinks = <>
    UseSystemFont = True
    Left = 1052
    Top = 432
    PixelsPerInch = 96
    object dxBarManager1Bar1: TdxBar
      Caption = 'Action'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 930
      FloatTop = 3
      FloatClientWidth = 56
      FloatClientHeight = 123
      ItemLinks = <
        item
          UserDefine = [udWidth]
          UserWidth = 154
          Visible = True
          ItemName = 'dtAwal'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 156
          Visible = True
          ItemName = 'dtAkhir'
        end
        item
          BeginGroup = True
          UserDefine = [udWidth]
          UserWidth = 269
          Visible = True
          ItemName = 'edKaresidenan'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 271
          Visible = True
          ItemName = 'edKabupaten'
        end
        item
          Visible = True
          ItemName = 'cbSBU'
        end>
      OneOnRow = False
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object dxBarManager1Bar2: TdxBar
      Caption = 'Laporan'
      CaptionButtons = <>
      DockedLeft = 608
      DockedTop = 0
      FloatLeft = 852
      FloatTop = 2
      FloatClientWidth = 51
      FloatClientHeight = 28
      ItemLinks = <
        item
          Visible = True
          ItemName = 'btSearch'
        end
        item
          Visible = True
          ItemName = 'btPreview'
        end
        item
          Visible = True
          ItemName = 'dxBarLargeButton1'
        end>
      OneOnRow = False
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object btExportExcel: TdxBarLargeButton
      Caption = 'Export Excel'
      Category = 0
      Hint = 'Export Excel'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        0D0A3C2F7376673E0D0A}
    end
    object btExportPDF: TdxBarLargeButton
      Caption = 'Export PDF'
      Category = 0
      Hint = 'Export PDF'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        377A222F3E0D0A3C2F7376673E0D0A}
    end
    object dtAwal: TcxBarEditItem
      Caption = 'Tanggal Awal  '
      Category = 0
      Hint = 'Tanggal Awal  '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object dtAkhir: TcxBarEditItem
      Caption = 'Tanggal Akhir '
      Category = 0
      Hint = 'Tanggal Akhir '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object edKaresidenan: TcxBarEditItem
      Caption = 'Karesidenan  '
      Category = 0
      Hint = 'Karesidenan  '
      Visible = ivAlways
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = edKaresidenanPropertiesButtonClick
      InternalEditValue = nil
    end
    object btPreview: TdxBarLargeButton
      Caption = 'Preview'
      Category = 0
      Hint = 'Preview'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        }
      OnClick = btPreviewClick
    end
    object btSearch: TdxBarLargeButton
      Caption = 'Search'
      Category = 0
      Hint = 'Search'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        2F673E0D0A09093C2F673E0D0A093C2F673E0D0A3C2F7376673E0D0A}
      OnClick = btSearchClick
    end
    object edKabupaten: TcxBarEditItem
      Caption = 'Kabupaten    '
      Category = 0
      Hint = 'Kabupaten    '
      Visible = ivAlways
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = edKabupatenPropertiesButtonClick
    end
    object dxBarLargeButton1: TdxBarLargeButton
      Caption = 'Daftar Penerimaan (Versi Accurate)'
      Category = 0
      Hint = 'Daftar Penerimaan (Versi Accurate)'
      Visible = ivAlways
      OnClick = dxBarLargeButton1Click
      HotGlyph.SourceDPI = 96
      HotGlyph.Data = {
        }
    end
    object cbSBU: TdxBarCombo
      Caption = 'SBU                '
      Category = 0
      Hint = 'SBU                '
      Visible = ivAlways
      ItemIndex = -1
    end
  end
  object QBHPenerimaanKasBank: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'SELECT a.*,code_karesidenan,code_kab,name_kab,ket_faktur from "p' +
        'ublic"."vbhpenerimaan_kas_bank" a   LEFT JOIN (SELECT "code_prov' +
        'ince", "code" as code_kab, "name" as name_kab,  "code_karesidena' +
        'n"  from t_region_regency WHERE deleted_at IS NULL)b   ON "left"' +
        '(code_region, 4)=b.code_kab  '
      
        'LEFT JOIN (SELECT voucher_no, STRING_AGG('#39' No. Faktur '#39' || no_in' +
        'voice_tax || '#39' Tgl.'#39' || date_invoice_tax, '#39', '#39') AS ket_faktur'
      
        'from t_cash_bank_acceptance_receivable GROUP BY voucher_no)cc ON' +
        ' a.voucher_no=cc.voucher_no '
      'where trans_date between '#39'2024-12-18'#39'  and '#39'2024-12-18'#39' '
      ' ORDER BY trans_date, voucher_no')
    Left = 948
    Top = 344
    object QBHPenerimaanKasBankvoucher_no: TStringField
      FieldName = 'voucher_no'
      Size = 100
    end
    object QBHPenerimaanKasBanktrans_date: TDateField
      FieldName = 'trans_date'
    end
    object QBHPenerimaanKasBankmodule_id: TIntegerField
      FieldName = 'module_id'
    end
    object QBHPenerimaanKasBankcode_cust: TStringField
      FieldName = 'code_cust'
      Size = 100
    end
    object QBHPenerimaanKasBankname_cust: TStringField
      FieldName = 'name_cust'
      Size = 255
    end
    object QBHPenerimaanKasBankcode_region: TStringField
      FieldName = 'code_region'
      Size = 50
    end
    object QBHPenerimaanKasBankfor_acceptance: TStringField
      FieldName = 'for_acceptance'
      Size = 255
    end
    object QBHPenerimaanKasBankdescription: TMemoField
      FieldName = 'description'
      OnGetText = QBHPenerimaanKasBankdescriptionGetText
      BlobType = ftMemo
    end
    object QBHPenerimaanKasBankjum_kas: TFloatField
      FieldName = 'jum_kas'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object QBHPenerimaanKasBankaccount_name_bank: TStringField
      FieldName = 'account_name_bank'
      Size = 255
    end
    object QBHPenerimaanKasBankaccount_number_bank: TStringField
      FieldName = 'account_number_bank'
      Size = 100
    end
    object QBHPenerimaanKasBankjum_bank: TFloatField
      FieldName = 'jum_bank'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object QBHPenerimaanKasBankjum_piutang: TFloatField
      DisplayLabel = 'jum_bank'
      FieldName = 'jum_piutang'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object QBHPenerimaanKasBankak_lain_kre: TMemoField
      FieldName = 'ak_lain_kre'
      OnGetText = QBHPenerimaanKasBankak_lain_kreGetText
      BlobType = ftMemo
    end
    object QBHPenerimaanKasBankak_nm_lain_kre: TMemoField
      FieldName = 'ak_nm_lain_kre'
      OnGetText = QBHPenerimaanKasBankak_nm_lain_kreGetText
      BlobType = ftMemo
    end
    object QBHPenerimaanKasBankjum_lain_kre: TIntegerField
      FieldName = 'jum_lain_kre'
      DisplayFormat = '#,##0.##'
      EditFormat = '#,##0.##'
    end
    object QBHPenerimaanKasBankket_faktur: TMemoField
      FieldName = 'ket_faktur'
      ReadOnly = True
      OnGetText = QBHPenerimaanKasBankket_fakturGetText
      BlobType = ftMemo
    end
  end
  object DsBHPenerimaanKasBank: TDataSource
    DataSet = QBHPenerimaanKasBank
    Left = 1092
    Top = 344
  end
  object dsCetak: TDataSource
    DataSet = QCetak
    Left = 1041
    Top = 72
  end
  object Report: TfrxReport
    Version = '2022.1.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick, pbCopy, pbSelection]
    PreviewOptions.Zoom = 1.000000000000000000
    PreviewOptions.ZoomMode = zmWholePage
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 45673.924529294000000000
    ReportOptions.LastChange = 45674.410541099540000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      ''
      'begin'
      ''
      'end.')
    Left = 992
    Top = 32
    Datasets = <
      item
        DataSet = frxDBDDaftarPenerimaan
        DataSetName = 'frxDBDDaftarPenerimaan'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 5.000000000000000000
      BottomMargin = 5.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 120.944960000000000000
        Top = 18.897650000000000000
        Width = 755.906000000000000000
        object nama_pt: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Top = 0.102350000000000000
          Width = 748.346940000000000000
          Height = 26.456710000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Trebuchet MS'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'nama_pt')
          ParentFont = False
        end
        object periode: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Top = 52.338590000000000000
          Width = 748.346940000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'periode')
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Top = 22.015770000000000000
          Width = 748.346940000000000000
          Height = 37.795300000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -24
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Ringkasan Daftar Penerimaan')
          ParentFont = False
        end
        object wilayah: TfrxMemoView
          AllowVectorExport = True
          Left = 310.529444110000000000
          Top = 75.590600000000000000
          Width = 442.205010000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            'Wilayah :')
          ParentFont = False
        end
        object Memo28: TfrxMemoView
          AllowVectorExport = True
          Left = 60.472480000000000000
          Top = 98.267780000000000000
          Width = 91.535339230000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = 16744448
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Tanggal')
          ParentFont = False
        end
        object Memo29: TfrxMemoView
          AllowVectorExport = True
          Left = 263.481654900000000000
          Top = 98.267780000000000000
          Width = 237.993722030000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = 16744448
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Nama Akun Kas / Bank')
          ParentFont = False
        end
        object Memo30: TfrxMemoView
          AllowVectorExport = True
          Left = 507.732983500000000000
          Top = 98.267780000000000000
          Width = 164.764530630000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = 16744448
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Nilai')
          ParentFont = False
        end
        object Memo31: TfrxMemoView
          AllowVectorExport = True
          Left = 158.968126610000000000
          Top = 98.267780000000000000
          Width = 98.255921720000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = 16744448
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'No Cek')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 18.897637800000000000
        Top = 200.315090000000000000
        Width = 755.906000000000000000
        DataSet = frxDBDDaftarPenerimaan
        DataSetName = 'frxDBDDaftarPenerimaan'
        RowCount = 0
        Stretched = True
        object frxDBDBHPenj_dettrans_date: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 159.118110240000000000
          Width = 98.267716540000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDDaftarPenerimaan."cheque"]')
          ParentFont = False
        end
        object frxDBDBHPenj_detname_cust: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 263.433070870000000000
          Width = 238.110236220000000000
          Height = 18.897650000000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDDaftarPenerimaan."name_account"]')
          ParentFont = False
        end
        object frxDBDBHPenj_detjum_ak_deb: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 507.590551180000000000
          Width = 164.787401570000000000
          Height = 18.897650000000000000
          DisplayFormat.FormatStr = '%2.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDDaftarPenerimaan."total"]')
          ParentFont = False
        end
        object Memo5: TfrxMemoView
          IndexTag = 1
          AllowVectorExport = True
          Left = 60.472440940000000000
          Width = 91.464566930000000000
          Height = 18.897650000000000000
          DisplayFormat.FormatStr = 'dd mmm yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDDaftarPenerimaan."trans_date"]')
          ParentFont = False
        end
      end
      object PageFooter1: TfrxPageFooter
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 52.913420000000000000
        Top = 279.685220000000000000
        Width = 755.906000000000000000
        object Memo1: TfrxMemoView
          Align = baBottom
          AllowVectorExport = True
          Left = 438.047527000000000000
          Top = 34.015770000000000000
          Width = 313.700990000000000000
          Height = 18.897650000000000000
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            'Halaman [Page#] dari [TotalPages#]')
        end
        object Memo9: TfrxMemoView
          Align = baBottom
          AllowVectorExport = True
          Left = 3.779530000000000000
          Top = 34.015770000000000000
          Width = 389.291590000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'Tercetak Pada Tanggal [Date] [Time]')
          ParentFont = False
        end
        object Line1: TfrxLineView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Top = 7.559060000000000000
          Width = 748.346940000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Top = 6.897650000000000000
          Width = 748.346940000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Smart Backoffice Automation System Report')
          ParentFont = False
        end
      end
    end
  end
  object frxDBDBHPenerimaan: TfrxDBDataset
    UserName = 'frxDBDBHPenerimaan'
    CloseDataSource = False
    FieldAliases.Strings = (
      'voucher_no=voucher_no'
      'trans_date=trans_date'
      'module_id=module_id'
      'code_cust=code_cust'
      'name_cust=name_cust'
      'code_region=code_region'
      'for_acceptance=for_acceptance'
      'description=description'
      'jum_kas=jum_kas'
      'account_name_bank=account_name_bank'
      'account_number_bank=account_number_bank'
      'jum_bank=jum_bank'
      'jum_piutang=jum_piutang'
      'ak_lain_kre=ak_lain_kre'
      'ak_nm_lain_kre=ak_nm_lain_kre'
      'jum_lain_kre=jum_lain_kre'
      'code_karesidenan=code_karesidenan'
      'code_kab=code_kab'
      'name_kab=name_kab'
      'ket_faktur=ket_faktur')
    DataSet = QCetak
    BCDToCurrency = False
    DataSetOptions = []
    Left = 944
    Top = 72
  end
  object QCetak: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'SELECT a.*,code_karesidenan,code_kab,name_kab,ket_faktur from "p' +
        'ublic"."vbhpenerimaan_kas_bank" a   LEFT JOIN (SELECT "code_prov' +
        'ince", "code" as code_kab, "name" as name_kab,  "code_karesidena' +
        'n"  from t_region_regency WHERE deleted_at IS NULL)b   ON "left"' +
        '(code_region, 4)=b.code_kab  LEFT JOIN (SELECT voucher_no,  STRI' +
        'NG_AGG( '#39' No. Faktur '#39' || no_invoice_tax || '#39' Tgl. '#39' || date_inv' +
        'oice_tax, '#39', '#39') AS ket_faktur  from t_cash_bank_acceptance_recei' +
        'vable GROUP BY voucher_no)cc ON a.voucher_no=cc.voucher_no  wher' +
        'e trans_date between '#39'2024-12-18'#39'  and '#39'2024-12-18'#39' '
      ' ORDER BY trans_date, voucher_no')
    Left = 860
    Top = 72
    object QCetakvoucher_no: TStringField
      FieldName = 'voucher_no'
      Size = 100
    end
    object QCetaktrans_date: TDateField
      FieldName = 'trans_date'
    end
    object QCetakmodule_id: TIntegerField
      FieldName = 'module_id'
    end
    object QCetakcode_cust: TStringField
      FieldName = 'code_cust'
      Size = 100
    end
    object QCetakname_cust: TStringField
      FieldName = 'name_cust'
      Size = 255
    end
    object QCetakcode_region: TStringField
      FieldName = 'code_region'
      Size = 50
    end
    object QCetakfor_acceptance: TStringField
      FieldName = 'for_acceptance'
      Size = 255
    end
    object QCetakdescription: TMemoField
      FieldName = 'description'
      BlobType = ftMemo
    end
    object QCetakjum_kas: TFloatField
      FieldName = 'jum_kas'
    end
    object QCetakaccount_name_bank: TStringField
      FieldName = 'account_name_bank'
      Size = 255
    end
    object QCetakaccount_number_bank: TStringField
      FieldName = 'account_number_bank'
      Size = 100
    end
    object QCetakjum_bank: TFloatField
      FieldName = 'jum_bank'
    end
    object QCetakjum_piutang: TFloatField
      FieldName = 'jum_piutang'
    end
    object QCetakak_lain_kre: TMemoField
      FieldName = 'ak_lain_kre'
      BlobType = ftMemo
    end
    object QCetakak_nm_lain_kre: TMemoField
      FieldName = 'ak_nm_lain_kre'
      BlobType = ftMemo
    end
    object QCetakjum_lain_kre: TIntegerField
      FieldName = 'jum_lain_kre'
    end
    object QCetakcode_karesidenan: TStringField
      FieldName = 'code_karesidenan'
      ReadOnly = True
      Size = 255
    end
    object QCetakcode_kab: TStringField
      FieldName = 'code_kab'
      ReadOnly = True
      Size = 255
    end
    object QCetakname_kab: TStringField
      FieldName = 'name_kab'
      ReadOnly = True
      Size = 255
    end
    object QCetakket_faktur: TMemoField
      FieldName = 'ket_faktur'
      ReadOnly = True
      BlobType = ftMemo
    end
  end
  object QDaftarPenerimaan: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'SELECT a.*,code_karesidenan,code_kab,name_kab,ket_faktur from "p' +
        'ublic"."vdaftar_penerimaan_kasbank_accurate" a   LEFT JOIN (SELE' +
        'CT "code_province", "code" as code_kab, "name" as name_kab,  "co' +
        'de_karesidenan"  from t_region_regency WHERE deleted_at IS NULL)' +
        'b   ON "left"(code_region, 4)=b.code_kab  LEFT JOIN (SELECT vouc' +
        'her_no,  STRING_AGG( '#39' No. Faktur '#39' || no_invoice_tax || '#39' Tgl. ' +
        #39' || date_invoice_tax, '#39', '#39') AS ket_faktur  from t_cash_bank_acc' +
        'eptance_receivable GROUP BY voucher_no)cc ON a.voucher_no=cc.vou' +
        'cher_no  where trans_date between '#39'2024-12-18'#39'  and '#39'2024-12-18'#39 +
        ' '
      ' ORDER BY trans_date, voucher_no')
    Left = 860
    Top = 120
    object QDaftarPenerimaanvoucher_no: TStringField
      FieldName = 'voucher_no'
      Size = 100
    end
    object QDaftarPenerimaantrans_date: TDateField
      FieldName = 'trans_date'
    end
    object QDaftarPenerimaancode_cust: TStringField
      FieldName = 'code_cust'
      Size = 100
    end
    object QDaftarPenerimaanname_cust: TStringField
      FieldName = 'name_cust'
      Size = 255
    end
    object QDaftarPenerimaancode_region: TStringField
      FieldName = 'code_region'
      Size = 50
    end
    object QDaftarPenerimaanfor_acceptance: TStringField
      FieldName = 'for_acceptance'
      Size = 255
    end
    object QDaftarPenerimaandescription: TMemoField
      FieldName = 'description'
      BlobType = ftMemo
    end
    object QDaftarPenerimaancheque: TMemoField
      FieldName = 'cheque'
      BlobType = ftMemo
    end
    object QDaftarPenerimaancode_account: TStringField
      FieldName = 'code_account'
      Size = 100
    end
    object QDaftarPenerimaanname_account: TStringField
      FieldName = 'name_account'
      Size = 255
    end
    object QDaftarPenerimaantotal: TFloatField
      FieldName = 'total'
    end
    object QDaftarPenerimaancode_karesidenan: TStringField
      FieldName = 'code_karesidenan'
      ReadOnly = True
      Size = 255
    end
    object QDaftarPenerimaancode_kab: TStringField
      FieldName = 'code_kab'
      ReadOnly = True
      Size = 255
    end
    object QDaftarPenerimaanname_kab: TStringField
      FieldName = 'name_kab'
      ReadOnly = True
      Size = 255
    end
    object QDaftarPenerimaanket_faktur: TMemoField
      FieldName = 'ket_faktur'
      ReadOnly = True
      BlobType = ftMemo
    end
  end
  object frxDBDDaftarPenerimaan: TfrxDBDataset
    UserName = 'frxDBDDaftarPenerimaan'
    CloseDataSource = False
    FieldAliases.Strings = (
      'voucher_no=voucher_no'
      'trans_date=trans_date'
      'code_cust=code_cust'
      'name_cust=name_cust'
      'code_region=code_region'
      'for_acceptance=for_acceptance'
      'description=description'
      'cheque=cheque'
      'code_account=code_account'
      'name_account=name_account'
      'total=total'
      'code_karesidenan=code_karesidenan'
      'code_kab=code_kab'
      'name_kab=name_kab'
      'ket_faktur=ket_faktur')
    DataSet = QDaftarPenerimaan
    BCDToCurrency = False
    DataSetOptions = []
    Left = 944
    Top = 120
  end
  object dsDaftarPenerimaan: TDataSource
    DataSet = QDaftarPenerimaan
    Left = 1041
    Top = 120
  end
end
