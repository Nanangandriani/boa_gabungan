object F_catatan_ppn_pph: TF_catatan_ppn_pph
  Left = 0
  Top = 0
  Caption = 'F_catatan_ppn_pph'
  ClientHeight = 442
  ClientWidth = 960
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  TextHeight = 15
  object dxRibbon1: TdxRibbon
    Left = 0
    Top = 0
    Width = 960
    Height = 127
    BarManager = dxBarManager1
    Style = rs2010
    ColorSchemeName = 'Blue'
    Contexts = <>
    TabOrder = 0
    TabStop = False
    ExplicitWidth = 954
    object dxRibbon1Tab1: TdxRibbonTab
      Active = True
      Caption = 'Home'
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar1'
        end>
      Index = 0
    end
  end
  object DBGridCatatan_ppn_pph: TDBGridEh
    Left = 0
    Top = 127
    Width = 960
    Height = 315
    Align = alClient
    DataSource = DSCatatan_ppn_pph
    DrawMemoText = True
    DynProps = <>
    IndicatorOptions = [gioShowRowIndicatorEh, gioShowRecNoEh]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghDialogFind, dghShowRecNo, dghColumnResize, dghColumnMove, dghExtendVertLines]
    RowDetailPanel.Active = True
    SearchPanel.Enabled = True
    TabOrder = 1
    TitleParams.MultiTitle = True
    Columns = <
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'voucher'
        Footers = <>
        Title.Caption = 'No. BK'
        Width = 120
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'tanggal'
        Footers = <>
        Title.Caption = 'Tgl. BK'
        Width = 80
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'kepada'
        Footers = <>
        Title.Caption = 'Nama Supplier'
        Width = 200
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'Tgl. Faktur /Invoice'
        Width = 80
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'nofaktur'
        Footers = <>
        Title.Caption = 'No. Faktur /Invoice'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Width = 0
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'dpp'
        Footers = <>
        Title.Caption = 'Dpp'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'ppn'
        Footers = <>
        Title.Caption = 'Ppn'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pph23'
        Footers = <>
        Title.Caption = 'Pph Ps. 23'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pph22'
        Footers = <>
        Title.Caption = 'Pph Ps. 22'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pph15'
        Footers = <>
        Title.Caption = 'Pph Ps.15'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'pph4_2'
        Footers = <>
        Title.Caption = 'Pph Ps. 4 (2)'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'pph4_2'
        Footers = <>
        Width = 0
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'Total'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'Keterangan'
        Width = 200
      end>
    object RowDetailData: TRowDetailPanelControlEh
      object DBGridEh1: TDBGridEh
        Left = 0
        Top = 0
        Width = 912
        Height = 118
        Align = alClient
        DynProps = <>
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
        TabOrder = 0
        Visible = False
        object RowDetailData: TRowDetailPanelControlEh
        end
      end
    end
  end
  object DSCatatan_ppn_pph: TDataSource
    DataSet = MemCatatan_ppn_pph
    Left = 312
    Top = 224
  end
  object QPerusahaan: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      'select * from t_company')
    Active = True
    Left = 817
    Top = 40
  end
  object QCatatan_ppn_pph: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      '/*--Hutang PPH PS22  2130.06'#9'2130'#9'Hutang Pajak - PPh.Ps.22*/'
      
        'SELECT voucher,'#9'voucher_tmp,'#9'tanggal,'#9'account_code,'#9'deskripsi,'#9'k' +
        'epada,faktur_no'#9'nofaktur,'#9'saldo_kredit,'#9'saldo_debit,'#9'dpp,'#9'saldo,' +
        ' ppn,(case when account_code='#39'2130.01'#39' then saldo else 0 end)pph' +
        '21,'
      '(case when account_code='#39'2130.06'#39' then saldo else 0 end) pph22,'
      '(case when account_code='#39'2130.02'#39' then saldo else 0 end) pph23, '
      '(case when account_code='#39'2130.07'#39' then saldo else 0 end) pph15,'
      
        '(case when account_code='#39'2130.04'#39' then saldo else 0 end) pph4_2 ' +
        'from'
      '(select yy.* from '
      '(  '
      
        '(select distinct xx.voucher,xx.voucher_tmp,xx.tanggal,xx.account' +
        '_code,v.deskripsi,xx.kepada,t.faktur_no,0 ::float saldo_kredit,0' +
        ' ::float saldo_debit,case WHEN v.dpp is null then 0 else v.dpp e' +
        'nd dpp,  case WHEN t.npph is null then 0 else t.npph end saldo, ' +
        '0 as ppn from  '
      
        '(select a.voucher,b.voucher_tmp,b.tanggal,a.account_code,a.descr' +
        'iption as deskripsi,b.kepada,a.saldo saldo_kredit,case when c.sa' +
        'ldo  is null then 0 else c.saldo end saldo_debit from  '
      
        '(select * from t_credit_trx_real where account_code='#39'2130.06'#39')a ' +
        '  '
      
        'left join (select distinct voucher_no,voucher_tmp,to_ as kepada,' +
        'trans_date as tanggal from (SELECT voucher_no,a.voucher_tmp,a.to' +
        '_,a.trans_date from t_cash_bank_expenditure a INNER JOIN t_cash_' +
        'bank_expenditure_det b on a.voucher_no=b.no_voucher)x where tran' +
        's_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39')b on a.voucher=b.vo' +
        'ucher_no '
      
        'left join t_debit_trx_real c on a.voucher=c.voucher where b.tang' +
        'gal between '#39'2025-01-01'#39' and '#39'2025-04-21'#39'   '
      'UNION ALL '
      
        '(SELECT voucher_no,voucher_tmp,trans_date,code_account,deskripsi' +
        ',kepada,paid_amount as jumlah,saldo_debit from v_titipan_bpjs_pe' +
        'nerimaan_kas where code_account='#39'2130.06'#39' and trans_date between' +
        ' '#39'2025-01-01'#39' and '#39'2025-04-21'#39') '
      'UNION ALL '
      
        '(select voucher_no,'#39#39' voucher_tmp,trans_date,code_account,descri' +
        'ption as keterangan,case when code_account='#39'2130.02'#39' then '#39'PPH 2' +
        '3'#39' when code_account='#39'2130.01'#39' then '#39'PPH 21'#39' END kepada,paid_amo' +
        'unt saldo_kredit, 0 saldo_debit FROM (SELECT a.voucher_no,b.tran' +
        's_date,a.description,b.code_account,b.paid_amount FROM t_cash_ba' +
        'nk_acceptance a INNER JOIN t_cash_bank_acceptance_det b on a.vou' +
        'cher_no=b.voucher_no)x where trans_date between '#39'2025-01-01'#39' and' +
        ' '#39'2025-04-21'#39' and code_account='#39'2130.06'#39'  order by trans_date,vo' +
        'ucher_no Asc))xx '
      'LEFT JOIN '
      
        '(select DISTINCT k.voucher_no,K.trans_date as periode1,k.kepada,' +
        'l.supplier_code from(SELECT a.voucher_no,a.trans_date,a.to_ as k' +
        'epada from t_cash_bank_expenditure a INNER JOIN t_cash_bank_expe' +
        'nditure_det b on a.voucher_no=b.no_voucher)k '
      
        'left join t_supplier l on k.kepada=l.supplier_name where k.trans' +
        '_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39')r on xx.voucher=r.vo' +
        'ucher_no  LEFT JOIN t_paid_debt_det t on t.periode1=r.periode1 a' +
        'nd r.supplier_code=t.supplier_code '
      
        'LEFT JOIN "V_do_lpb2" v  on t.faktur_no=v.no_fp_no_inv where cas' +
        'e WHEN t.npph is null then 0 else t.npph end <> 0 ) '
      'UNION ALL '
      
        '(select a.voucher_no voucher, b.voucher_tmp, b.trans_date tangga' +
        'l, b.code_account, c.deskripsi,b.to_ as kepada, c.no_fp_no_inv n' +
        'ofaktur,0 ::float saldo_kredit, 0 ::float saldo_debit,c.dpp,c.pp' +
        'h23 saldo, 0 ppn from t_payment_detail_real a '
      
        'left join (SELECT a.voucher_no,a.voucher_tmp,b.trans_date,b.code' +
        '_account,a.to_  from t_cash_bank_expenditure a INNER JOIN t_cash' +
        '_bank_expenditure_det b on a.voucher_no=b.no_voucher)b  on a.vou' +
        'cher_no=b.voucher_no left join "V_do_lpb2" c on a.voucher_no=c.v' +
        'oucher  where b.code_account='#39'2130.06'#39'  and b.trans_date  betwee' +
        'n '#39'2025-01-01'#39' and '#39'2025-04-21'#39'  and  a.source_id=3 and no_fp_no' +
        '_inv is not null) '
      'UNION ALL '
      
        '(select distinct m.voucher_no,m.voucher_tmp,m.trans_date as tang' +
        'gal,n.account_code,(case when n.description1='#39#39' then n.descripti' +
        'on else n.description end) deskripsi,m.to_ as kepada,n.inv_supp_' +
        'no nofaktur,0 ::float saldo_kredit,0 ::float saldo_debit,n.dpp,n' +
        '.saldo, 0 ppn from (SELECT a.voucher_no,a.voucher_tmp,b.trans_da' +
        'te,a.to_ from t_cash_bank_expenditure a INNER JOIN t_cash_bank_e' +
        'xpenditure_det b on a.voucher_no=b.no_voucher) m , t_credit_trx_' +
        'real n where m.voucher_no=n.voucher and n.account_code='#39'2130.06'#39 +
        ' and m.trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39') '
      'UNION ALL '
      
        '(SELECT a.voucher_no as voucher,'#39#39' voucher_tmp,a.trans_date as t' +
        'anggal,a.code_account,a.description as deskripsi,a.description a' +
        's kepada,'#39#39' as nofaktur, 0 saldo_kredit,  0 saldo_debit,0 as dpp' +
        ',a.paid_amount as saldo, 0 ppn from (SELECT a.voucher_no,a.trans' +
        '_date,b.code_account,a.description,b.paid_amount FROM t_cash_ban' +
        'k_acceptance a INNER JOIN t_cash_bank_acceptance_det b on a.vouc' +
        'her_no=b.voucher_no)a where trans_date between '#39'2025-01-01'#39' and ' +
        #39'2025-04-21'#39'  and code_account='#39'2130.06'#39' order by trans_date,vou' +
        'cher_no Asc) '
      'UNION ALL '
      
        'SELECT '#39#39' as voucher,'#39#39' as voucher_tmp,b.trans_date,a.account_pp' +
        'h_code, (case when a.account_pph_code='#39'2142'#39' then '#39'PPH PS 23'#39' wh' +
        'en a.account_pph_code='#39'2141'#39' then '#39'PPH PS 21'#39' when a.account_pph' +
        '_code='#39'2143'#39' then '#39'PPH PS 25'#39'  when a.account_pph_code='#39'2146'#39' th' +
        'en '#39'PPH PS 4(2)'#39' when a.account_pph_code='#39'2147'#39' then '#39'PPH PS 15'#39 +
        '  when a.account_pph_code='#39'2148'#39' then '#39'PPH PS 22'#39' else a.account' +
        '_pph_code end) as deskripsi,supplier_name as kepada,b.faktur_no,' +
        '0 saldo_kredit,0 saldo_debit,a.subtotalrp as dpp,a.pph_rp as sal' +
        'do,a.ppn_rp from t_purchase_invoice_det a  left join t_purchase_' +
        'invoice b on a.trans_no=b.trans_no LEFT JOIN t_supplier d on b.s' +
        'upplier_code=d.supplier_code where b.trans_date > '#39'2023-01-01'#39' a' +
        'nd b.trans_date BETWEEN '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and a.acco' +
        'unt_pph_code='#39'2130.06'#39'  '
      'UNION ALL '
      
        '(SELECT a.bk_no as voucher,'#39#39' as voucher_tmp,a.trans_date,b.acco' +
        'unt_code,a.notes as deskripsi,'#39#39' as kepada,'#39#39'as nofaktur,0 saldo' +
        '_kredit,0 saldo_debit,0 as dpp,-(b.debit) as saldo,0 ppn FROM t_' +
        'memorial_journal a '
      
        'LEFT JOIN t_memorial_journal_detail b ON a.memo_no=b.memo_no WHE' +
        'RE b.account_code='#39'2130.06'#39' and a.trans_date BETWEEN '#39'2025-01-01' +
        #39' and '#39'2025-04-21'#39' order by a.trans_date asc)'
      
        ')yy where yy.voucher is not null  order by yy.tanggal,yy.voucher' +
        ',yy.faktur_no asc)xx '
      ''
      'UNION All'
      '   /*--Hutang PPH PS23  2130.02'#9'2130'#9'Hutang Pajak - PPh.Ps.23 */'
      
        'SELECT voucher,'#9#39#39' as voucher_tmp,'#9'tanggal,kodesp as'#9'kode_accoun' +
        't,'#9'deskripsi,'#9'kepada,'#9'nofaktur,'#9'0 saldo_kredit,'#9'0 saldo_debit,'#9'd' +
        'pp,'#9'pph as saldo,ppn,'
      '(case when kodesp='#39'2130.01'#39' then pph else 0 end) pph21,'
      '(case when kodesp='#39'2130.06'#39' then pph else 0 end) pph22,'
      '(case when kodesp='#39'2130.02'#39' then pph else 0 end) pph23, '
      '(case when kodesp='#39'2130.07'#39' then pph else 0 end) pph15,'
      '(case when kodesp='#39'2130.04'#39' then pph else 0 end) pph4_2 FROM'
      '( SELECT yyy.* FROM '
      '(SELECT yy.* FROM '
      '( '
      
        'SELECT  a.voucher,a.tanggal,a.kodesp,x.deskripsi,kepada,a.faktur' +
        '_no as nofaktur ,y.dpp as dpp,a.pph as pph,'#39#39' as nodo,a.supplier' +
        '_code as kdsup,y.ppn_rp as ppn FROM '
      
        '(SELECT DISTINCT a.*,b.faktur_no,c.saldo as pph,b.supplier_code ' +
        'FROM '
      
        '(SELECT DISTINCT voucher_no as voucher,trans_date tanggal,to_ ke' +
        'pada,code_account as kodesp FROM (SELECT a.voucher_no,a.trans_da' +
        'te,a.to_,b.code_account,b."position" from t_cash_bank_expenditur' +
        'e a INNER JOIN t_cash_bank_expenditure_det b on a.voucher_no=b.n' +
        'o_voucher)x where trans_date between '#39'2025-01-01'#39' and '#39'2025-04-2' +
        '1'#39' and code_account='#39'2130.02'#39' and "position"='#39'K'#39')a  '
      
        'LEFT JOIN t_cash_bank_expenditure_payable b on a.voucher=b.vouch' +
        'er_no '
      
        'LEFT JOIN t_credit_trx_real c on a.voucher=c.voucher and b.faktu' +
        'r_no=c.inv_supp_no WHERE (c.voucher is not null ) and (b.faktur_' +
        'no is not null)) a '
      'LEFT JOIN  t_purchase_invoice f ON f.faktur_no=a.faktur_no '
      
        'LEFT JOIN (SELECT a.trans_no,string_agg (b.item_name, '#39', '#39' ) AS ' +
        'deskripsi FROM t_purchase_invoice_det a '
      
        'LEFT JOIN t_item_stock b ON a.item_stock_code::text = b.item_sto' +
        'ck_code::text GROUP BY a.trans_no) x On x.trans_no=f.trans_no LE' +
        'FT JOIN (SELECT trans_no,sum(subtotal) as dpp,ppn_rp FROM t_purc' +
        'hase_invoice_det GROUP BY trans_no,ppn_rp) y on f.trans_no=y.tra' +
        'ns_no '
      'UNION ALL '
      
        'SELECT aa.voucher_no,aa.trans_date tanggal,aa.code as kodesp,b.d' +
        'escription1,aa.to_ as kepada,b.inv_supp_no as nofaktur ,b.dpp as' +
        ' dpp,b.saldo as pph,'#39#39' as nodo,supplier_code as kdsup,0 ppn  FRO' +
        'M '
      
        '(SELECT a.*,c.faktur_no as no_inv_supp,b.inv_supp_no,e.code  FRO' +
        'M '
      
        '(SELECT distinct voucher_no,trans_date,supplier_code,to_ FROM (S' +
        'ELECT a.voucher_no,a.trans_date,a.supplier_code,a.to_,b.code_acc' +
        'ount,b."position" from t_cash_bank_expenditure a INNER JOIN t_ca' +
        'sh_bank_expenditure_det b on a.voucher_no=b.no_voucher)x where t' +
        'rans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and code_account' +
        '='#39'2130.02'#39' and "position"='#39'K'#39')a '
      'LEFT JOIN t_credit_trx_real b on a.voucher_no=b.voucher '
      
        'LEFT JOIN t_cash_bank_expenditure_payable c on a.voucher_no=c.vo' +
        'ucher_no'
      'INNER JOIN v_ak_account e on c.account_acc=e.account_code2 '
      'LEFT JOIN t_payment_detail_real d on a.voucher_no=d.voucher_no '
      
        'WHERE (b.voucher is not null) and (b.voucher is null)and (c.vouc' +
        'her_no is null))aa '
      'LEFT JOIN t_credit_trx_real b ON aa.voucher_no=b.voucher   '
      'UNION ALL '
      'SELECT e.voucher_no as voucher,b.trans_date,a.account_pph_code, '
      '(case when a.account_pph_code='#39'2130.02'#39' then '#39'PPH PS 23'#39' '
      'when a.account_pph_code='#39'2130.01'#39' then '#39'PPH PS 21'#39' '
      'when a.account_pph_code='#39'2130.03'#39' then '#39'PPH PS 25'#39'  '
      'when a.account_pph_code='#39'2130.04'#39' then '#39'PPH PS 4(2)'#39' '
      'when a.account_pph_code='#39'2130.07'#39' then '#39'PPH PS 15'#39'  '
      
        'when a.account_pph_code='#39'2130.06'#39' then '#39'PPH PS 22'#39' else a.accoun' +
        't_pph_code end) as deskripsi,d.supplier_name as kepada,b.faktur_' +
        'no,a.subtotalrp as dpp,a.pph_rp as pph,'#39#39' as nodo,d.supplier_cod' +
        'e,a.ppn_rp from t_purchase_invoice_det a  '
      'left join t_purchase_invoice b on a.trans_no=b.trans_no '
      'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code '
      
        'LEFT JOIN t_cash_bank_expenditure_payable e on a.trans_no=e.invo' +
        'ice_no'
      'INNER JOIN v_ak_account f on a.account_code=f.account_code2'
      
        'where b.trans_date >'#39'2023-01-01'#39' and b.trans_date BETWEEN '#39'2025-' +
        '01-01'#39' and '#39'2025-04-21'#39' and f.code='#39'2130.02'#39'    '
      'UNION ALL '
      
        'SELECT d.bk_no as voucher ,e.trans_date,'#39'2130.02'#39' as kdsp,'#39'PPH P' +
        'S 23'#39' as jenis_pph, c.supplier_name as kepada,'#39#39' as nofaktur ,su' +
        'btotal as dpp,b.pph_rp as pph, '#39#39' nodo,c.supplier_code,0 ppn FRO' +
        'M t_po a '
      'left join t_podetail b ON a.po_no=b.po_no '
      'left join t_supplier c on a.supplier_code=c.supplier_code '
      'left join t_payment_detail_real d on a.po_no=d.voucher_no '
      
        'LEFT JOIN (SELECT a.voucher_no,a.trans_date from t_cash_bank_exp' +
        'enditure a INNER JOIN t_cash_bank_expenditure_det b on a.voucher' +
        '_no=b.no_voucher)e on d.bk_no=e.voucher_no '
      'where trans_date BETWEEN '#39'2025-01-01'#39' and '#39'2025-04-21'#39'  '
      
        'and b.pph_rp<>0 GROUP BY e.trans_date,c.supplier_name,c.supplier' +
        '_code,d.bk_no,b.pph_rp,b.subtotal '
      'UNION ALL  '
      
        'SELECT distinct m.voucher_no voucher,m.trans_date as tanggal,n.a' +
        'ccount_code as kodesp,n.description1 deskripsi,m.to_ as kepada,n' +
        '.inv_supp_no nofaktur,n.dpp,n.saldo,'#39#39' as nodo,'#39#39' as kdsup, 0 pp' +
        'n from (SELECT a.voucher_no,a.trans_date,a.to_  from t_cash_bank' +
        '_expenditure a INNER JOIN t_cash_bank_expenditure_det b on a.vou' +
        'cher_no=b.no_voucher)m ,t_credit_trx_real n '
      
        'WHERE m.voucher_no=n.voucher and n.Account_code='#39'2130.02'#39' and de' +
        'scription1<>'#39#39' and m.trans_date between '#39'2025-01-01'#39' and '#39'2025-0' +
        '4-21'#39')yy order by yy.tanggal,yy.voucher,yy.nofaktur asc )yyy ORD' +
        'ER BY kepada,voucher,nofaktur,tanggal ASC)yyyy  '
      ''
      'UNION ALL '
      
        '/*--Hutang PPH Pasal 15  2130.07'#9'2130'#9'Hutang Pajak - PPh.Ps.15 *' +
        '/ '
      
        'SELECT voucher,voucher_tmp,tanggal,account_code,deskripsi,kepada' +
        ',faktur_no,saldo_kredit,saldo_debit,dpp,saldo,0 ppn,'
      '(case when account_code='#39'2130.01'#39' then saldo else 0 end) pph21,'
      '(case when account_code='#39'2130.06'#39' then saldo else 0 end) pph22,'
      '(case when account_code='#39'2130.02'#39' then saldo else 0 end) pph23,'
      '(case when account_code='#39'2130.07'#39' then saldo else 0 end) pph15,'
      
        '(case when account_code='#39'2130.04'#39' then saldo else 0 end) pph4_2 ' +
        'FROM  '
      '(select yy.* from '
      '( '
      
        '(select distinct xx.voucher,xx.voucher_tmp,xx.tanggal,xx.account' +
        '_code,v.deskripsi,xx.kepada,  t.faktur_no,0 ::float saldo_kredit' +
        ',0 ::float saldo_debit,case WHEN v.dpp is null then 0 else v.dpp' +
        ' end dpp,  case WHEN t.npph is null then 0 else t.npph end saldo' +
        ',0 ppn from  '
      
        '(select a.voucher,b.voucher_tmp,b.tanggal,a.account_code,a.descr' +
        'iption as deskripsi,b.kepada,a.saldo saldo_kredit,case when c.sa' +
        'ldo  is null then 0 else c.saldo end saldo_debit from  '
      
        '(select * from t_credit_trx_real where account_code='#39'2130.07'#39')a ' +
        ' '
      'left join   '
      
        '(select distinct voucher_no,voucher_tmp,to_ as kepada,trans_date' +
        ' as tanggal from (SELECT a.voucher_no,a.voucher_tmp,a.to_,a.tran' +
        's_date from t_cash_bank_expenditure a INNER JOIN t_cash_bank_exp' +
        'enditure_det b on a.voucher_no=b.no_voucher)a '
      'where  '
      
        'trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39')b on a.voucher=' +
        'b.voucher_no  '
      
        'left join t_debit_trx_real c on a.voucher=c.voucher where b.tang' +
        'gal between '#39'2025-01-01'#39' and '#39'2025-04-21'#39'   '
      'UNION ALL'
      
        '(SELECT voucher_no,voucher_tmp,trans_date,code_account,descripti' +
        'on,kepada,paid_amount,saldo_debit from v_titipan_bpjs_penerimaan' +
        '_kas where code_account='#39'2130.07'#39' and trans_date between '#39'2025-0' +
        '1-01'#39' and '#39'2025-04-21'#39')  '
      'UNION ALL '
      
        '(select voucher_no,'#39#39' voucher_tmp,trans_date,code_account,descri' +
        'ption deskripsi,case when code_account='#39'2130.02'#39' then '#39'PPH 23'#39' w' +
        'hen code_account='#39'2130.01'#39' then '#39'PPH 21'#39' END kepada, paid_amount' +
        ' saldo_kredit, 0 saldo_debit from '
      
        '(SELECT  a.voucher_no,a.trans_date,b.code_account,a.description,' +
        'a.name_cust,b.paid_amount FROM t_cash_bank_acceptance a INNER JO' +
        'IN t_cash_bank_acceptance_det b on a.voucher_no=b.voucher_no)a '
      
        'where trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and code_' +
        'account='#39'2130.07'#39'  order by trans_date,voucher_no Asc ))xx  '
      'LEFT JOIN '
      
        '(select DISTINCT k.voucher_no voucher,K.trans_date as periode1,k' +
        '.kepada,l.supplier_code from (SELECT a.voucher_no,a.trans_date,a' +
        '.to_ as kepada from t_cash_bank_expenditure a INNER JOIN t_cash_' +
        'bank_expenditure_det b on a.voucher_no=b.no_voucher) k '
      
        'left join t_supplier l on k.kepada=l.supplier_name where k.trans' +
        '_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39') r on xx.voucher=r.v' +
        'oucher  '
      
        'LEFT JOIN t_paid_debt_det t on t.periode1=r.periode1  and r.supp' +
        'lier_code=t.supplier_code '
      
        'LEFT JOIN "V_do_lpb2" v  on t.faktur_no=v.no_fp_no_inv where cas' +
        'e WHEN t.npph is null then 0 else t.npph end <> 0 ) '
      'UNION ALL '
      
        '(select a.voucher_no voucher, b.voucher_tmp, b.trans_date tangga' +
        'l, b.code_account, c.deskripsi, kepada, c.no_fp_no_inv nofaktur,' +
        '0 ::float saldo_kredit, 0 ::float saldo_debit,c.dpp,c.pph23 sald' +
        'o,0 ppn from t_payment_detail_real a '
      
        'left join (SELECT a.voucher_no,a.voucher_tmp,a.trans_date,b.code' +
        '_account,a.to_ as kepada from t_cash_bank_expenditure a INNER JO' +
        'IN t_cash_bank_expenditure_det b on a.voucher_no=b.no_voucher)b ' +
        'on a.voucher_no=b.voucher_no '
      'left join "V_do_lpb2" c on a.voucher_no=c.voucher  '
      
        'where b.code_account='#39'2130.07'#39' and b.trans_date  between '#39'2025-0' +
        '1-01'#39' and '#39'2025-04-21'#39'  and  a.source_id=3 and no_fp_no_inv is n' +
        'ot null ) '
      'UNION ALL  '
      
        '(SELECT a.voucher_no as voucher,'#39#39' voucher_tmp,a.trans_date as t' +
        'anggal,a.code_account,a.description deskripsi,a.name_cust as kep' +
        'ada,'#39#39' as nofaktur, 0 saldo_kredit,  0 saldo_debit,0 as dpp,paid' +
        '_amount as saldo,0 ppn  from '
      
        '(SELECT a.voucher_no,a.trans_date,b.code_account,a.description,a' +
        '.name_cust,b.paid_amount FROM t_cash_bank_acceptance a INNER JOI' +
        'N t_cash_bank_acceptance_det b on a.voucher_no=b.voucher_no)a '
      
        'where trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39'  and code' +
        '_account='#39'2130.07'#39' order by trans_date,voucher_no Asc)'
      'UNION ALL  '
      
        'SELECT '#39#39' as voucher,'#39#39' as voucher_tmp,b.trans_date,a.account_pp' +
        'h_code, '
      '(case when a.account_pph_code='#39'2130.02'#39' then '#39'PPH PS 23'#39' '
      'when a.account_pph_code='#39'2130.01'#39' then '#39'PPH PS 21'#39'  '
      'when a.account_pph_code='#39'2130.03'#39' then '#39'PPH PS 25'#39'  '
      'when a.account_pph_code='#39'2130.04'#39' then '#39'PPH PS 4(2)'#39' '
      'when a.account_pph_code='#39'2130.07'#39' then '#39'PPH PS 15'#39'  '
      
        'when a.account_pph_code='#39'2130.06'#39' then '#39'PPH PS 22'#39' else a.accoun' +
        't_pph_code end) as deskripsi,supplier_name as kepada,b.faktur_no' +
        ',0 saldo_kredit,0 saldo_debit,a.subtotalrp as dpp,a.pph_rp as sa' +
        'ldo ,a.ppn_rp from t_purchase_invoice_det a  '
      'left join t_purchase_invoice b on a.trans_no=b.trans_no  '
      
        'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code where ' +
        'b.trans_date > '#39'2023-01-01'#39' and b.trans_date BETWEEN '#39'2025-01-01' +
        #39' and '#39'2025-04-21'#39' and a.account_pph_code='#39'2130.07'#39' and a.trans_' +
        'no not in (select invoice_no FROM t_cash_bank_expenditure_payabl' +
        'e)  '
      'UNION ALL '
      
        '(SELECT a.bk_no as voucher,'#39#39' as voucher_tmp,a.trans_date tangga' +
        'l,b.account_code,notes deskripsi,'#39#39' as kepada,'#39#39'as nofaktur,0 sa' +
        'ldo_kredit,0 saldo_debit,0 as dpp,-(b.debit) as saldo,0 ppn FROM' +
        ' t_memorial_journal a '
      
        'LEFT JOIN t_memorial_journal_detail b ON a.memo_no=b.memo_no WHE' +
        'RE b.account_code='#39'2130.07'#39' and a.trans_date between '#39'2025-01-01' +
        #39' and '#39'2025-04-21'#39' and b.debit <0  order by a.trans_date asc))yy' +
        ' where yy.voucher is not null  order by yy.tanggal,yy.voucher,yy' +
        '.faktur_no asc)z '
      ''
      'UNION ALL'
      '/* --Hutang PPH PS 25 2130.03'#9'2130'#9'Hutang Pajak - PPh.Ps.25 */ '
      
        'SELECT voucher,voucher_tmp,tanggal,account_code kode_account,des' +
        'kripsi,kepada,faktur_no,saldo_kredit,saldo_debit,dpp,saldo,0 ppn' +
        ','
      '(case when account_code='#39'2130.01'#39' then saldo else 0 end)pph21,'
      '(case when account_code='#39'2130.06'#39' then saldo else 0 end) pph22,'
      '(case when account_code='#39'2130.02'#39' then saldo else 0 end) pph23,'
      '(case when account_code='#39'2130.07'#39' then saldo else 0 end) pph15,'
      
        '(case when account_code='#39'2130.04'#39' then saldo else 0 end) pph4_2 ' +
        'FROM '
      '( select yy.* from '
      '('
      
        '(select distinct xx.voucher,xx.voucher_tmp,xx.tanggal,xx.account' +
        '_code,v.deskripsi,xx.kepada,  t.faktur_no,0 ::float saldo_kredit' +
        ',0 ::float saldo_debit,case WHEN v.dpp is null then 0 else v.dpp' +
        ' end dpp,  case WHEN t.npph is null then 0 else t.npph end saldo' +
        ', 0 ppn from  '
      
        '(select a.voucher,b.voucher_tmp,b.tanggal,a.account_code,a.descr' +
        'iption as deskripsi, b.kepada,a.saldo saldo_kredit,case when c.s' +
        'aldo  is null then 0 else c.saldo end saldo_debit from  '
      
        '(select * from t_credit_trx_real where account_code='#39'2130.03'#39')a ' +
        '  '
      'left join '
      
        '(select distinct voucher_no,voucher_tmp,to_ as kepada,trans_date' +
        ' as tanggal from '
      
        '(SELECT a.voucher_no,a.voucher_tmp,a.to_,a.trans_date from t_cas' +
        'h_bank_expenditure a INNER JOIN t_cash_bank_expenditure_det b on' +
        ' a.voucher_no=b.no_voucher)x where trans_date between '#39'2025-01-0' +
        '1'#39' and '#39'2025-04-21'#39')b on a.voucher=b.voucher_no   '
      
        'left join t_debit_trx_real c on a.voucher=c.voucher where b.tang' +
        'gal between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' '
      'UNION ALL  '
      
        '(SELECT voucher_no,voucher_tmp,trans_date,code_account,deskripsi' +
        ',kepada,paid_amount as jumlah,saldo_debit from v_titipan_bpjs_pe' +
        'nerimaan_kas where code_account='#39'2130.03'#39' and trans_date between' +
        ' '#39'2025-01-01'#39' and '#39'2025-04-21'#39') '
      'UNION ALL '
      
        '(select voucher_no,'#39#39' voucher_tmp,trans_date,code_account,descri' +
        'ption deskripsi, '
      
        'case when code_account='#39'2130.02'#39' then '#39'PPH 23'#39' when code_account' +
        '='#39'2130.01'#39' then '#39'PPH 21'#39' END kepada,paid_amount saldo_kredit,0 s' +
        'aldo_debit from '
      
        '(SELECT a.voucher_no,a.trans_date,b.code_account,a.description,b' +
        '.paid_amount FROM t_cash_bank_acceptance a INNER JOIN t_cash_ban' +
        'k_acceptance_det b on a.voucher_no=b.voucher_no)j where trans_da' +
        'te between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and code_account='#39'2130.' +
        '03'#39'  order by trans_date,voucher_no Asc ))xx  '
      
        'LEFT JOIN (select DISTINCT k.voucher_no,K.trans_date as periode1' +
        ',k.kepada,l.supplier_code from '
      
        '(SELECT a.voucher_no,a.trans_date,a.to_ as kepada from t_cash_ba' +
        'nk_expenditure a INNER JOIN t_cash_bank_expenditure_det b on a.v' +
        'oucher_no=b.no_voucher) k '
      
        'left join t_supplier l on k.kepada=l.supplier_name where k.trans' +
        '_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' ) r on xx.voucher=r.' +
        'voucher_no  '
      
        'LEFT JOIN t_paid_debt_det t on t.periode1=r.periode1 and r.suppl' +
        'ier_code=t.supplier_code '
      
        'LEFT JOIN "V_do_lpb2" v  on t.faktur_no=v.no_fp_no_inv where cas' +
        'e WHEN t.npph is null then 0 else t.npph end <> 0 )  '
      'UNION ALL '
      
        '(select a.voucher_no voucher,b.voucher_tmp,b.trans_date tanggal,' +
        'b.code_account,c.deskripsi,b.to_ as kepada,c.no_fp_no_inv nofakt' +
        'ur,0 ::float saldo_kredit, 0 ::float saldo_debit,c.dpp,c.pph23 s' +
        'aldo,0 ppn from t_payment_detail_real a '
      
        'left join (SELECT a.voucher_no,a.voucher_tmp,a.trans_date,b.code' +
        '_account,a.to_ from t_cash_bank_expenditure a INNER JOIN t_cash_' +
        'bank_expenditure_det b on a.voucher_no=b.no_voucher) b on a.vouc' +
        'her_no=b.voucher_no '
      
        'left join "V_do_lpb2" c on a.voucher_no=c.voucher  where b.code_' +
        'account='#39'2130.03'#39' and b.trans_date between '#39'2025-01-01'#39' and '#39'202' +
        '5-04-21'#39' and  a.source_id=3 and no_fp_no_inv is not null ) '
      'UNION ALL '
      
        '(select distinct m.voucher_no,m.voucher_tmp,m.trans_date as tang' +
        'gal,n.account_code,(case when n.description1='#39#39' then n.descripti' +
        'on else n.description1 end) deskripsi,m.to_ as kepada,n.inv_supp' +
        '_no nofaktur,0 ::float saldo_kredit,0 ::float saldo_debit,n.dpp,' +
        'n.saldo,0 ppn from '
      
        '(SELECT a.voucher_no,a.voucher_tmp,a.trans_date,a.to_ from t_cas' +
        'h_bank_expenditure a INNER JOIN t_cash_bank_expenditure_det b on' +
        ' a.voucher_no=b.no_voucher) m ,t_credit_trx_real n where m.vouch' +
        'er_no=n.voucher and n.Account_code='#39'2130.03'#39' and m.trans_date be' +
        'tween '#39'2025-01-01'#39' and '#39'2025-04-21'#39' ) '
      'UNION ALL'
      
        '(SELECT voucher_no as voucher,'#39#39' voucher_tmp,trans_date as tangg' +
        'al,code_account,description deskripsi,name_cust as kepada,'#39#39' as ' +
        'nofaktur, 0 saldo_kredit,  0 saldo_debit,0 as dpp,paid_amount as' +
        ' saldo,0 ppn from (SELECT a.voucher_no,a.trans_date,b.code_accou' +
        'nt,b.description,a.name_cust,b.paid_amount FROM t_cash_bank_acce' +
        'ptance a INNER JOIN t_cash_bank_acceptance_det b on a.voucher_no' +
        '=b.voucher_no)a where trans_date between '#39'2025-01-01'#39' and '#39'2025-' +
        '04-21'#39'  and code_account='#39'2130.03'#39' order by trans_date,voucher_n' +
        'o Asc) '
      'UNION ALL '
      
        'SELECT '#39#39' as voucher,'#39#39' as voucher_tmp,b.trans_date,a.account_pp' +
        'h_code, '
      '(case when a.account_pph_code='#39'2130.02'#39' then '#39'PPH PS 23'#39' '
      'when a.account_pph_code='#39'2130.01'#39' then '#39'PPH PS 21'#39' '
      'when a.account_pph_code='#39'2130.03'#39' then '#39'PPH PS 25'#39'  '
      'when a.account_pph_code='#39'2130.04'#39' then '#39'PPH PS 4(2)'#39' '
      'when a.account_pph_code='#39'2130.07'#39' then '#39'PPH PS 15'#39'  '
      
        'when a.account_pph_code='#39'2130.06'#39' then '#39'PPH PS 22'#39' else a.accoun' +
        't_pph_code end) as deskripsi, supplier_name as kepada,b.faktur_n' +
        'o,0 saldo_kredit,0 saldo_debit,a.subtotalrp as dpp,a.pph_rp as s' +
        'aldo,a.ppn_rp from t_purchase_invoice_det a  '
      'left join t_purchase_invoice b on a.trans_no=b.trans_no  '
      'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code '
      
        'where b.trans_date > '#39'2023-01-01'#39'   and b.trans_date BETWEEN '#39'20' +
        '25-01-01'#39' and '#39'2025-04-21'#39' and a.account_pph_code='#39'2130.03'#39'  '
      'UNION ALL  '
      
        '(SELECT a.bk_no as voucher,'#39#39' as voucher_tmp,a.trans_date,b.acco' +
        'unt_code,notes deskripsi,'#39#39' as kepada,'#39#39'as nofaktur,0 saldo_kred' +
        'it,0 saldo_debit,0 as dpp,-(b.debit) as saldo,0 ppn FROM t_memor' +
        'ial_journal a '
      
        'LEFT JOIN t_memorial_journal_detail b ON a.memo_no=b.memo_no WHE' +
        'RE b.account_code='#39'2130.03'#39' and a.trans_date between '#39'2025-01-01' +
        #39' and '#39'2025-04-21'#39' order by a.trans_date asc))yy where yy.vouche' +
        'r is not null  order by yy.tanggal,yy.voucher,yy.faktur_no asc)K' +
        'K   '
      ''
      'UNION ALL '
      '/*---Hutang PPH PS 4 ayat 2 */ '
      
        'SELECT voucher,'#9'voucher_tmp,tanggal,account_code,deskripsi,kepad' +
        'a,nofaktur,saldo_kredit,saldo_debit,dpp,saldo,0 ppn,'
      '(case when account_code='#39'2130.01'#39' then saldo else 0 end)pph21,'
      '(case when account_code='#39'2130.06'#39' then saldo else 0 end) pph22,'
      '(case when account_code='#39'2130.02'#39' then saldo else 0 end) pph23,'
      '(case when account_code='#39'2130.07'#39' then saldo else 0 end) pph15, '
      
        '(case when account_code='#39'2130.04'#39' then saldo else 0 end) pph4_2 ' +
        'FROM '
      '(  '
      
        '(select xx.voucher,xx.voucher_tmp,xx.tanggal,xx.account_code,xx.' +
        'deskripsi,xx.kepada,xx.nofaktur,0 ::float saldo_kredit,0 ::float' +
        ' saldo_debit,xx.dpp, case WHEN xx.saldo_kredit is null then 0 el' +
        'se xx.saldo_kredit end saldo, 0 ppn from '
      
        '(select a.voucher,b.voucher_tmp,b.trans_date tanggal,a.account_c' +
        'ode,a.description1 as deskripsi, b.kepada,a.inv_supp_no nofaktur' +
        ',a.saldo saldo_kredit,case when c.saldo  is null then 0 else c.s' +
        'aldo end saldo_debit,A.dpp from '
      
        '(select * from t_credit_trx_real where account_code='#39'2130.04'#39')a ' +
        ' '
      'left join '
      '(select distinct voucher_no,voucher_tmp,kepada,trans_date  from '
      
        '(SELECT a.voucher_no,a.voucher_tmp,a.trans_date,a.to_ as kepada ' +
        'from t_cash_bank_expenditure a INNER JOIN t_cash_bank_expenditur' +
        'e_det b on a.voucher_no=b.no_voucher)x where trans_date between ' +
        #39'2025-01-01'#39' and '#39'2025-04-21'#39')b on a.VOUCHER=b.voucher_no '
      
        'left join t_debit_trx_real c on a.voucher=c.voucher where b.tran' +
        's_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39'  '
      'UNION ALL '
      
        '(SELECT voucher_no,voucher_tmp,trans_date,code_account,deskripsi' +
        ',kepada,description keterangan,paid_amount,saldo_debit,0 dpp fro' +
        'm v_titipan_bpjs_penerimaan_kas where code_account='#39'2130.04'#39' and' +
        ' trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39') '
      'UNION ALL  '
      
        '(select voucher_no,'#39#39' voucher_tmp,trans_date,code_account,descri' +
        'ption deskripsi,name_cust diterima_dari,description, paid_amount' +
        ' saldo_kredit, 0 saldo_debit,0 dpp from '
      
        '(SELECT a.voucher_no,a.trans_date,b.code_account,a.description,a' +
        '.name_cust,b.paid_amount FROM t_cash_bank_acceptance a INNER JOI' +
        'N t_cash_bank_acceptance_det b on a.voucher_no=b.voucher_no)x wh' +
        'ere trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and code_ac' +
        'count='#39'2130.04'#39' order by trans_date,voucher_no Asc ))xx '
      'LEFT JOIN '
      
        '(select DISTINCT k.voucher_no,k.periode1,k.periode2,k.plan_to,k.' +
        'to_,l.supplier_code from (SELECT a.voucher_no,a.periode1,a.perio' +
        'de2,a.plan_to,a.to_ from t_cash_bank_expenditure_submission a IN' +
        'NER JOIN t_cash_bank_expenditure_submission_det b on a.voucher_n' +
        'o=b.no_voucher'
      ') k '
      
        'left join t_supplier l on k.to_=l.supplier_name) r on xx.voucher' +
        '_tmp=r.voucher_no where case WHEN xx.saldo_kredit is null then 0' +
        ' else xx.saldo_kredit end <> 0 order by xx.tanggal,xx.voucher) '
      'UNION All  '
      
        'SELECT '#39#39' as voucher,'#39#39' as voucher_tmp,b.trans_date tgl_terima,a' +
        '.account_pph_code, (case when a.account_pph_code='#39'2130.02'#39' then ' +
        #39'PPH PS 23'#39' when a.account_pph_code='#39'2130.01'#39' then '#39'PPH PS 21'#39' w' +
        'hen a.account_pph_code='#39'2130.03'#39' then '#39'PPH PS 25'#39'  when a.accoun' +
        't_pph_code='#39'2130.04'#39' then '#39'PPH PS 4(2)'#39' when a.account_pph_code=' +
        #39'2130.07'#39' then '#39'PPH PS 15'#39'  when a.account_pph_code='#39'2130.06'#39' th' +
        'en '#39'PPH PS 22'#39' else a.account_pph_code end) as deskripsi,  suppl' +
        'ier_name as kepada,b.faktur_no,0 saldo_kredit,0 saldo_debit,a.su' +
        'btotalrp as dpp,a.pph_rp as saldo,a.ppn_rp  from t_purchase_invo' +
        'ice_det a '
      'left join t_purchase_invoice b on a.trans_no=b.trans_no '
      
        'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code where ' +
        'b.trans_date >'#39'2023-01-01'#39' and b.trans_date BETWEEN '#39'2025-01-01'#39 +
        ' and '#39'2025-04-21'#39' and a.account_pph_code='#39'2130.04'#39') zz')
    Left = 304
    Top = 160
    object QCatatan_ppn_pphvoucher: TMemoField
      FieldName = 'voucher'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QCatatan_ppn_pphvoucher_tmp: TMemoField
      FieldName = 'voucher_tmp'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QCatatan_ppn_pphtanggal: TDateField
      FieldName = 'tanggal'
      ReadOnly = True
    end
    object QCatatan_ppn_pphaccount_code: TMemoField
      FieldName = 'account_code'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QCatatan_ppn_pphdeskripsi: TMemoField
      FieldName = 'deskripsi'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QCatatan_ppn_pphkepada: TMemoField
      FieldName = 'kepada'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QCatatan_ppn_pphnofaktur: TMemoField
      FieldName = 'nofaktur'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QCatatan_ppn_pphsaldo_kredit: TFloatField
      FieldName = 'saldo_kredit'
      ReadOnly = True
    end
    object QCatatan_ppn_pphsaldo_debit: TFloatField
      FieldName = 'saldo_debit'
      ReadOnly = True
    end
    object QCatatan_ppn_pphdpp: TFloatField
      FieldName = 'dpp'
      ReadOnly = True
    end
    object QCatatan_ppn_pphsaldo: TFloatField
      FieldName = 'saldo'
      ReadOnly = True
    end
    object QCatatan_ppn_pphppn: TFloatField
      FieldName = 'ppn'
      ReadOnly = True
    end
    object QCatatan_ppn_pphpph21: TFloatField
      FieldName = 'pph21'
      ReadOnly = True
    end
    object QCatatan_ppn_pphpph22: TFloatField
      FieldName = 'pph22'
      ReadOnly = True
    end
    object QCatatan_ppn_pphpph23: TFloatField
      FieldName = 'pph23'
      ReadOnly = True
    end
    object QCatatan_ppn_pphpph15: TFloatField
      FieldName = 'pph15'
      ReadOnly = True
    end
    object QCatatan_ppn_pphpph4_2: TFloatField
      FieldName = 'pph4_2'
      ReadOnly = True
    end
  end
  object MemCatatan_ppn_pph: TMemTableEh
    Params = <>
    DataDriver = DataSetDriverEh1
    Left = 384
    Top = 160
  end
  object frxDBDataset4: TfrxDBDataset
    UserName = 'frxDBDatasetPers'
    CloseDataSource = False
    FieldAliases.Strings = (
      'company_code=company_code'
      'company_name=company_name'
      'address=address'
      'telp=telp'
      'email=email'
      'npwp=npwp'
      'city=city'
      'address2=address2'
      'id=id'
      'created_at=created_at'
      'created_by=created_by'
      'updated_at=updated_at'
      'updated_by=updated_by'
      'deleted_at=deleted_at'
      'deleted_by=deleted_by'
      'type_of_business=type_of_business'
      'latitude=latitude'
      'longitude=longitude'
      'tax_status=tax_status'
      'currency=currency'
      'fp_name=fp_name'
      'kk_year=kk_year'
      'kkas_year=kkas_year'
      #13#10'inventory_date='#13#10'inventory_date'
      #13#10'month_sa_acc='#13#10'month_sa_acc'
      'item_date=item_date'
      'debt_date=debt_date'
      'expenditure_date=expenditure_date'
      'serial_no=serial_no'
      'reporter=reporter'
      'position=position'
      'nitku=nitku')
    DataSet = QPerusahaan
    BCDToCurrency = False
    DataSetOptions = []
    Left = 876
    Top = 40
  end
  object dxBarManager1: TdxBarManager
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    Categories.Strings = (
      'Default')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    PopupMenuLinks = <>
    UseSystemFont = True
    Left = 689
    Top = 40
    PixelsPerInch = 96
    object dxBarManager1Bar1: TdxBar
      Caption = 'Action'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 930
      FloatTop = 3
      FloatClientWidth = 254
      FloatClientHeight = 238
      ItemLinks = <
        item
          Visible = True
          ItemName = 'DTawal'
        end
        item
          Visible = True
          ItemName = 'dxBarLargeButton2'
        end
        item
          Visible = True
          ItemName = 'DTakhir'
        end
        item
          Visible = True
          ItemName = 'dxBarLargeButton3'
        end
        item
          Visible = True
          ItemName = 'dxRefresh'
        end
        item
          Visible = True
          ItemName = 'dxBarLargeButton4'
        end>
      OneOnRow = True
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object dxBarButton2: TdxBarButton
      Caption = 'New Button'
      Category = 0
      Hint = 'New Button'
      Visible = ivAlways
    end
    object dxBarButton1: TdxBarButton
      Caption = 'New'
      Category = 0
      Hint = 'New'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        44E03B805C64CDB4C3E1300000000049454E44AE426082}
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        44E03B805C64CDB4C3E1300000000049454E44AE426082}
    end
    object dxBarButton3: TdxBarButton
      Caption = 'Refresh'
      Category = 0
      Hint = 'Refresh'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        10230000000049454E44AE426082}
    end
    object dxBarButton4: TdxBarButton
      Caption = 'Delete'
      Category = 0
      Hint = 'Delete'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        01D589CC0000000049454E44AE426082}
    end
    object dxBarButton5: TdxBarButton
      Caption = 'Update'
      Category = 0
      Hint = 'Update'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        01BA0F4212027F001ABA574BB80B6B080000000049454E44AE426082}
    end
    object dxBarButton6: TdxBarButton
      Caption = 'Refresh'
      Category = 0
      Hint = 'Refresh'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        }
    end
    object dxBarButton7: TdxBarButton
      Caption = 'Delete'
      Category = 0
      Hint = 'Refresh'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        BFC080A205F60000000049454E44AE426082}
    end
    object dxBarBaru: TdxBarLargeButton
      Caption = 'New'
      Category = 0
      Hint = 'New'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        0F208A3B0000000049454E44AE426082}
    end
    object dxBarUpdate: TdxBarButton
      Caption = 'Update'
      Category = 0
      Hint = 'Update'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        82}
    end
    object dxbarRefresh: TdxBarButton
      Caption = 'Refresh'
      Category = 0
      Hint = 'Refresh'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        02FE19007E4E40427BAAAF350000000049454E44AE426082}
    end
    object dxBarDelete: TdxBarButton
      Caption = 'Delete'
      Category = 0
      Hint = 'Delete'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        0000000049454E44AE426082}
    end
    object datetimepicker1: TcxBarEditItem
      Caption = 'Pilih Tanggal'
      Category = 0
      Hint = 'Pilih Tanggal'
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object dxBarLargeButton1: TdxBarLargeButton
      Caption = 'Print'
      Category = 0
      Hint = 'Print'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        }
    end
    object dxBarButton8: TdxBarButton
      Caption = 'New Button'
      Category = 0
      Hint = 'New Button'
      Visible = ivAlways
    end
    object dxBarLargeButton2: TdxBarLargeButton
      Category = 0
      Visible = ivAlways
    end
    object dxBarLargeButton3: TdxBarLargeButton
      Category = 0
      Visible = ivAlways
    end
    object DateTimePicker2: TcxBarEditItem
      Caption = 'Pilih Tanggal'
      Category = 0
      Hint = 'Pilih Tanggal'
      Visible = ivNever
    end
    object Ed_supplier: TcxBarEditItem
      Caption = 'Supplier          : '
      Category = 0
      Hint = 'Supplier          : '
      Visible = ivAlways
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
    end
    object Jenis_Hutang: TdxBarCombo
      Caption = 'Jenis Hutang    :'
      Category = 0
      Hint = 'Jenis Hutang    :'
      Visible = ivAlways
      ItemIndex = -1
    end
    object Cb_Valas: TdxBarCombo
      Caption = 'Valas                : '
      Category = 0
      Hint = 'Valas                : '
      Visible = ivAlways
      Text = 'IDR'
      ItemIndex = -1
    end
    object cbbulan1: TcxBarEditItem
      Caption = 'Bulan   :'
      Category = 0
      Hint = 'Bulan   :'
      Visible = ivAlways
      PropertiesClassName = 'TcxComboBoxProperties'
      Properties.Items.Strings = (
        ''
        'JANUARI'
        'FEBRUARI'
        'MARET'
        'APRIL'
        'MEI'
        'JUNI'
        'JULI'
        'AGUSTUS'
        'SEPTEMBER'
        'OKTOBER'
        'NOPEMBER'
        'DESEMBER')
    end
    object dxBarLargeButton4: TdxBarLargeButton
      Caption = 'Print'
      Category = 0
      Hint = 'Print'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        }
      OnClick = dxBarLargeButton4Click
    end
    object cbtahun: TcxBarEditItem
      Caption = 'Tahun  :'
      Category = 0
      Hint = 'Tahun    :'
      Visible = ivAlways
      PropertiesClassName = 'TcxSpinEditProperties'
      Properties.AssignedValues.MinValue = True
      InternalEditValue = 2024
    end
    object dxRefresh: TdxBarLargeButton
      Caption = 'Cari'
      Category = 0
      Hint = 'Cari'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        2F673E0D0A09093C2F673E0D0A093C2F673E0D0A3C2F7376673E0D0A}
      OnClick = dxRefreshClick
    end
    object ed_code_supp: TcxBarEditItem
      Caption = '                         '
      Category = 0
      Enabled = False
      Hint = '                         '
      Visible = ivAlways
    end
    object cbbulan: TdxBarCombo
      Caption = 'Bulan  :'
      Category = 0
      Hint = 'Bulan  :'
      Visible = ivAlways
      Items.Strings = (
        ''
        'JANUARI'
        'FEBRUARI'
        'MARET'
        'APRIL'
        'MEI'
        'JUNI'
        'JULI'
        'AGUSTUS'
        'SEPTEMBER'
        'OKTOBER'
        'NOPEMBER'
        'DESEMBER')
      ItemIndex = -1
    end
    object dxBarEdit1: TdxBarEdit
      Caption = 'New Item'
      Category = 0
      Hint = 'New Item'
      Visible = ivAlways
    end
    object DTawal: TcxBarEditItem
      Caption = 'Tanggal Awal    :   '
      Category = 0
      Hint = 'Tanggal Awal    :   '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object DTakhir: TcxBarEditItem
      Caption = 'Tanggal Akhir   :   '
      Category = 0
      Hint = 'Tanggal Akhir   :   '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
  end
  object DataSetDriverEh1: TDataSetDriverEh
    ProviderDataSet = QCatatan_ppn_pph
    Left = 408
    Top = 224
  end
  object ActMenu: TActionManager
    Left = 601
    Top = 48
    StyleName = 'Platform Default'
    object ActBaru: TAction
      Caption = 'Baru  '
    end
    object ActUpdate: TAction
      Caption = 'Update  '
    end
    object ActRO: TAction
      Caption = 'Refresh  '
    end
    object ActDel: TAction
      Caption = 'Delete  '
    end
    object ActPrint: TAction
      Caption = 'Print  '
    end
    object ActApp: TAction
      Caption = 'Approve  '
      Enabled = False
    end
    object ActReject: TAction
      Caption = 'Reject  '
      Enabled = False
    end
    object ActClose: TAction
      Caption = 'CLose PO    '
      Enabled = False
    end
  end
  object DSQTgl_Hutang: TDataSource
    DataSet = QTgl_hutang
    Left = 743
    Top = 216
  end
  object QTgl_hutang: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      'select * from t_tmpsyst')
    Left = 728
    Top = 160
  end
  object frxDBDatasetCat_ppn_pph: TfrxDBDataset
    UserName = 'frxDBDatasetCat_ppn_pph'
    CloseDataSource = False
    FieldAliases.Strings = (
      'voucher=voucher'
      'voucher_tmp=voucher_tmp'
      'tanggal=tanggal'
      'account_code=account_code'
      'deskripsi=deskripsi'
      'kepada=kepada'
      'nofaktur=nofaktur'
      'saldo_kredit=saldo_kredit'
      'saldo_debit=saldo_debit'
      'dpp=dpp'
      'saldo=saldo'
      'ppn=ppn'
      'pph21=pph21'
      'pph22=pph22'
      'pph23=pph23'
      'pph15=pph15'
      'pph4_2=pph4_2')
    DataSet = QCat_ppn_pph
    BCDToCurrency = False
    DataSetOptions = []
    Left = 915
    Top = 168
  end
  object frxReport2: TfrxReport
    Version = '2022.1.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick, pbCopy, pbSelection]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 44630.556593796300000000
    ReportOptions.LastChange = 45271.601852835700000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 816
    Top = 168
    Datasets = <
      item
        DataSet = frxDBDataset4
        DataSetName = 'frxDBDatasetPers'
      end
      item
        DataSet = frxDBDatasetCat_ppn_pph
        DataSetName = 'frxDBDatasetCat_ppn_pph'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 330.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 256
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 131.905511810000000000
        Top = 18.897650000000000000
        Width = 1171.654300000000000000
        object Memo1: TfrxMemoView
          AllowVectorExport = True
          Top = 5.000000000000000000
          Width = 1166.929887500000000000
          Height = 33.070887500000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'CATATAN PPN, PPH PS 23,PPH PS 22, PPH 15, PPH 4(2)')
          ParentFont = False
        end
        object MemoPers: TfrxMemoView
          AllowVectorExport = True
          Top = 32.795300000000000000
          Width = 1166.929887500000000000
          Height = 33.070887500000000000
          DataField = 'company_name'
          DataSet = frxDBDataset4
          DataSetName = 'frxDBDatasetPers'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetPers."company_name"]')
          ParentFont = False
        end
        object Memperiode: TfrxMemoView
          AllowVectorExport = True
          Top = 59.354360000000000000
          Width = 1166.929887500000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Periode')
          ParentFont = False
        end
        object Memo4: TfrxMemoView
          AllowVectorExport = True
          Top = 101.669291340000000000
          Width = 29.291357500000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'NO')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo5: TfrxMemoView
          AllowVectorExport = True
          Left = 30.236240000000000000
          Top = 101.669291340000000000
          Width = 80.125984251968500000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'NO BK')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo6: TfrxMemoView
          AllowVectorExport = True
          Left = 110.488250000000000000
          Top = 101.669291338583000000
          Width = 59.527597500000000000
          Height = 30.236220472440900000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'TGL BK')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo7: TfrxMemoView
          AllowVectorExport = True
          Left = 171.094620000000000000
          Top = 101.669291338583000000
          Width = 157.795377500000000000
          Height = 30.236220472440900000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'NAMA SUPPLIER')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo8: TfrxMemoView
          AllowVectorExport = True
          Left = 330.205010000000000000
          Top = 101.669291338583000000
          Width = 59.527597500000000000
          Height = 30.236220472440900000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'TGL FP/ INV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo9: TfrxMemoView
          AllowVectorExport = True
          Left = 390.764070000000000000
          Top = 101.669291338583000000
          Width = 85.984307500000000000
          Height = 30.236220472440900000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'NO FP / INV')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo10: TfrxMemoView
          AllowVectorExport = True
          Left = 477.488560000000000000
          Top = 101.669291340000000000
          Width = 98.267716535433100000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'TAGIHAN')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 576.315400000000000000
          Top = 101.669291340000000000
          Width = 82.204777500000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'DPP')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo12: TfrxMemoView
          AllowVectorExport = True
          Left = 659.142240000000000000
          Top = 101.669291340000000000
          Width = 79.370078740157500000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'PPN')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          AllowVectorExport = True
          Left = 739.276130000000000000
          Top = 101.669291340000000000
          Width = 64.251968503937000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'PPH 23')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo14: TfrxMemoView
          AllowVectorExport = True
          Left = 866.646260000000000000
          Top = 101.669291338583000000
          Width = 63.496062990000000000
          Height = 30.236220472440900000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'PPH PS 15')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo15: TfrxMemoView
          AllowVectorExport = True
          Left = 931.016390000000000000
          Top = 101.669291338583000000
          Width = 63.496062990000000000
          Height = 30.236220472440900000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'PPH 4(2)')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo16: TfrxMemoView
          AllowVectorExport = True
          Left = 995.386520000000000000
          Top = 101.669291340000000000
          Width = 76.346456692913400000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'TOTAL')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          AllowVectorExport = True
          Left = 803.528140000000000000
          Top = 101.669291338583000000
          Width = 63.307127500000000000
          Height = 30.236220472440900000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'PPH 22')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo18: TfrxMemoView
          AllowVectorExport = True
          Left = 1072.386520000000000000
          Top = 101.669291340000000000
          Width = 96.413806590000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'KETERANGAN')
          ParentFont = False
          VAlign = vaCenter
        end
        object Shape1: TfrxShapeView
          AllowVectorExport = True
          Width = 1169.385826770000000000
          Height = 98.267780000000000000
          Frame.Typ = []
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 37.795275590000000000
        Top = 257.008040000000000000
        Width = 1171.654300000000000000
        DataSet = frxDBDatasetCat_ppn_pph
        DataSetName = 'frxDBDatasetCat_ppn_pph'
        RowCount = 0
        Stretched = True
        object Memo19: TfrxMemoView
          AllowVectorExport = True
          Width = 29.291357500000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[Line#]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo20: TfrxMemoView
          AllowVectorExport = True
          Left = 30.236240000000000000
          Width = 80.125984250000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'voucher'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."voucher"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          AllowVectorExport = True
          Left = 110.488250000000000000
          Width = 59.527597500000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'tanggal'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          DisplayFormat.FormatStr = 'dd-mm-yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."tanggal"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo22: TfrxMemoView
          AllowVectorExport = True
          Left = 171.094620000000000000
          Width = 157.795377500000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'kepada'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."kepada"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo23: TfrxMemoView
          AllowVectorExport = True
          Left = 330.205010000000000000
          Width = 59.527597500000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = 'dd-mm-yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haCenter
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo24: TfrxMemoView
          AllowVectorExport = True
          Left = 390.764070000000000000
          Width = 85.984307500000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'nofaktur'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."nofaktur"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo25: TfrxMemoView
          AllowVectorExport = True
          Left = 477.488560000000000000
          Width = 98.267716540000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo26: TfrxMemoView
          AllowVectorExport = True
          Left = 576.315400000000000000
          Width = 82.204777500000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'dpp'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."dpp"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo27: TfrxMemoView
          AllowVectorExport = True
          Left = 659.142240000000000000
          Width = 79.370078740000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'ppn'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."ppn"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo28: TfrxMemoView
          AllowVectorExport = True
          Left = 739.276130000000000000
          Width = 64.251968500000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'pph23'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."pph23"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo29: TfrxMemoView
          AllowVectorExport = True
          Left = 866.646260000000000000
          Width = 63.496062990000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'pph15'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."pph15"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo30: TfrxMemoView
          AllowVectorExport = True
          Left = 931.016390000000000000
          Width = 63.496062990000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'pph4_2'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."pph4_2"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo31: TfrxMemoView
          AllowVectorExport = True
          Left = 995.386520000000000000
          Width = 76.346456690000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo32: TfrxMemoView
          AllowVectorExport = True
          Left = 803.528140000000000000
          Width = 63.307127500000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          DataField = 'pph22'
          DataSet = frxDBDatasetCat_ppn_pph
          DataSetName = 'frxDBDatasetCat_ppn_pph'
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetCat_ppn_pph."pph22"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo33: TfrxMemoView
          AllowVectorExport = True
          Left = 1072.386520000000000000
          Width = 96.413806590000000000
          Height = 37.795275590000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677165350000000000
        Top = 400.630180000000000000
        Width = 1171.654300000000000000
        object Memo50: TfrxMemoView
          AllowVectorExport = True
          Width = 476.220472440945000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo51: TfrxMemoView
          AllowVectorExport = True
          Left = 395.645911250000000000
          Top = 3.055117500000000000
          Width = 47.244125000000000000
          Height = 16.535443750000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'TOTAL                                            ')
          ParentFont = False
        end
        object Memo52: TfrxMemoView
          AllowVectorExport = True
          Left = 477.220780000000000000
          Width = 98.267716540000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo53: TfrxMemoView
          AllowVectorExport = True
          Left = 576.047620000000000000
          Width = 82.204777500000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."dpp">,MasterData1,1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo54: TfrxMemoView
          AllowVectorExport = True
          Left = 658.874460000000000000
          Width = 79.370078740000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo55: TfrxMemoView
          AllowVectorExport = True
          Left = 739.008350000000000000
          Width = 64.251968500000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."pph23">,MasterData1,1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo56: TfrxMemoView
          AllowVectorExport = True
          Left = 866.378480000000000000
          Width = 63.496062990000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."pph15">,MasterData1,1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo57: TfrxMemoView
          AllowVectorExport = True
          Left = 930.748610000000000000
          Width = 63.496062990000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."pph4_2">,MasterData1,1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo58: TfrxMemoView
          AllowVectorExport = True
          Left = 995.118740000000000000
          Width = 76.346456690000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo59: TfrxMemoView
          AllowVectorExport = True
          Left = 803.260360000000000000
          Width = 63.307127500000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."pph22">,MasterData1,1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo60: TfrxMemoView
          AllowVectorExport = True
          Left = 1072.118740000000000000
          Width = 97.322897500000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo61: TfrxMemoView
          AllowVectorExport = True
          Left = 461.102660000000000000
          Top = 2.779530000000000000
          Width = 17.007885000000000000
          Height = 16.535443750000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            ':')
          ParentFont = False
        end
      end
      object GroupFooter1: TfrxGroupFooter
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677165350000000000
        Top = 317.480520000000000000
        Width = 1171.654300000000000000
        object Memo37: TfrxMemoView
          AllowVectorExport = True
          Width = 476.220472440945000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo39: TfrxMemoView
          AllowVectorExport = True
          Left = 395.645911250000000000
          Top = 3.055117500000000000
          Width = 66.141775000000000000
          Height = 16.535443750000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'SUB TOTAL ')
          ParentFont = False
        end
        object Memo40: TfrxMemoView
          AllowVectorExport = True
          Left = 477.220780000000000000
          Width = 98.267716540000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo42: TfrxMemoView
          AllowVectorExport = True
          Left = 576.047620000000000000
          Width = 82.204777500000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."dpp">,MasterData1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo43: TfrxMemoView
          AllowVectorExport = True
          Left = 658.874460000000000000
          Width = 79.370078740000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."ppn">,MasterData1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo44: TfrxMemoView
          AllowVectorExport = True
          Left = 739.008350000000000000
          Width = 64.251968500000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."pph23">,MasterData1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo45: TfrxMemoView
          AllowVectorExport = True
          Left = 866.378480000000000000
          Width = 63.496062990000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."pph15">,MasterData1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo46: TfrxMemoView
          AllowVectorExport = True
          Left = 930.748610000000000000
          Width = 63.496062990000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."pph4_2">,MasterData1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo47: TfrxMemoView
          AllowVectorExport = True
          Left = 995.118740000000000000
          Width = 76.346456690000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo48: TfrxMemoView
          AllowVectorExport = True
          Left = 803.260360000000000000
          Width = 63.307127500000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '#,###,###,###.##'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetCat_ppn_pph."pph22">,MasterData1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo49: TfrxMemoView
          AllowVectorExport = True
          Left = 1072.118740000000000000
          Width = 97.322897500000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo62: TfrxMemoView
          AllowVectorExport = True
          Left = 461.102660000000000000
          Top = 3.779530000000000000
          Width = 17.007885000000000000
          Height = 16.535443750000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            ':')
          ParentFont = False
        end
      end
      object GroupHeader1: TfrxGroupHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 211.653680000000000000
        Visible = False
        Width = 1171.654300000000000000
        Condition = 'frxDBDatasetCat_ppn_pph."kepada"'
        object Memo41: TfrxMemoView
          AllowVectorExport = True
          Top = 1.000000000000000000
          Width = 64.252010000000000000
          Height = 15.118088270000000000
          Visible = False
          StretchMode = smMaxHeight
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDataset1."voucher"]')
          ParentFont = False
        end
      end
    end
  end
  object QCat_ppn_pph: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      '/*--Hutang PPH PS22  2130.06'#9'2130'#9'Hutang Pajak - PPh.Ps.22*/'
      
        'SELECT voucher,'#9'voucher_tmp,'#9'tanggal,'#9'account_code,'#9'deskripsi,'#9'k' +
        'epada,faktur_no'#9'nofaktur,'#9'saldo_kredit,'#9'saldo_debit,'#9'dpp,'#9'saldo,' +
        ' ppn,(case when account_code='#39'2130.01'#39' then saldo else 0 end)pph' +
        '21,'
      '(case when account_code='#39'2130.06'#39' then saldo else 0 end) pph22,'
      '(case when account_code='#39'2130.02'#39' then saldo else 0 end) pph23, '
      '(case when account_code='#39'2130.07'#39' then saldo else 0 end) pph15,'
      
        '(case when account_code='#39'2130.04'#39' then saldo else 0 end) pph4_2 ' +
        'from'
      '(select yy.* from '
      '(  '
      
        '(select distinct xx.voucher,xx.voucher_tmp,xx.tanggal,xx.account' +
        '_code,v.deskripsi,xx.kepada,t.faktur_no,0 ::float saldo_kredit,0' +
        ' ::float saldo_debit,case WHEN v.dpp is null then 0 else v.dpp e' +
        'nd dpp,  case WHEN t.npph is null then 0 else t.npph end saldo, ' +
        '0 as ppn from  '
      
        '(select a.voucher,b.voucher_tmp,b.tanggal,a.account_code,a.descr' +
        'iption as deskripsi,b.kepada,a.saldo saldo_kredit,case when c.sa' +
        'ldo  is null then 0 else c.saldo end saldo_debit from  '
      
        '(select * from t_credit_trx_real where account_code='#39'2130.06'#39')a ' +
        '  '
      
        'left join (select distinct voucher_no,voucher_tmp,to_ as kepada,' +
        'trans_date as tanggal from (SELECT voucher_no,a.voucher_tmp,a.to' +
        '_,a.trans_date from t_cash_bank_expenditure a INNER JOIN t_cash_' +
        'bank_expenditure_det b on a.voucher_no=b.no_voucher)x where tran' +
        's_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39')b on a.voucher=b.vo' +
        'ucher_no '
      
        'left join t_debit_trx_real c on a.voucher=c.voucher where b.tang' +
        'gal between '#39'2025-01-01'#39' and '#39'2025-04-21'#39'   '
      'UNION ALL '
      
        '(SELECT voucher_no,voucher_tmp,trans_date,code_account,deskripsi' +
        ',kepada,paid_amount as jumlah,saldo_debit from v_titipan_bpjs_pe' +
        'nerimaan_kas where code_account='#39'2130.06'#39' and trans_date between' +
        ' '#39'2025-01-01'#39' and '#39'2025-04-21'#39') '
      'UNION ALL '
      
        '(select voucher_no,'#39#39' voucher_tmp,trans_date,code_account,descri' +
        'ption as keterangan,case when code_account='#39'2130.02'#39' then '#39'PPH 2' +
        '3'#39' when code_account='#39'2130.01'#39' then '#39'PPH 21'#39' END kepada,paid_amo' +
        'unt saldo_kredit, 0 saldo_debit FROM (SELECT a.voucher_no,b.tran' +
        's_date,a.description,b.code_account,b.paid_amount FROM t_cash_ba' +
        'nk_acceptance a INNER JOIN t_cash_bank_acceptance_det b on a.vou' +
        'cher_no=b.voucher_no)x where trans_date between '#39'2025-01-01'#39' and' +
        ' '#39'2025-04-21'#39' and code_account='#39'2130.06'#39'  order by trans_date,vo' +
        'ucher_no Asc))xx '
      'LEFT JOIN '
      
        '(select DISTINCT k.voucher_no,K.trans_date as periode1,k.kepada,' +
        'l.supplier_code from(SELECT a.voucher_no,a.trans_date,a.to_ as k' +
        'epada from t_cash_bank_expenditure a INNER JOIN t_cash_bank_expe' +
        'nditure_det b on a.voucher_no=b.no_voucher)k '
      
        'left join t_supplier l on k.kepada=l.supplier_name where k.trans' +
        '_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39')r on xx.voucher=r.vo' +
        'ucher_no  LEFT JOIN t_paid_debt_det t on t.periode1=r.periode1 a' +
        'nd r.supplier_code=t.supplier_code '
      
        'LEFT JOIN "V_do_lpb2" v  on t.faktur_no=v.no_fp_no_inv where cas' +
        'e WHEN t.npph is null then 0 else t.npph end <> 0 ) '
      'UNION ALL '
      
        '(select a.voucher_no voucher, b.voucher_tmp, b.trans_date tangga' +
        'l, b.code_account, c.deskripsi,b.to_ as kepada, c.no_fp_no_inv n' +
        'ofaktur,0 ::float saldo_kredit, 0 ::float saldo_debit,c.dpp,c.pp' +
        'h23 saldo, 0 ppn from t_payment_detail_real a '
      
        'left join (SELECT a.voucher_no,a.voucher_tmp,b.trans_date,b.code' +
        '_account,a.to_  from t_cash_bank_expenditure a INNER JOIN t_cash' +
        '_bank_expenditure_det b on a.voucher_no=b.no_voucher)b  on a.vou' +
        'cher_no=b.voucher_no left join "V_do_lpb2" c on a.voucher_no=c.v' +
        'oucher  where b.code_account='#39'2130.06'#39'  and b.trans_date  betwee' +
        'n '#39'2025-01-01'#39' and '#39'2025-04-21'#39'  and  a.source_id=3 and no_fp_no' +
        '_inv is not null) '
      'UNION ALL '
      
        '(select distinct m.voucher_no,m.voucher_tmp,m.trans_date as tang' +
        'gal,n.account_code,(case when n.description1='#39#39' then n.descripti' +
        'on else n.description end) deskripsi,m.to_ as kepada,n.inv_supp_' +
        'no nofaktur,0 ::float saldo_kredit,0 ::float saldo_debit,n.dpp,n' +
        '.saldo, 0 ppn from (SELECT a.voucher_no,a.voucher_tmp,b.trans_da' +
        'te,a.to_ from t_cash_bank_expenditure a INNER JOIN t_cash_bank_e' +
        'xpenditure_det b on a.voucher_no=b.no_voucher) m , t_credit_trx_' +
        'real n where m.voucher_no=n.voucher and n.account_code='#39'2130.06'#39 +
        ' and m.trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39') '
      'UNION ALL '
      
        '(SELECT a.voucher_no as voucher,'#39#39' voucher_tmp,a.trans_date as t' +
        'anggal,a.code_account,a.description as deskripsi,a.description a' +
        's kepada,'#39#39' as nofaktur, 0 saldo_kredit,  0 saldo_debit,0 as dpp' +
        ',a.paid_amount as saldo, 0 ppn from (SELECT a.voucher_no,a.trans' +
        '_date,b.code_account,a.description,b.paid_amount FROM t_cash_ban' +
        'k_acceptance a INNER JOIN t_cash_bank_acceptance_det b on a.vouc' +
        'her_no=b.voucher_no)a where trans_date between '#39'2025-01-01'#39' and ' +
        #39'2025-04-21'#39'  and code_account='#39'2130.06'#39' order by trans_date,vou' +
        'cher_no Asc) '
      'UNION ALL '
      
        'SELECT '#39#39' as voucher,'#39#39' as voucher_tmp,b.trans_date,a.account_pp' +
        'h_code, (case when a.account_pph_code='#39'2142'#39' then '#39'PPH PS 23'#39' wh' +
        'en a.account_pph_code='#39'2141'#39' then '#39'PPH PS 21'#39' when a.account_pph' +
        '_code='#39'2143'#39' then '#39'PPH PS 25'#39'  when a.account_pph_code='#39'2146'#39' th' +
        'en '#39'PPH PS 4(2)'#39' when a.account_pph_code='#39'2147'#39' then '#39'PPH PS 15'#39 +
        '  when a.account_pph_code='#39'2148'#39' then '#39'PPH PS 22'#39' else a.account' +
        '_pph_code end) as deskripsi,supplier_name as kepada,b.faktur_no,' +
        '0 saldo_kredit,0 saldo_debit,a.subtotalrp as dpp,a.pph_rp as sal' +
        'do,a.ppn_rp from t_purchase_invoice_det a  left join t_purchase_' +
        'invoice b on a.trans_no=b.trans_no LEFT JOIN t_supplier d on b.s' +
        'upplier_code=d.supplier_code where b.trans_date > '#39'2023-01-01'#39' a' +
        'nd b.trans_date BETWEEN '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and a.acco' +
        'unt_pph_code='#39'2130.06'#39'  '
      'UNION ALL '
      
        '(SELECT a.bk_no as voucher,'#39#39' as voucher_tmp,a.trans_date,b.acco' +
        'unt_code,a.notes as deskripsi,'#39#39' as kepada,'#39#39'as nofaktur,0 saldo' +
        '_kredit,0 saldo_debit,0 as dpp,-(b.debit) as saldo,0 ppn FROM t_' +
        'memorial_journal a '
      
        'LEFT JOIN t_memorial_journal_detail b ON a.memo_no=b.memo_no WHE' +
        'RE b.account_code='#39'2130.06'#39' and a.trans_date BETWEEN '#39'2025-01-01' +
        #39' and '#39'2025-04-21'#39' order by a.trans_date asc)'
      
        ')yy where yy.voucher is not null  order by yy.tanggal,yy.voucher' +
        ',yy.faktur_no asc)xx '
      ''
      'UNION All'
      '   /*--Hutang PPH PS23  2130.02'#9'2130'#9'Hutang Pajak - PPh.Ps.23 */'
      
        'SELECT voucher,'#9#39#39' as voucher_tmp,'#9'tanggal,kodesp as'#9'kode_accoun' +
        't,'#9'deskripsi,'#9'kepada,'#9'nofaktur,'#9'0 saldo_kredit,'#9'0 saldo_debit,'#9'd' +
        'pp,'#9'pph as saldo,ppn,'
      '(case when kodesp='#39'2130.01'#39' then pph else 0 end) pph21,'
      '(case when kodesp='#39'2130.06'#39' then pph else 0 end) pph22,'
      '(case when kodesp='#39'2130.02'#39' then pph else 0 end) pph23, '
      '(case when kodesp='#39'2130.07'#39' then pph else 0 end) pph15,'
      '(case when kodesp='#39'2130.04'#39' then pph else 0 end) pph4_2 FROM'
      '( SELECT yyy.* FROM '
      '(SELECT yy.* FROM '
      '( '
      
        'SELECT  a.voucher,a.tanggal,a.kodesp,x.deskripsi,kepada,a.faktur' +
        '_no as nofaktur ,y.dpp as dpp,a.pph as pph,'#39#39' as nodo,a.supplier' +
        '_code as kdsup,y.ppn_rp as ppn FROM '
      
        '(SELECT DISTINCT a.*,b.faktur_no,c.saldo as pph,b.supplier_code ' +
        'FROM '
      
        '(SELECT DISTINCT voucher_no as voucher,trans_date tanggal,to_ ke' +
        'pada,code_account as kodesp FROM (SELECT a.voucher_no,a.trans_da' +
        'te,a.to_,b.code_account,b."position" from t_cash_bank_expenditur' +
        'e a INNER JOIN t_cash_bank_expenditure_det b on a.voucher_no=b.n' +
        'o_voucher)x where trans_date between '#39'2025-01-01'#39' and '#39'2025-04-2' +
        '1'#39' and code_account='#39'2130.02'#39' and "position"='#39'K'#39')a  '
      
        'LEFT JOIN t_cash_bank_expenditure_payable b on a.voucher=b.vouch' +
        'er_no '
      
        'LEFT JOIN t_credit_trx_real c on a.voucher=c.voucher and b.faktu' +
        'r_no=c.inv_supp_no WHERE (c.voucher is not null ) and (b.faktur_' +
        'no is not null)) a '
      'LEFT JOIN  t_purchase_invoice f ON f.faktur_no=a.faktur_no '
      
        'LEFT JOIN (SELECT a.trans_no,string_agg (b.item_name, '#39', '#39' ) AS ' +
        'deskripsi FROM t_purchase_invoice_det a '
      
        'LEFT JOIN t_item_stock b ON a.item_stock_code::text = b.item_sto' +
        'ck_code::text GROUP BY a.trans_no) x On x.trans_no=f.trans_no LE' +
        'FT JOIN (SELECT trans_no,sum(subtotal) as dpp,ppn_rp FROM t_purc' +
        'hase_invoice_det GROUP BY trans_no,ppn_rp) y on f.trans_no=y.tra' +
        'ns_no '
      'UNION ALL '
      
        'SELECT aa.voucher_no,aa.trans_date tanggal,aa.code as kodesp,b.d' +
        'escription1,aa.to_ as kepada,b.inv_supp_no as nofaktur ,b.dpp as' +
        ' dpp,b.saldo as pph,'#39#39' as nodo,supplier_code as kdsup,0 ppn  FRO' +
        'M '
      
        '(SELECT a.*,c.faktur_no as no_inv_supp,b.inv_supp_no,e.code  FRO' +
        'M '
      
        '(SELECT distinct voucher_no,trans_date,supplier_code,to_ FROM (S' +
        'ELECT a.voucher_no,a.trans_date,a.supplier_code,a.to_,b.code_acc' +
        'ount,b."position" from t_cash_bank_expenditure a INNER JOIN t_ca' +
        'sh_bank_expenditure_det b on a.voucher_no=b.no_voucher)x where t' +
        'rans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and code_account' +
        '='#39'2130.02'#39' and "position"='#39'K'#39')a '
      'LEFT JOIN t_credit_trx_real b on a.voucher_no=b.voucher '
      
        'LEFT JOIN t_cash_bank_expenditure_payable c on a.voucher_no=c.vo' +
        'ucher_no'
      'INNER JOIN v_ak_account e on c.account_acc=e.account_code2 '
      'LEFT JOIN t_payment_detail_real d on a.voucher_no=d.voucher_no '
      
        'WHERE (b.voucher is not null) and (b.voucher is null)and (c.vouc' +
        'her_no is null))aa '
      'LEFT JOIN t_credit_trx_real b ON aa.voucher_no=b.voucher   '
      'UNION ALL '
      'SELECT e.voucher_no as voucher,b.trans_date,a.account_pph_code, '
      '(case when a.account_pph_code='#39'2130.02'#39' then '#39'PPH PS 23'#39' '
      'when a.account_pph_code='#39'2130.01'#39' then '#39'PPH PS 21'#39' '
      'when a.account_pph_code='#39'2130.03'#39' then '#39'PPH PS 25'#39'  '
      'when a.account_pph_code='#39'2130.04'#39' then '#39'PPH PS 4(2)'#39' '
      'when a.account_pph_code='#39'2130.07'#39' then '#39'PPH PS 15'#39'  '
      
        'when a.account_pph_code='#39'2130.06'#39' then '#39'PPH PS 22'#39' else a.accoun' +
        't_pph_code end) as deskripsi,d.supplier_name as kepada,b.faktur_' +
        'no,a.subtotalrp as dpp,a.pph_rp as pph,'#39#39' as nodo,d.supplier_cod' +
        'e,a.ppn_rp from t_purchase_invoice_det a  '
      'left join t_purchase_invoice b on a.trans_no=b.trans_no '
      'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code '
      
        'LEFT JOIN t_cash_bank_expenditure_payable e on a.trans_no=e.invo' +
        'ice_no'
      'INNER JOIN v_ak_account f on a.account_code=f.account_code2'
      
        'where b.trans_date >'#39'2023-01-01'#39' and b.trans_date BETWEEN '#39'2025-' +
        '01-01'#39' and '#39'2025-04-21'#39' and f.code='#39'2130.02'#39'    '
      'UNION ALL '
      
        'SELECT d.bk_no as voucher ,e.trans_date,'#39'2130.02'#39' as kdsp,'#39'PPH P' +
        'S 23'#39' as jenis_pph, c.supplier_name as kepada,'#39#39' as nofaktur ,su' +
        'btotal as dpp,b.pph_rp as pph, '#39#39' nodo,c.supplier_code,0 ppn FRO' +
        'M t_po a '
      'left join t_podetail b ON a.po_no=b.po_no '
      'left join t_supplier c on a.supplier_code=c.supplier_code '
      'left join t_payment_detail_real d on a.po_no=d.voucher_no '
      
        'LEFT JOIN (SELECT a.voucher_no,a.trans_date from t_cash_bank_exp' +
        'enditure a INNER JOIN t_cash_bank_expenditure_det b on a.voucher' +
        '_no=b.no_voucher)e on d.bk_no=e.voucher_no '
      'where trans_date BETWEEN '#39'2025-01-01'#39' and '#39'2025-04-21'#39'  '
      
        'and b.pph_rp<>0 GROUP BY e.trans_date,c.supplier_name,c.supplier' +
        '_code,d.bk_no,b.pph_rp,b.subtotal '
      'UNION ALL  '
      
        'SELECT distinct m.voucher_no voucher,m.trans_date as tanggal,n.a' +
        'ccount_code as kodesp,n.description1 deskripsi,m.to_ as kepada,n' +
        '.inv_supp_no nofaktur,n.dpp,n.saldo,'#39#39' as nodo,'#39#39' as kdsup, 0 pp' +
        'n from (SELECT a.voucher_no,a.trans_date,a.to_  from t_cash_bank' +
        '_expenditure a INNER JOIN t_cash_bank_expenditure_det b on a.vou' +
        'cher_no=b.no_voucher)m ,t_credit_trx_real n '
      
        'WHERE m.voucher_no=n.voucher and n.Account_code='#39'2130.02'#39' and de' +
        'scription1<>'#39#39' and m.trans_date between '#39'2025-01-01'#39' and '#39'2025-0' +
        '4-21'#39')yy order by yy.tanggal,yy.voucher,yy.nofaktur asc )yyy ORD' +
        'ER BY kepada,voucher,nofaktur,tanggal ASC)yyyy  '
      ''
      'UNION ALL '
      
        '/*--Hutang PPH Pasal 15  2130.07'#9'2130'#9'Hutang Pajak - PPh.Ps.15 *' +
        '/ '
      
        'SELECT voucher,voucher_tmp,tanggal,account_code,deskripsi,kepada' +
        ',faktur_no,saldo_kredit,saldo_debit,dpp,saldo,0 ppn,'
      '(case when account_code='#39'2130.01'#39' then saldo else 0 end) pph21,'
      '(case when account_code='#39'2130.06'#39' then saldo else 0 end) pph22,'
      '(case when account_code='#39'2130.02'#39' then saldo else 0 end) pph23,'
      '(case when account_code='#39'2130.07'#39' then saldo else 0 end) pph15,'
      
        '(case when account_code='#39'2130.04'#39' then saldo else 0 end) pph4_2 ' +
        'FROM  '
      '(select yy.* from '
      '( '
      
        '(select distinct xx.voucher,xx.voucher_tmp,xx.tanggal,xx.account' +
        '_code,v.deskripsi,xx.kepada,  t.faktur_no,0 ::float saldo_kredit' +
        ',0 ::float saldo_debit,case WHEN v.dpp is null then 0 else v.dpp' +
        ' end dpp,  case WHEN t.npph is null then 0 else t.npph end saldo' +
        ',0 ppn from  '
      
        '(select a.voucher,b.voucher_tmp,b.tanggal,a.account_code,a.descr' +
        'iption as deskripsi,b.kepada,a.saldo saldo_kredit,case when c.sa' +
        'ldo  is null then 0 else c.saldo end saldo_debit from  '
      
        '(select * from t_credit_trx_real where account_code='#39'2130.07'#39')a ' +
        ' '
      'left join   '
      
        '(select distinct voucher_no,voucher_tmp,to_ as kepada,trans_date' +
        ' as tanggal from (SELECT a.voucher_no,a.voucher_tmp,a.to_,a.tran' +
        's_date from t_cash_bank_expenditure a INNER JOIN t_cash_bank_exp' +
        'enditure_det b on a.voucher_no=b.no_voucher)a '
      'where  '
      
        'trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39')b on a.voucher=' +
        'b.voucher_no  '
      
        'left join t_debit_trx_real c on a.voucher=c.voucher where b.tang' +
        'gal between '#39'2025-01-01'#39' and '#39'2025-04-21'#39'   '
      'UNION ALL'
      
        '(SELECT voucher_no,voucher_tmp,trans_date,code_account,descripti' +
        'on,kepada,paid_amount,saldo_debit from v_titipan_bpjs_penerimaan' +
        '_kas where code_account='#39'2130.07'#39' and trans_date between '#39'2025-0' +
        '1-01'#39' and '#39'2025-04-21'#39')  '
      'UNION ALL '
      
        '(select voucher_no,'#39#39' voucher_tmp,trans_date,code_account,descri' +
        'ption deskripsi,case when code_account='#39'2130.02'#39' then '#39'PPH 23'#39' w' +
        'hen code_account='#39'2130.01'#39' then '#39'PPH 21'#39' END kepada, paid_amount' +
        ' saldo_kredit, 0 saldo_debit from '
      
        '(SELECT  a.voucher_no,a.trans_date,b.code_account,a.description,' +
        'a.name_cust,b.paid_amount FROM t_cash_bank_acceptance a INNER JO' +
        'IN t_cash_bank_acceptance_det b on a.voucher_no=b.voucher_no)a '
      
        'where trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and code_' +
        'account='#39'2130.07'#39'  order by trans_date,voucher_no Asc ))xx  '
      'LEFT JOIN '
      
        '(select DISTINCT k.voucher_no voucher,K.trans_date as periode1,k' +
        '.kepada,l.supplier_code from (SELECT a.voucher_no,a.trans_date,a' +
        '.to_ as kepada from t_cash_bank_expenditure a INNER JOIN t_cash_' +
        'bank_expenditure_det b on a.voucher_no=b.no_voucher) k '
      
        'left join t_supplier l on k.kepada=l.supplier_name where k.trans' +
        '_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39') r on xx.voucher=r.v' +
        'oucher  '
      
        'LEFT JOIN t_paid_debt_det t on t.periode1=r.periode1  and r.supp' +
        'lier_code=t.supplier_code '
      
        'LEFT JOIN "V_do_lpb2" v  on t.faktur_no=v.no_fp_no_inv where cas' +
        'e WHEN t.npph is null then 0 else t.npph end <> 0 ) '
      'UNION ALL '
      
        '(select a.voucher_no voucher, b.voucher_tmp, b.trans_date tangga' +
        'l, b.code_account, c.deskripsi, kepada, c.no_fp_no_inv nofaktur,' +
        '0 ::float saldo_kredit, 0 ::float saldo_debit,c.dpp,c.pph23 sald' +
        'o,0 ppn from t_payment_detail_real a '
      
        'left join (SELECT a.voucher_no,a.voucher_tmp,a.trans_date,b.code' +
        '_account,a.to_ as kepada from t_cash_bank_expenditure a INNER JO' +
        'IN t_cash_bank_expenditure_det b on a.voucher_no=b.no_voucher)b ' +
        'on a.voucher_no=b.voucher_no '
      'left join "V_do_lpb2" c on a.voucher_no=c.voucher  '
      
        'where b.code_account='#39'2130.07'#39' and b.trans_date  between '#39'2025-0' +
        '1-01'#39' and '#39'2025-04-21'#39'  and  a.source_id=3 and no_fp_no_inv is n' +
        'ot null ) '
      'UNION ALL  '
      
        '(SELECT a.voucher_no as voucher,'#39#39' voucher_tmp,a.trans_date as t' +
        'anggal,a.code_account,a.description deskripsi,a.name_cust as kep' +
        'ada,'#39#39' as nofaktur, 0 saldo_kredit,  0 saldo_debit,0 as dpp,paid' +
        '_amount as saldo,0 ppn  from '
      
        '(SELECT a.voucher_no,a.trans_date,b.code_account,a.description,a' +
        '.name_cust,b.paid_amount FROM t_cash_bank_acceptance a INNER JOI' +
        'N t_cash_bank_acceptance_det b on a.voucher_no=b.voucher_no)a '
      
        'where trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39'  and code' +
        '_account='#39'2130.07'#39' order by trans_date,voucher_no Asc)'
      'UNION ALL  '
      
        'SELECT '#39#39' as voucher,'#39#39' as voucher_tmp,b.trans_date,a.account_pp' +
        'h_code, '
      '(case when a.account_pph_code='#39'2130.02'#39' then '#39'PPH PS 23'#39' '
      'when a.account_pph_code='#39'2130.01'#39' then '#39'PPH PS 21'#39'  '
      'when a.account_pph_code='#39'2130.03'#39' then '#39'PPH PS 25'#39'  '
      'when a.account_pph_code='#39'2130.04'#39' then '#39'PPH PS 4(2)'#39' '
      'when a.account_pph_code='#39'2130.07'#39' then '#39'PPH PS 15'#39'  '
      
        'when a.account_pph_code='#39'2130.06'#39' then '#39'PPH PS 22'#39' else a.accoun' +
        't_pph_code end) as deskripsi,supplier_name as kepada,b.faktur_no' +
        ',0 saldo_kredit,0 saldo_debit,a.subtotalrp as dpp,a.pph_rp as sa' +
        'ldo ,a.ppn_rp from t_purchase_invoice_det a  '
      'left join t_purchase_invoice b on a.trans_no=b.trans_no  '
      
        'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code where ' +
        'b.trans_date > '#39'2023-01-01'#39' and b.trans_date BETWEEN '#39'2025-01-01' +
        #39' and '#39'2025-04-21'#39' and a.account_pph_code='#39'2130.07'#39' and a.trans_' +
        'no not in (select invoice_no FROM t_cash_bank_expenditure_payabl' +
        'e)  '
      'UNION ALL '
      
        '(SELECT a.bk_no as voucher,'#39#39' as voucher_tmp,a.trans_date tangga' +
        'l,b.account_code,notes deskripsi,'#39#39' as kepada,'#39#39'as nofaktur,0 sa' +
        'ldo_kredit,0 saldo_debit,0 as dpp,-(b.debit) as saldo,0 ppn FROM' +
        ' t_memorial_journal a '
      
        'LEFT JOIN t_memorial_journal_detail b ON a.memo_no=b.memo_no WHE' +
        'RE b.account_code='#39'2130.07'#39' and a.trans_date between '#39'2025-01-01' +
        #39' and '#39'2025-04-21'#39' and b.debit <0  order by a.trans_date asc))yy' +
        ' where yy.voucher is not null  order by yy.tanggal,yy.voucher,yy' +
        '.faktur_no asc)z '
      ''
      'UNION ALL'
      '/* --Hutang PPH PS 25 2130.03'#9'2130'#9'Hutang Pajak - PPh.Ps.25 */ '
      
        'SELECT voucher,voucher_tmp,tanggal,account_code kode_account,des' +
        'kripsi,kepada,faktur_no,saldo_kredit,saldo_debit,dpp,saldo,0 ppn' +
        ','
      '(case when account_code='#39'2130.01'#39' then saldo else 0 end)pph21,'
      '(case when account_code='#39'2130.06'#39' then saldo else 0 end) pph22,'
      '(case when account_code='#39'2130.02'#39' then saldo else 0 end) pph23,'
      '(case when account_code='#39'2130.07'#39' then saldo else 0 end) pph15,'
      
        '(case when account_code='#39'2130.04'#39' then saldo else 0 end) pph4_2 ' +
        'FROM '
      '( select yy.* from '
      '('
      
        '(select distinct xx.voucher,xx.voucher_tmp,xx.tanggal,xx.account' +
        '_code,v.deskripsi,xx.kepada,  t.faktur_no,0 ::float saldo_kredit' +
        ',0 ::float saldo_debit,case WHEN v.dpp is null then 0 else v.dpp' +
        ' end dpp,  case WHEN t.npph is null then 0 else t.npph end saldo' +
        ', 0 ppn from  '
      
        '(select a.voucher,b.voucher_tmp,b.tanggal,a.account_code,a.descr' +
        'iption as deskripsi, b.kepada,a.saldo saldo_kredit,case when c.s' +
        'aldo  is null then 0 else c.saldo end saldo_debit from  '
      
        '(select * from t_credit_trx_real where account_code='#39'2130.03'#39')a ' +
        '  '
      'left join '
      
        '(select distinct voucher_no,voucher_tmp,to_ as kepada,trans_date' +
        ' as tanggal from '
      
        '(SELECT a.voucher_no,a.voucher_tmp,a.to_,a.trans_date from t_cas' +
        'h_bank_expenditure a INNER JOIN t_cash_bank_expenditure_det b on' +
        ' a.voucher_no=b.no_voucher)x where trans_date between '#39'2025-01-0' +
        '1'#39' and '#39'2025-04-21'#39')b on a.voucher=b.voucher_no   '
      
        'left join t_debit_trx_real c on a.voucher=c.voucher where b.tang' +
        'gal between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' '
      'UNION ALL  '
      
        '(SELECT voucher_no,voucher_tmp,trans_date,code_account,deskripsi' +
        ',kepada,paid_amount as jumlah,saldo_debit from v_titipan_bpjs_pe' +
        'nerimaan_kas where code_account='#39'2130.03'#39' and trans_date between' +
        ' '#39'2025-01-01'#39' and '#39'2025-04-21'#39') '
      'UNION ALL '
      
        '(select voucher_no,'#39#39' voucher_tmp,trans_date,code_account,descri' +
        'ption deskripsi, '
      
        'case when code_account='#39'2130.02'#39' then '#39'PPH 23'#39' when code_account' +
        '='#39'2130.01'#39' then '#39'PPH 21'#39' END kepada,paid_amount saldo_kredit,0 s' +
        'aldo_debit from '
      
        '(SELECT a.voucher_no,a.trans_date,b.code_account,a.description,b' +
        '.paid_amount FROM t_cash_bank_acceptance a INNER JOIN t_cash_ban' +
        'k_acceptance_det b on a.voucher_no=b.voucher_no)j where trans_da' +
        'te between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and code_account='#39'2130.' +
        '03'#39'  order by trans_date,voucher_no Asc ))xx  '
      
        'LEFT JOIN (select DISTINCT k.voucher_no,K.trans_date as periode1' +
        ',k.kepada,l.supplier_code from '
      
        '(SELECT a.voucher_no,a.trans_date,a.to_ as kepada from t_cash_ba' +
        'nk_expenditure a INNER JOIN t_cash_bank_expenditure_det b on a.v' +
        'oucher_no=b.no_voucher) k '
      
        'left join t_supplier l on k.kepada=l.supplier_name where k.trans' +
        '_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' ) r on xx.voucher=r.' +
        'voucher_no  '
      
        'LEFT JOIN t_paid_debt_det t on t.periode1=r.periode1 and r.suppl' +
        'ier_code=t.supplier_code '
      
        'LEFT JOIN "V_do_lpb2" v  on t.faktur_no=v.no_fp_no_inv where cas' +
        'e WHEN t.npph is null then 0 else t.npph end <> 0 )  '
      'UNION ALL '
      
        '(select a.voucher_no voucher,b.voucher_tmp,b.trans_date tanggal,' +
        'b.code_account,c.deskripsi,b.to_ as kepada,c.no_fp_no_inv nofakt' +
        'ur,0 ::float saldo_kredit, 0 ::float saldo_debit,c.dpp,c.pph23 s' +
        'aldo,0 ppn from t_payment_detail_real a '
      
        'left join (SELECT a.voucher_no,a.voucher_tmp,a.trans_date,b.code' +
        '_account,a.to_ from t_cash_bank_expenditure a INNER JOIN t_cash_' +
        'bank_expenditure_det b on a.voucher_no=b.no_voucher) b on a.vouc' +
        'her_no=b.voucher_no '
      
        'left join "V_do_lpb2" c on a.voucher_no=c.voucher  where b.code_' +
        'account='#39'2130.03'#39' and b.trans_date between '#39'2025-01-01'#39' and '#39'202' +
        '5-04-21'#39' and  a.source_id=3 and no_fp_no_inv is not null ) '
      'UNION ALL '
      
        '(select distinct m.voucher_no,m.voucher_tmp,m.trans_date as tang' +
        'gal,n.account_code,(case when n.description1='#39#39' then n.descripti' +
        'on else n.description1 end) deskripsi,m.to_ as kepada,n.inv_supp' +
        '_no nofaktur,0 ::float saldo_kredit,0 ::float saldo_debit,n.dpp,' +
        'n.saldo,0 ppn from '
      
        '(SELECT a.voucher_no,a.voucher_tmp,a.trans_date,a.to_ from t_cas' +
        'h_bank_expenditure a INNER JOIN t_cash_bank_expenditure_det b on' +
        ' a.voucher_no=b.no_voucher) m ,t_credit_trx_real n where m.vouch' +
        'er_no=n.voucher and n.Account_code='#39'2130.03'#39' and m.trans_date be' +
        'tween '#39'2025-01-01'#39' and '#39'2025-04-21'#39' ) '
      'UNION ALL'
      
        '(SELECT voucher_no as voucher,'#39#39' voucher_tmp,trans_date as tangg' +
        'al,code_account,description deskripsi,name_cust as kepada,'#39#39' as ' +
        'nofaktur, 0 saldo_kredit,  0 saldo_debit,0 as dpp,paid_amount as' +
        ' saldo,0 ppn from (SELECT a.voucher_no,a.trans_date,b.code_accou' +
        'nt,b.description,a.name_cust,b.paid_amount FROM t_cash_bank_acce' +
        'ptance a INNER JOIN t_cash_bank_acceptance_det b on a.voucher_no' +
        '=b.voucher_no)a where trans_date between '#39'2025-01-01'#39' and '#39'2025-' +
        '04-21'#39'  and code_account='#39'2130.03'#39' order by trans_date,voucher_n' +
        'o Asc) '
      'UNION ALL '
      
        'SELECT '#39#39' as voucher,'#39#39' as voucher_tmp,b.trans_date,a.account_pp' +
        'h_code, '
      '(case when a.account_pph_code='#39'2130.02'#39' then '#39'PPH PS 23'#39' '
      'when a.account_pph_code='#39'2130.01'#39' then '#39'PPH PS 21'#39' '
      'when a.account_pph_code='#39'2130.03'#39' then '#39'PPH PS 25'#39'  '
      'when a.account_pph_code='#39'2130.04'#39' then '#39'PPH PS 4(2)'#39' '
      'when a.account_pph_code='#39'2130.07'#39' then '#39'PPH PS 15'#39'  '
      
        'when a.account_pph_code='#39'2130.06'#39' then '#39'PPH PS 22'#39' else a.accoun' +
        't_pph_code end) as deskripsi, supplier_name as kepada,b.faktur_n' +
        'o,0 saldo_kredit,0 saldo_debit,a.subtotalrp as dpp,a.pph_rp as s' +
        'aldo,a.ppn_rp from t_purchase_invoice_det a  '
      'left join t_purchase_invoice b on a.trans_no=b.trans_no  '
      'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code '
      
        'where b.trans_date > '#39'2023-01-01'#39'   and b.trans_date BETWEEN '#39'20' +
        '25-01-01'#39' and '#39'2025-04-21'#39' and a.account_pph_code='#39'2130.03'#39'  '
      'UNION ALL  '
      
        '(SELECT a.bk_no as voucher,'#39#39' as voucher_tmp,a.trans_date,b.acco' +
        'unt_code,notes deskripsi,'#39#39' as kepada,'#39#39'as nofaktur,0 saldo_kred' +
        'it,0 saldo_debit,0 as dpp,-(b.debit) as saldo,0 ppn FROM t_memor' +
        'ial_journal a '
      
        'LEFT JOIN t_memorial_journal_detail b ON a.memo_no=b.memo_no WHE' +
        'RE b.account_code='#39'2130.03'#39' and a.trans_date between '#39'2025-01-01' +
        #39' and '#39'2025-04-21'#39' order by a.trans_date asc))yy where yy.vouche' +
        'r is not null  order by yy.tanggal,yy.voucher,yy.faktur_no asc)K' +
        'K   '
      ''
      'UNION ALL '
      '/*---Hutang PPH PS 4 ayat 2 */ '
      
        'SELECT voucher,'#9'voucher_tmp,tanggal,account_code,deskripsi,kepad' +
        'a,nofaktur,saldo_kredit,saldo_debit,dpp,saldo,0 ppn,'
      '(case when account_code='#39'2130.01'#39' then saldo else 0 end)pph21,'
      '(case when account_code='#39'2130.06'#39' then saldo else 0 end) pph22,'
      '(case when account_code='#39'2130.02'#39' then saldo else 0 end) pph23,'
      '(case when account_code='#39'2130.07'#39' then saldo else 0 end) pph15, '
      
        '(case when account_code='#39'2130.04'#39' then saldo else 0 end) pph4_2 ' +
        'FROM '
      '(  '
      
        '(select xx.voucher,xx.voucher_tmp,xx.tanggal,xx.account_code,xx.' +
        'deskripsi,xx.kepada,xx.nofaktur,0 ::float saldo_kredit,0 ::float' +
        ' saldo_debit,xx.dpp, case WHEN xx.saldo_kredit is null then 0 el' +
        'se xx.saldo_kredit end saldo, 0 ppn from '
      
        '(select a.voucher,b.voucher_tmp,b.trans_date tanggal,a.account_c' +
        'ode,a.description1 as deskripsi, b.kepada,a.inv_supp_no nofaktur' +
        ',a.saldo saldo_kredit,case when c.saldo  is null then 0 else c.s' +
        'aldo end saldo_debit,A.dpp from '
      
        '(select * from t_credit_trx_real where account_code='#39'2130.04'#39')a ' +
        ' '
      'left join '
      '(select distinct voucher_no,voucher_tmp,kepada,trans_date  from '
      
        '(SELECT a.voucher_no,a.voucher_tmp,a.trans_date,a.to_ as kepada ' +
        'from t_cash_bank_expenditure a INNER JOIN t_cash_bank_expenditur' +
        'e_det b on a.voucher_no=b.no_voucher)x where trans_date between ' +
        #39'2025-01-01'#39' and '#39'2025-04-21'#39')b on a.VOUCHER=b.voucher_no '
      
        'left join t_debit_trx_real c on a.voucher=c.voucher where b.tran' +
        's_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39'  '
      'UNION ALL '
      
        '(SELECT voucher_no,voucher_tmp,trans_date,code_account,deskripsi' +
        ',kepada,description keterangan,paid_amount,saldo_debit,0 dpp fro' +
        'm v_titipan_bpjs_penerimaan_kas where code_account='#39'2130.04'#39' and' +
        ' trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39') '
      'UNION ALL  '
      
        '(select voucher_no,'#39#39' voucher_tmp,trans_date,code_account,descri' +
        'ption deskripsi,name_cust diterima_dari,description, paid_amount' +
        ' saldo_kredit, 0 saldo_debit,0 dpp from '
      
        '(SELECT a.voucher_no,a.trans_date,b.code_account,a.description,a' +
        '.name_cust,b.paid_amount FROM t_cash_bank_acceptance a INNER JOI' +
        'N t_cash_bank_acceptance_det b on a.voucher_no=b.voucher_no)x wh' +
        'ere trans_date between '#39'2025-01-01'#39' and '#39'2025-04-21'#39' and code_ac' +
        'count='#39'2130.04'#39' order by trans_date,voucher_no Asc ))xx '
      'LEFT JOIN '
      
        '(select DISTINCT k.voucher_no,k.periode1,k.periode2,k.plan_to,k.' +
        'to_,l.supplier_code from (SELECT a.voucher_no,a.periode1,a.perio' +
        'de2,a.plan_to,a.to_ from t_cash_bank_expenditure_submission a IN' +
        'NER JOIN t_cash_bank_expenditure_submission_det b on a.voucher_n' +
        'o=b.no_voucher'
      ') k '
      
        'left join t_supplier l on k.to_=l.supplier_name) r on xx.voucher' +
        '_tmp=r.voucher_no where case WHEN xx.saldo_kredit is null then 0' +
        ' else xx.saldo_kredit end <> 0 order by xx.tanggal,xx.voucher) '
      'UNION All  '
      
        'SELECT '#39#39' as voucher,'#39#39' as voucher_tmp,b.trans_date tgl_terima,a' +
        '.account_pph_code, (case when a.account_pph_code='#39'2130.02'#39' then ' +
        #39'PPH PS 23'#39' when a.account_pph_code='#39'2130.01'#39' then '#39'PPH PS 21'#39' w' +
        'hen a.account_pph_code='#39'2130.03'#39' then '#39'PPH PS 25'#39'  when a.accoun' +
        't_pph_code='#39'2130.04'#39' then '#39'PPH PS 4(2)'#39' when a.account_pph_code=' +
        #39'2130.07'#39' then '#39'PPH PS 15'#39'  when a.account_pph_code='#39'2130.06'#39' th' +
        'en '#39'PPH PS 22'#39' else a.account_pph_code end) as deskripsi,  suppl' +
        'ier_name as kepada,b.faktur_no,0 saldo_kredit,0 saldo_debit,a.su' +
        'btotalrp as dpp,a.pph_rp as saldo,a.ppn_rp  from t_purchase_invo' +
        'ice_det a '
      'left join t_purchase_invoice b on a.trans_no=b.trans_no '
      
        'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code where ' +
        'b.trans_date >'#39'2023-01-01'#39' and b.trans_date BETWEEN '#39'2025-01-01'#39 +
        ' and '#39'2025-04-21'#39' and a.account_pph_code='#39'2130.04'#39') zz')
    Active = True
    Left = 864
    Top = 168
  end
end
