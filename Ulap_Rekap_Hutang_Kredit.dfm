object Flap_Rekap_Hutang_Kredit: TFlap_Rekap_Hutang_Kredit
  Left = 0
  Top = 0
  Caption = 'Flap_Rekap_Hutang_Kredit'
  ClientHeight = 442
  ClientWidth = 852
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  TextHeight = 15
  object dxRibbon1: TdxRibbon
    Left = 0
    Top = 0
    Width = 852
    Height = 127
    BarManager = dxBarManager1
    Style = rs2010
    ColorSchemeName = 'Blue'
    Contexts = <>
    TabOrder = 0
    TabStop = False
    object dxRibbon1Tab1: TdxRibbonTab
      Active = True
      Caption = 'Home'
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar1'
        end>
      Index = 0
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 401
    Width = 852
    Height = 41
    Align = alBottom
    TabOrder = 1
    Visible = False
    ExplicitTop = 392
    ExplicitWidth = 846
    object BBatal: TRzBitBtn
      Left = 776
      Top = 1
      Height = 39
      Align = alRight
      Caption = 'Batal'
      TabOrder = 0
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 770
    end
    object BPrint: TRzBitBtn
      Left = 697
      Top = 1
      Width = 79
      Height = 39
      Align = alRight
      Caption = 'Print'
      TabOrder = 1
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 691
    end
  end
  object DBGridHutangKredit: TDBGridEh
    Left = 0
    Top = 127
    Width = 852
    Height = 274
    Align = alClient
    Color = clBtnFace
    DataGrouping.GroupLevels = <
      item
      end
      item
      end>
    DataSource = DSRekap_Hutang_Kredit
    DrawMemoText = True
    DynProps = <>
    IndicatorOptions = [gioShowRowIndicatorEh, gioShowRecNoEh]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
    OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghDialogFind, dghShowRecNo, dghColumnResize, dghColumnMove, dghExtendVertLines]
    SearchPanel.Enabled = True
    TabOrder = 2
    TitleParams.MultiTitle = True
    Columns = <
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'No. BK'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'Tanggal'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'No. Faktur / Invoice'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'Deskripsi'
        Width = 200
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'Nama Kena Pph'
        Width = 200
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'Dpp'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        Footers = <>
        Title.Caption = 'Jumlah'
        Width = 100
      end>
    object RowDetailData: TRowDetailPanelControlEh
    end
  end
  object DSRekap_Hutang_Kredit: TDataSource
    AutoEdit = False
    DataSet = MemRekap_Hutang_Kredit
    Left = 280
    Top = 280
  end
  object MemRekap_Hutang_Kredit: TMemTableEh
    Params = <>
    DataDriver = DataSetDriverEh1
    Left = 376
    Top = 216
  end
  object DataSetDriverEh1: TDataSetDriverEh
    Left = 424
    Top = 304
  end
  object ActMenu: TActionManager
    Left = 736
    Top = 29
    StyleName = 'Platform Default'
    object ActBaru: TAction
      Caption = 'Baru  '
    end
    object ActUpdate: TAction
      Caption = 'Update  '
    end
    object ActRO: TAction
      Caption = 'Refresh  '
    end
    object ActDel: TAction
      Caption = 'Delete'
    end
    object ActPrint: TAction
      Caption = 'Print  '
      OnExecute = ActPrintExecute
    end
    object ActApp: TAction
      Caption = 'Approve  '
      Enabled = False
    end
    object ActReject: TAction
      Caption = 'Reject  '
      Enabled = False
    end
    object ActClose: TAction
      Caption = 'CLose PO    '
      Enabled = False
    end
  end
  object QPerusahaan: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      'select * from t_company')
    Left = 800
    Top = 80
    object QPerusahaancompany_code: TStringField
      FieldName = 'company_code'
      Required = True
      Size = 10
    end
    object QPerusahaancompany_name: TStringField
      FieldName = 'company_name'
      Size = 255
    end
    object QPerusahaanaddress: TStringField
      FieldName = 'address'
      Size = 255
    end
    object QPerusahaantelp: TStringField
      FieldName = 'telp'
    end
    object QPerusahaanemail: TStringField
      FieldName = 'email'
      Size = 255
    end
    object QPerusahaannpwp: TStringField
      FieldName = 'npwp'
      Size = 50
    end
    object QPerusahaancity: TStringField
      FieldName = 'city'
      Size = 255
    end
    object QPerusahaanaddress2: TStringField
      FieldName = 'address2'
      Size = 255
    end
    object QPerusahaanid: TGuidField
      FieldName = 'id'
      Required = True
      Size = 38
    end
    object QPerusahaancreated_at: TDateTimeField
      FieldName = 'created_at'
    end
    object QPerusahaancreated_by: TStringField
      FieldName = 'created_by'
      Size = 50
    end
    object QPerusahaanupdated_at: TDateTimeField
      FieldName = 'updated_at'
    end
    object QPerusahaanupdated_by: TStringField
      FieldName = 'updated_by'
      Size = 50
    end
    object QPerusahaandeleted_at: TDateTimeField
      FieldName = 'deleted_at'
    end
    object QPerusahaandeleted_by: TStringField
      FieldName = 'deleted_by'
      Size = 50
    end
    object QPerusahaantype_of_business: TStringField
      FieldName = 'type_of_business'
      Size = 255
    end
    object QPerusahaanlatitude: TStringField
      FieldName = 'latitude'
      Size = 255
    end
    object QPerusahaanlongitude: TStringField
      FieldName = 'longitude'
      Size = 255
    end
    object QPerusahaantax_status: TBooleanField
      FieldName = 'tax_status'
    end
    object QPerusahaancurrency: TStringField
      FieldName = 'currency'
      Size = 10
    end
  end
  object dxBarManager1: TdxBarManager
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    Categories.Strings = (
      'Default')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    PopupMenuLinks = <>
    UseSystemFont = True
    Left = 696
    Top = 31
    PixelsPerInch = 96
    object dxBarManager1Bar1: TdxBar
      Caption = 'Action'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 805
      FloatTop = 8
      FloatClientWidth = 146
      FloatClientHeight = 126
      ItemLinks = <
        item
          UserDefine = [udWidth]
          UserWidth = 104
          Visible = True
          ItemName = 'DTPick1'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 107
          Visible = True
          ItemName = 'DTPick2'
        end
        item
          BeginGroup = True
          Visible = True
          ItemName = 'dxBarLargeButton1'
        end
        item
          Visible = True
          ItemName = 'edkode_akun'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 233
          Visible = True
          ItemName = 'Ed_Account'
        end
        item
          Visible = True
          ItemName = 'DxRefresh'
        end
        item
          BeginGroup = True
          Visible = True
          ItemName = 'dxPrint'
        end>
      OneOnRow = True
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object dxBUpdate: TdxBarButton
      Caption = 'Update  '
      Category = 0
      Visible = ivNever
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFCF7ECFFEDC36AFFF1D08BFFFFFFFFFFFFFFFFFF}
    end
    object dxBDelete: TdxBarButton
      Caption = 'Delete  '
      Category = 0
      Visible = ivNever
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFBFBFFFFF0C0CFFFFABABFFFFFFFFFFFFFFFFFFFF}
    end
    object dxBbaru: TdxBarLargeButton
      Caption = 'Baru  '
      Category = 0
      Visible = ivNever
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        FFFFFFFFFFFF}
    end
    object dxBarSubItem1: TdxBarSubItem
      Caption = 'Print  '
      Category = 0
      Enabled = False
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ItemLinks = <
        item
          Visible = True
          ItemName = 'BProduksi'
        end
        item
          Visible = True
          ItemName = 'BNonProduksi'
        end>
    end
    object BProduksi: TdxBarButton
      Caption = 'Barang Produksi'
      Category = 0
      Hint = 'Barang Produksi'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object BNonProduksi: TdxBarButton
      Caption = 'Barang Non Produksi'
      Category = 0
      Hint = 'Barang Non Produksi'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object BClosed: TdxBarButton
      Caption = 'CLose Kontrak    '
      Category = 0
      Enabled = False
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        F7FFF7F7F7FF}
    end
    object dxPrint: TdxBarLargeButton
      Action = ActPrint
      Category = 0
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        }
    end
    object DxRefresh: TdxBarLargeButton
      Caption = 'Cari'
      Category = 0
      Hint = 'Cari'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        2F673E0D0A09093C2F673E0D0A093C2F673E0D0A3C2F7376673E0D0A}
    end
    object DtMulai: TcxBarEditItem
      Caption = 'Tanggal Awal   '
      Category = 0
      Hint = 'Tanggal Awal   '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object DtSelesai: TcxBarEditItem
      Caption = 'Tanggal Akhir  '
      Category = 0
      Hint = 'Tanggal Akhir  '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object CbSupplier: TcxBarEditItem
      Caption = 'Supplier'
      Category = 0
      Hint = 'Supplier'
      Visible = ivAlways
      Width = 150
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
    end
    object CbBarang: TcxBarEditItem
      Caption = 'Barang  '
      Category = 0
      Hint = 'Barang  '
      Visible = ivAlways
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
    end
    object Cxppn: TcxBarEditItem
      Caption = 'PPN'
      Category = 0
      Hint = 'PPN'
      Visible = ivAlways
      ShowCaption = True
      Width = 0
      PropertiesClassName = 'TcxCheckBoxProperties'
      Properties.ImmediatePost = True
      InternalEditValue = False
    end
    object BtnClear: TdxBarLargeButton
      Caption = 'Clear'
      Category = 0
      Hint = 'Clear'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        B8F198F83F1812900DC3DC08450000000049454E44AE426082}
    end
    object txtkdacckredit1: TdxBarEdit
      Caption = 'Kode Akun         : '
      Category = 0
      Hint = 'Kode Akun         : '
      Visible = ivAlways
      Text = '1112'
    end
    object dxBarEdit3: TdxBarEdit
      Caption = 'Nama Akun    :   '
      Category = 0
      Hint = 'Nama Akun    :   '
      Visible = ivAlways
      Text = 'KAS KECIL'
    end
    object dxBarEdit2: TdxBarEdit
      Caption = 'Tanggal'
      Category = 0
      Hint = 'Tanggal'
      Visible = ivAlways
    end
    object dxBarEdit4: TdxBarEdit
      Category = 0
      Visible = ivNever
    end
    object cxBarEditItem1: TcxBarEditItem
      Category = 0
      Visible = ivNever
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object DTPick1: TcxBarEditItem
      Caption = 'Tanggal Awal    :   '
      Category = 0
      Hint = 'Tanggal Awal    :   '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object DTPick2: TcxBarEditItem
      Caption = 'Tanggal Akhir   :   '
      Category = 0
      Hint = 'Tanggal Akhir   :   '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object dxBarLargeButton1: TdxBarLargeButton
      Category = 0
      Visible = ivNever
    end
    object Ed_Account: TcxBarEditItem
      Caption = 'Pilih Account :'
      Category = 0
      Hint = 'Pilih Account :'
      Visible = ivAlways
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = cxBarEditItem2PropertiesButtonClick
    end
    object edkode_akun: TcxBarEditItem
      Caption = 'Kode              :'
      Category = 0
      Hint = 'Kode              :'
      Visible = ivAlways
    end
  end
  object frxDBDatasetPers: TfrxDBDataset
    UserName = 'frxDBDatasetPers'
    CloseDataSource = False
    FieldAliases.Strings = (
      'company_code=company_code'
      'company_name=company_name'
      'address=address'
      'telp=telp'
      'email=email'
      'npwp=npwp'
      'city=city'
      'address2=address2'
      'id=id'
      'created_at=created_at'
      'created_by=created_by'
      'updated_at=updated_at'
      'updated_by=updated_by'
      'deleted_at=deleted_at'
      'deleted_by=deleted_by'
      'type_of_business=type_of_business'
      'latitude=latitude'
      'longitude=longitude'
      'tax_status=tax_status'
      'currency=currency')
    DataSet = QPerusahaan
    BCDToCurrency = False
    DataSetOptions = []
    Left = 776
    Top = 32
  end
  object QRekap_Hutang_Kredit: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'select a.* ,c.bank_name as nabankkk,d.bank_name as nabankkm, b.d' +
        'eposito_no,b.bk_no,b.interest_value,b.bm_no,b.description,b.desc' +
        'ription2,c.amount as jum_kk,d.amount as jum_km,c.trans_date as t' +
        'glbk,d.trans_date as tglbm from  '
      
        '(select * from t_deposito_submission where deposito_date between' +
        ' '#39'2022-05-23'#39' and '#39'2022-05-23'#39')a '
      'left join t_deposito b on a.id=b.id_submission  '
      
        'left join(select a.voucher_no,a.bank_name,a.amount,b.trans_date ' +
        'from t_cash_bank_expenditure a INNER JOIN t_cash_bank_expenditur' +
        'e_det b on a.voucher_no=b.no_voucher) c on b.bk_no=c.voucher_no '
      
        'left join(select a.voucher_no,a.amount,a.bank_name,b.trans_date ' +
        'from t_cash_bank_expenditure a INNER JOIN t_cash_bank_expenditur' +
        'e_det b on a.voucher_no=b.no_voucher) d on b.bm_no=d.voucher_no ' +
        ' order by deposito_date,deposito_no')
    Left = 264
    Top = 200
  end
  object QLap_Rekap_Hutang_Kredit: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'SELECT voucher,voucher_tmp,tanggal,account_code,deskripsi,kepada' +
        ',nofaktur,saldo_kredit,saldo_debit,dpp,saldo,ppn,'
      '(case when account_code='#39'2130.01'#39' then saldo else 0 end)pph21,'
      '(case when account_code='#39'2130.06'#39' then saldo else 0 end) pph22,'
      '(case when account_code='#39'2130.02'#39' then saldo else 0 end) pph23, '
      '(case when account_code='#39'2130.07'#39' then saldo else 0 end) pph15,'
      '(case when account_code='#39'2130.04'#39' then saldo else 0 end) pph4_2,'
      '(case when account_code='#39'2130.03'#39' then saldo else 0 end) pph25,'
      '(case when account_code='#39'2130.05'#39' then saldo else 0 end) pph29'
      'from'
      '(select yy.* from '
      '('
      
        '(select distinct xx.voucher,xx.voucher_tmp,xx.tanggal,xx.account' +
        '_code,v.deskripsi,xx.kepada,t.faktur_no nofaktur,0 ::float saldo' +
        '_kredit,0 ::float saldo_debit,case WHEN v.dpp is null then 0 els' +
        'e v.dpp end dpp,  case WHEN t.npph is null then 0 else t.npph en' +
        'd saldo, 0 as ppn from  '
      
        '(select a.voucher,b.voucher_tmp,b.trans_date as tanggal,a.accoun' +
        't_code,a.description as deskripsi,b.to_ kepada,a.saldo saldo_kre' +
        'dit,case when c.saldo  is null then 0 else c.saldo end saldo_deb' +
        'it from  '
      '(select * from t_credit_trx_real where account_code='#39'2130.06'#39')a '
      'left join '
      '(select distinct voucher_no,voucher_tmp,to_,trans_date from '
      
        '(select a.voucher_no,a.voucher_tmp,a.to_,b.trans_date from t_cas' +
        'h_bank_expenditure a INNER JOIN t_cash_bank_expenditure_det b on' +
        ' a.voucher_no=b.no_voucher)a where trans_date between '#39'2025-02-0' +
        '4'#39' and '#39'2025-02-18'#39' and substring(voucher_no,1,2)='#39'BK'#39' '
      'union all '
      
        'select distinct voucher_no,voucher_tmp,to_,trans_date as tanggal' +
        ' from (SELECT a.voucher_no,a.voucher_tmp,a.to_,b.trans_date FROM' +
        ' t_cash_bank_expenditure a INNER JOIN t_cash_bank_expenditure_de' +
        't b on a.voucher_no=b.no_voucher)a where  substring(voucher_no,1' +
        ',2)='#39'KK'#39' and trans_date between '#39'2025-02-04'#39' and '#39'2025-02-18'#39')b ' +
        ' on a.voucher=b.voucher_no '
      
        'left join t_debit_trx_real c on a.voucher=c.voucher where b.tran' +
        's_date between '#39'2025-02-04'#39' and '#39'2025-02-18'#39' '
      'UNION ALL '
      
        '(SELECT voucher_no,voucher_tmp,trans_date,code_account,deskripsi' +
        ',kepada,paid_amount,saldo_debit from v_titipan_bpjs_penerimaan_k' +
        'as where code_account='#39'2130.06'#39' and trans_date between '#39'2025-02-' +
        '04'#39' and '#39'2025-02-18'#39') '
      'UNION ALL '
      
        '(select voucher_no,'#39#39' voucher_tmp,trans_date,code_account,descri' +
        'ption deskripsi,case when code_account='#39'2130.02'#39' then '#39'Hutang Pa' +
        'jak - PPh.Ps.23'#39' when code_account='#39'2130.01'#39' then '#39'Hutang Pajak ' +
        '- PPh.Ps.21'#39' END kepada,saldo_kredit, 0 saldo_debit from (SELECT' +
        ' a.voucher_no,b.trans_date,b.description,b.code_account,a.for_ac' +
        'ceptance,b.paid_amount saldo_kredit FROM t_cash_bank_acceptance ' +
        'a INNER JOIN t_cash_bank_acceptance_det b on a.voucher_no=b.vouc' +
        'her_no)a where trans_date between '#39'2025-02-04'#39' and '#39'2025-02-18'#39' ' +
        'and code_account='#39'2130.06'#39'  order by trans_date,voucher_no Asc))' +
        'xx  '
      ''
      'LEFT JOIN '
      
        '(select DISTINCT k.voucher_no,K.periode1,k.to_,l.supplier_code f' +
        'rom (select a.voucher_no,b.trans_date as periode1,a.to_ from t_c' +
        'ash_bank_expenditure a INNER JOIN t_cash_bank_expenditure_det b ' +
        'on a.voucher_no=b.no_voucher) k left join t_supplier l on k.to_=' +
        'l.supplier_name where k.periode1 between '#39'2025-02-04'#39' and '#39'2025-' +
        '02-18'#39') r on xx.voucher=r.voucher_no '
      'LEFT JOIN '
      
        't_paid_debt_det t on t.periode1=r.periode1  and r.supplier_code=' +
        't.supplier_code '
      
        'LEFT JOIN "V_do_lpb2" v  on t.faktur_no=v.no_fp_no_inv where cas' +
        'e WHEN t.npph is null then 0 else t.npph end <> 0 ) '
      'UNION ALL'
      
        ' (select a.voucher_no voucher, b.voucher_tmp,b.tanggal, b.code_a' +
        'ccount, c.deskripsi,b.to_ as kepada, c.no_fp_no_inv nofaktur,0 :' +
        ':float saldo_kredit, 0 ::float saldo_debit,c.dpp,c.pph23 saldo, ' +
        '0 ppn from t_payment_detail_real a '
      
        ' left join (select a.voucher_no,a.voucher_tmp,b.trans_date tangg' +
        'al,b.code_account,a.to_ from t_cash_bank_expenditure a INNER JOI' +
        'N t_cash_bank_expenditure_det b on a.voucher_no=b.no_voucher) b ' +
        ' on a.voucher_no=b.voucher_no left join "V_do_lpb2" c on a.vouch' +
        'er_no=c.voucher where b.code_account='#39'2130.06'#39' and tanggal  betw' +
        'een '#39'2025-02-04'#39' and '#39'2025-02-18'#39' and a.source_id=3 and no_fp_no' +
        '_inv is not null) '
      ' UNION ALL'
      
        '(select distinct m.voucher_no,m.voucher_tmp,m.trans_date as tang' +
        'gal,n.account_code,(case when n.description1='#39#39' then n.descripti' +
        'on else n.description1 end) description, m.to_ kepada,n.inv_supp' +
        '_no nofaktur,0 ::float saldo_kredit,0 ::float saldo_debit,n.dpp,' +
        'n.saldo, 0 ppn from (select a.voucher_no,a.voucher_tmp,b.trans_d' +
        'ate,b.code_account,a."to_" from t_cash_bank_expenditure a INNER ' +
        'JOIN t_cash_bank_expenditure_det b on a.voucher_no=b.no_voucher)' +
        ' m , t_credit_trx_real n where m.voucher_no=n.voucher and n.acco' +
        'unt_code='#39'2130.06'#39' and m.trans_date between '#39'2025-02-04'#39' and '#39'20' +
        '25-02-18'#39') '
      'UNION ALL  '
      
        '(SELECT voucher_no as voucher,'#39#39' voucher_tmp,trans_date as tangg' +
        'al,code_account akun_kredit,description deskripsi,for_acceptance' +
        ' as kepada,'#39#39' as nofaktur, 0 saldo_kredit,  0 saldo_debit,0 as d' +
        'pp,paid_amount as saldo, 0 ppn from (SELECT a.voucher_no,a.trans' +
        '_date,b.code_account,a.description,a.for_acceptance,a.paid_amoun' +
        't FROM t_cash_bank_acceptance a INNER JOIN t_cash_bank_acceptanc' +
        'e_det b on a.voucher_no=b.voucher_no)a where trans_date between ' +
        #39'2025-02-04'#39' and '#39'2025-02-18'#39' and code_account='#39'2130.06'#39' order b' +
        'y trans_date,voucher Asc) '
      'UNION ALL '
      
        'SELECT '#39#39' as voucher,'#39#39' as voucher_tmp,b.receive_date,a.account_' +
        'pph_code,(case when a.account_pph_code='#39'2130.02'#39' then '#39'Hutang Pa' +
        'jak - PPh.Ps.23'#39' when a.account_pph_code='#39'2130.01'#39' then '#39'Hutang ' +
        'Pajak - PPh.Ps.21'#39' when a.account_pph_code='#39'2130.03'#39' then '#39'Hutan' +
        'g Pajak - PPh.Ps.25'#39'  when a.account_pph_code='#39'2130.04'#39' then '#39'Hu' +
        'tang Pajak - PPh.Ps.4(2)'#39' when a.account_pph_code='#39'2130.07'#39' then' +
        ' '#39'Hutang Pajak - PPh.Ps.15'#39'  when a.account_pph_code='#39'2130.06'#39' t' +
        'hen '#39'Hutang Pajak - PPh.Ps.22'#39' else a.account_pph_code end) as d' +
        'eskripsi, supplier_name as kepada,b.faktur_no,0 saldo_kredit,0 s' +
        'aldo_debit,a.subtotalrp as dpp,a.pph_rp as saldo,a.ppn_rp from t' +
        '_item_receive_det a  left join t_item_receive b on a.receive_no=' +
        'b.receive_no  LEFT JOIN t_supplier d on b.supplier_code=d.suppli' +
        'er_code where b.receive_date > '#39'2023-01-01'#39' and b.receive_date B' +
        'ETWEEN '#39'2025-02-04'#39' and '#39'2025-02-18'#39' and a.account_pph_code='#39'213' +
        '0.06'#39' '
      'UNION ALL '
      
        '(SELECT a.bk_no as voucher,'#39#39' as voucher_tmp,a.trans_date,b.acco' +
        'unt_code,notes as deskripsi,'#39#39' as kepada,'#39#39'as nofaktur,0 saldo_k' +
        'redit,0 saldo_debit,0 as dpp,-(b.debit) as saldo,0 ppn FROM t_me' +
        'morial_journal a LEFT JOIN t_memorial_journal_detail b ON a.memo' +
        '_no=b.memo_no WHERE b.account_code='#39'2130.06'#39' and a.trans_date be' +
        'tween '#39'2025-02-04'#39' and '#39'2025-02-18'#39' order by a.trans_date asc))y' +
        'y where yy.voucher is not null  order by yy.tanggal,yy.voucher,y' +
        'y.nofaktur asc)xx ')
    Left = 688
    Top = 165
  end
  object frxdbRekap_Hutang_Kredit: TfrxDBDataset
    UserName = 'frxdbdeposito'
    CloseDataSource = False
    FieldAliases.Strings = (
      'id=id'
      'bank_code=bank_code'
      'nobk_submission=nobk_submission'
      'on_behalf=on_behalf'
      'bank_norek=bank_norek'
      'deposito_date=deposito_date'
      'due_date=due_date'
      'deposito_value=deposito_value'
      'description=description'
      'company_code=company_code'
      'nabankkk=nabankkk'
      'nabankkm=nabankkm'
      'deposito_no=deposito_no'
      'bk_no=bk_no'
      'interest_value=interest_value'
      'bm_no=bm_no'
      'description_1=description_1'
      'description2=description2'
      'jum_kk=jum_kk'
      'jum_km=jum_km'
      'tglbk=tglbk'
      'tglbm=tglbm')
    DataSet = QLap_Rekap_Hutang_Kredit
    BCDToCurrency = False
    DataSetOptions = []
    Left = 788
    Top = 173
  end
  object unirekapkredittrx: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      ''
      
        'SELECT  a.voucher_no,a.trans_date tanggal,a.kodesp,x.deskripsi,k' +
        'epada,a.inv_supp_no as nofaktur ,y.dpp as dpp,a.pph as pph,'#39#39' as' +
        ' nodo FROM '
      '(SELECT DISTINCT a.*,b.inv_supp_no,c.saldo as pph FROM '
      
        '(SELECT DISTINCT voucher_no,trans_date,to_ kepada,code_account k' +
        'odesp FROM '
      
        '(SELECT a.voucher_no,b.trans_date,a.to_ ,b.code_account,b."posit' +
        'ion" from t_cash_bank_expenditure a INNER JOIN t_cash_bank_expen' +
        'diture_det b on a.voucher_no=b.no_voucher)r where trans_date bet' +
        'ween '#39'2022-11-15'#39' and '#39'2022-11-15'#39' and code_account='#39'2142'#39' and "' +
        'position"='#39'K'#39')a  LEFT JOIN t_buy_pay b on a.voucher_no=b.voucher' +
        '_no LEFT JOIN t_credit_trx_real c on a.voucher_no=c.voucher and ' +
        'b.inv_supp_no=c.inv_supp_no WHERE (c.voucher is not null ) and (' +
        'b.inv_supp_no is not null)) a LEFT JOIN  t_item_receive f ON f.f' +
        'aktur_no=a.inv_supp_no LEFT JOIN (SELECT  a.receive_no,string_ag' +
        'g (b.item_name, '#39', '#39' ) AS deskripsi FROM  t_item_receive_det a L' +
        'EFT JOIN t_item_stock b ON a.item_stock_code::text = b.item_stoc' +
        'k_code::text GROUP BY a.receive_no ) x On x.receive_no=f.receive' +
        '_no LEFT JOIN (SELECT receive_no,sum(subtotal) as dpp FROM t_ite' +
        'm_receive_det GROUP BY  receive_no) y on f.receive_no=y.receive_' +
        'no '
      'UNION ALL '
      
        'SELECT aa.voucher_no,aa.trans_date tanggal,aa.kodesp,b.descripti' +
        'on1,aa.kepada,b.inv_supp_no as nofaktur ,b.dpp as dpp,b.saldo as' +
        ' pph,'#39#39' as nodo  FROM (SELECT a.*,c.inv_supp_no,b.inv_supp_no FR' +
        'OM '
      
        '(SELECT distinct voucher_no,trans_date,kodesp,kepada FROM (SELEC' +
        'T a.voucher_no,b.trans_date,b.code_account kodesp,a.to_ kepada,b' +
        '."position" from t_cash_bank_expenditure a INNER JOIN t_cash_ban' +
        'k_expenditure_det b on a.voucher_no=b.no_voucher)j where trans_d' +
        'ate between '#39'2022-11-15'#39' and '#39'2022-11-15'#39' and kodesp='#39'2142'#39' and ' +
        '"position"='#39'K'#39')a LEFT JOIN t_credit_trx_real b on a.voucher_no=b' +
        '.voucher LEFT JOIN t_buy_pay c on a.voucher_no=c.voucher_no LEFT' +
        ' JOIN t_payment_detail_real d on a.voucher_no=d.voucher_no WHERE' +
        ' (b.voucher is not null) and (b.voucher is null)and (c.voucher_n' +
        'o is null))aa LEFT JOIN t_credit_trx_real b ON aa.voucher_no=b.v' +
        'oucher'
      'UNION ALL '
      
        'SELECT DISTINCT m.voucher_no voucher,m.trans_date as tanggal,n.a' +
        'ccount_code as kodesp,n.description1 deskripsi,m.to_ kepada,n.in' +
        'v_supp_no nofaktur,n.dpp,n.saldo,'#39#39' as nodo from (SELECT a.vouch' +
        'er_no,b.trans_date,a.to_ from t_cash_bank_expenditure a INNER JO' +
        'IN t_cash_bank_expenditure_det b on a.voucher_no=b.no_voucher) m' +
        ' ,t_credit_trx_real n where m.voucher_no=n.voucher and n.account' +
        '_code='#39'2142'#39' and description1<>'#39#39' and m.trans_date between '#39'2022' +
        '-11-15'#39' and '#39'2022-11-15'#39
      ''
      'UNION ALL '
      'SELECT   '#39#39' as voucher,b.receive_date,a.account_pph_code,'
      
        '(case when a.account_pph_code='#39'2130.02'#39' then '#39'Hutang Pajak - PPh' +
        '.Ps.23'#39' when a.account_pph_code='#39'2130.01'#39' then '#39'Hutang Pajak - P' +
        'Ph.Ps.21'#39' when a.account_pph_code='#39'2130.03'#39' then '#39'Hutang Pajak -' +
        ' PPh.Ps.25'#39' when a.account_pph_code='#39'2130.04'#39' then '#39'Hutang Pajak' +
        ' - PPh.Ps.4(2)'#39' when a.account_pph_code='#39'2130.07'#39' then '#39'Hutang P' +
        'ajak - PPh.Ps.15'#39' when a.account_pph_code='#39'2130.06'#39' then '#39'Hutang' +
        ' Pajak - PPh.Ps.22'#39' else a.account_pph_code end) as deskripsi,'
      
        'supplier_name as kepada,b.faktur_no,a.subtotalrp as dpp,a.pph_rp' +
        ' as pph,'#39#39' as nodo'
      ' from t_item_receive_det a '
      'left join t_item_receive b on a.receive_no=b.receive_no'
      'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code'
      'where '
      'b.receive_date >'#39'2024-01-01'#39' '
      'and b.receive_date BETWEEN '#39'2023-03-01'#39' and '#39'2023-03-31'#39' '
      
        'and a.account_pph_code='#39'2142'#39' and a.receive_no not in (select lp' +
        'b_no from t_buy_pay)')
    Left = 574
    Top = 8
  end
  object frxrekapkreditrx: TfrxReport
    Version = '2022.1.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 40907.381461944500000000
    ReportOptions.LastChange = 45932.629227418980000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'procedure MasterData1OnBeforePrint(Sender: TfrxComponent);'
      'begin              '
      'end;'
      ''
      'procedure MasterData1OnAfterPrint(Sender: TfrxComponent);'
      'begin    '
      'end;'
      ''
      'begin'
      ''
      'end.')
    Left = 454
    Top = 11
    Datasets = <
      item
        DataSet = frxDBDatasetPers
        DataSetName = 'frxDBDatasetPers'
      end
      item
        DataSet = frxrekdafkredittrx
        DataSetName = 'frxdbdafrekapkredittrx'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 215.900000000000000000
      PaperHeight = 330.000000000000000000
      PaperSize = 256
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 7.500000000000000000
      BottomMargin = 15.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 71.811070000000000000
        Top = 18.897650000000000000
        Width = 740.409927000000000000
        object Memo1: TfrxMemoView
          Align = baCenter
          AllowVectorExport = True
          Left = 5.874103500000000000
          Top = 3.779530000000000000
          Width = 728.661720000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[judul]')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Align = baCenter
          AllowVectorExport = True
          Left = 135.874103500000000000
          Top = 23.338590000000000000
          Width = 468.661720000000000000
          Height = 18.897650000000000000
          DataField = 'company_name'
          DataSet = frxDBDatasetPers
          DataSetName = 'frxDBDatasetPers'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetPers."company_name"]')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Align = baCenter
          AllowVectorExport = True
          Left = 135.874103500000000000
          Top = 47.574830000000000000
          Width = 468.661720000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Courier New'
          Font.Style = [fsBold, fsUnderline]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[periode]')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 49.133890000000000000
        Top = 257.008040000000000000
        Width = 740.409927000000000000
        OnAfterPrint = 'MasterData1OnAfterPrint'
        OnBeforePrint = 'MasterData1OnBeforePrint'
        DataSet = frxrekdafkredittrx
        DataSetName = 'frxdbdafrekapkredittrx'
        RowCount = 0
        Stretched = True
        object Memo20: TfrxMemoView
          AllowVectorExport = True
          Width = 26.456710000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[line#]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          AllowVectorExport = True
          Left = 28.031540000000000000
          Width = 64.252010000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'voucher_no'
          DataSet = frxrekdafkredittrx
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."voucher_no"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line5: TfrxLineView
          AllowVectorExport = True
          Left = 28.393700790000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo29: TfrxMemoView
          AllowVectorExport = True
          Left = 91.929190000000000000
          Width = 49.488250000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'tanggal'
          DataSet = frxrekdafkredittrx
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."tanggal"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line7: TfrxLineView
          AllowVectorExport = True
          Left = 92.669291340000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line24: TfrxLineView
          AllowVectorExport = True
          Left = 142.173228350000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line55: TfrxLineView
          AllowVectorExport = True
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line17: TfrxLineView
          AllowVectorExport = True
          Left = 565.401574800000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line18: TfrxLineView
          AllowVectorExport = True
          Left = 420.370078740000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo23: TfrxMemoView
          AllowVectorExport = True
          Left = 646.504330000000000000
          Width = 91.063080000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataSet = frxrekdafkredittrx
          DataSetName = 'frxdbdafrekapkredittrx'
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."saldo"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo24: TfrxMemoView
          AllowVectorExport = True
          Left = 227.614410000000000000
          Width = 193.110390000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'deskripsi'
          DataSet = frxrekdafkredittrx
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."deskripsi"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo8: TfrxMemoView
          AllowVectorExport = True
          Left = 422.118430000000000000
          Width = 140.196970000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'kepada'
          DataSet = frxrekdafkredittrx
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."kepada"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line2: TfrxLineView
          AllowVectorExport = True
          Left = 740.228820000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line13: TfrxLineView
          AllowVectorExport = True
          Left = 646.520100000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo22: TfrxMemoView
          AllowVectorExport = True
          Left = 565.897960000000000000
          Width = 78.614173230000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'dpp'
          DataSet = frxrekdafkredittrx
          DataSetName = 'frxdbdafrekapkredittrx'
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."dpp"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line22: TfrxLineView
          AllowVectorExport = True
          Left = 226.771653540000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo28: TfrxMemoView
          AllowVectorExport = True
          Left = 143.622140000000000000
          Width = 83.504020000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'nofaktur'
          DataSet = frxrekdafkredittrx
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."nofaktur"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line20: TfrxLineView
          AllowVectorExport = True
          Top = 49.133890000000000000
          Width = 740.409448818898000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
      object ColumnHeader1: TfrxColumnHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 36.236191180000000000
        Top = 113.385900000000000000
        Width = 740.409927000000000000
        object Shape1: TfrxShapeView
          AllowVectorExport = True
          Top = 1.220470000000000000
          Width = 739.968770000000000000
          Height = 34.015770000000000000
          Frame.Typ = []
        end
        object Memo2: TfrxMemoView
          AllowVectorExport = True
          Left = 0.889765000000000000
          Top = 5.000000000000000000
          Width = 22.677180000000000000
          Height = 26.456710000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'NO.')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line1: TfrxLineView
          AllowVectorExport = True
          Left = 28.574830000000000000
          Top = 1.220470000000000000
          Height = 34.015770000000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line6: TfrxLineView
          AllowVectorExport = True
          Left = 92.645950000000000000
          Top = 2.220470000000000000
          Height = 34.015721180000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo92: TfrxMemoView
          AllowVectorExport = True
          Left = 29.574830000000000000
          Top = 5.000000000000000000
          Width = 61.984251970000000000
          Height = 26.456710000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'NO. BK')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo5: TfrxMemoView
          AllowVectorExport = True
          Left = 93.401980000000000000
          Top = 3.220470000000000000
          Width = 47.606370000000000000
          Height = 27.354360000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'TANGGAL')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line10: TfrxLineView
          AllowVectorExport = True
          Left = 142.000000000000000000
          Top = 1.834570000000000000
          Height = 34.015770000000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 225.212740000000000000
          Top = 2.543290000000000000
          Width = 193.133880240000000000
          Height = 31.133890000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'DESKRIPSI')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line12: TfrxLineView
          AllowVectorExport = True
          Left = 420.346395670000000000
          Top = 1.440940000000000000
          Height = 34.015748030000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo16: TfrxMemoView
          AllowVectorExport = True
          Left = 650.134200000000000000
          Top = 3.000000000000000000
          Width = 86.929190000000000000
          Height = 31.133890000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'JUMLAH')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line4: TfrxLineView
          AllowVectorExport = True
          Left = 565.228346460000000000
          Top = 1.220470000000000000
          Height = 34.015748030000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo4: TfrxMemoView
          AllowVectorExport = True
          Left = 422.897960000000000000
          Top = 2.220470000000000000
          Width = 139.842610000000000000
          Height = 31.133890000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'NAMA KENA PPH')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line14: TfrxLineView
          AllowVectorExport = True
          Left = 646.520505200000000000
          Top = 1.000000000000000000
          Height = 34.015748031496100000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo19: TfrxMemoView
          AllowVectorExport = True
          Left = 565.897960000000000000
          Top = 1.779530000000000000
          Width = 79.370076300000000000
          Height = 31.133890000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'DPP')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line21: TfrxLineView
          AllowVectorExport = True
          Left = 226.771800000000000000
          Top = 1.889763780000000000
          Height = 34.015748030000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo27: TfrxMemoView
          AllowVectorExport = True
          Left = 145.181200000000000000
          Top = 3.779530000000000000
          Width = 74.063080000000000000
          Height = 27.354360000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'NO FAKTUR/ INVOICE')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 26.456710000000000000
        Top = 411.968770000000000000
        Width = 740.409927000000000000
        object Shape2: TfrxShapeView
          AllowVectorExport = True
          Width = 740.031496060000000000
          Height = 26.456710000000000000
          Frame.Typ = []
        end
        object Memo3: TfrxMemoView
          AllowVectorExport = True
          Left = 429.811380000000000000
          Top = 0.519480000000000000
          Width = 109.606370000000000000
          Height = 22.677180000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            'TOTAL :')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line3: TfrxLineView
          AllowVectorExport = True
          Left = 645.921259842520000000
          Height = 26.456692910000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo7: TfrxMemoView
          AllowVectorExport = True
          Left = 647.165740000000000000
          Top = 2.000000000000000000
          Width = 91.063080000000000000
          Height = 22.677180000000000000
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[sum(<frxdbdafrekapkredittrx."saldo">)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo25: TfrxMemoView
          AllowVectorExport = True
          Left = 572.488560000000000000
          Width = 72.165430000000000000
          Height = 26.456710000000000000
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[sum(<frxdbdafrekapkredittrx."dpp">)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line19: TfrxLineView
          AllowVectorExport = True
          Left = 565.417322834646000000
          Height = 26.456692910000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
      end
      object GroupFooter1: TfrxGroupFooter
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677165350000000000
        Top = 328.819110000000000000
        Width = 740.409927000000000000
        object Memo31: TfrxMemoView
          AllowVectorExport = True
          Width = 740.031496060000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo26: TfrxMemoView
          AllowVectorExport = True
          Left = 645.819316670000000000
          Top = 0.740156670000000000
          Width = 90.708661420000000000
          Height = 21.417336670000000000
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxdbdafrekapkredittrx."saldo">)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo32: TfrxMemoView
          AllowVectorExport = True
          Left = 465.354631250000000000
          Top = 3.055117500000000000
          Width = 96.378015000000000000
          Height = 16.535443750000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'SUB TOTAL  :                                      ')
          ParentFont = False
        end
        object Memo33: TfrxMemoView
          AllowVectorExport = True
          Left = 565.417322830000000000
          Width = 79.370071420000000000
          Height = 21.417336670000000000
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxdbdafrekapkredittrx."dpp">)]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object GroupHeader1: TfrxGroupHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 211.653680000000000000
        Visible = False
        Width = 740.409927000000000000
        Condition = 'frxdbdafrekapkredittrx."voucher"'
        object Memo30: TfrxMemoView
          AllowVectorExport = True
          Top = 1.000000000000000000
          Width = 64.252010000000000000
          Height = 15.118088270000000000
          Visible = False
          StretchMode = smMaxHeight
          DataField = 'voucher'
          DataSet = frxrekdafkredittrx
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."voucher"]')
          ParentFont = False
        end
      end
    end
  end
  object frxrekdafkredittrx: TfrxDBDataset
    UserName = 'frxdbdafrekapkredittrx'
    CloseDataSource = False
    FieldAliases.Strings = (
      'voucher_no=voucher_no'
      'tanggal=tanggal'
      'kodesp=kodesp'
      'deskripsi=deskripsi'
      'kepada=kepada'
      'nofaktur=nofaktur'
      'dpp=dpp'
      'pph=pph'
      'nodo=nodo')
    DataSet = unirekapkredittrx
    BCDToCurrency = True
    DataSetOptions = []
    Left = 526
    Top = 14
  end
  object frxrekapkreditrx2: TfrxReport
    Version = '2022.1.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 40907.381461944500000000
    ReportOptions.LastChange = 44890.605522199100000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'procedure MasterData1OnBeforePrint(Sender: TfrxComponent);'
      'begin              '
      'end;'
      ''
      'procedure MasterData1OnAfterPrint(Sender: TfrxComponent);'
      'begin    '
      'end;'
      ''
      'begin'
      ''
      'end.')
    Left = 198
    Top = 8
    Datasets = <
      item
        DataSet = frxDBDatasetPers
        DataSetName = 'frxDBDatasetPers'
      end
      item
        DataSet = frxrekdafkredittrx2
        DataSetName = 'frxdbdafrekapkredittrx'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 215.900000000000000000
      PaperHeight = 330.000000000000000000
      PaperSize = 256
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 7.500000000000000000
      BottomMargin = 15.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 71.811070000000000000
        Top = 18.897650000000000000
        Width = 740.409927000000000000
        object Memo1: TfrxMemoView
          Align = baCenter
          AllowVectorExport = True
          Left = 5.874103500000000000
          Top = 3.779530000000000000
          Width = 728.661720000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[judul]')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          Align = baCenter
          AllowVectorExport = True
          Left = 135.874103500000000000
          Top = 23.338590000000000000
          Width = 468.661720000000000000
          Height = 18.897650000000000000
          DataField = 'company_name'
          DataSet = frxDBDatasetPers
          DataSetName = 'frxDBDatasetPers'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetPers."company_name"]')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Align = baCenter
          AllowVectorExport = True
          Left = 135.874103500000000000
          Top = 47.574830000000000000
          Width = 468.661720000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Courier New'
          Font.Style = [fsBold, fsUnderline]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[periode]')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 49.133890000000000000
        Top = 257.008040000000000000
        Width = 740.409927000000000000
        OnAfterPrint = 'MasterData1OnAfterPrint'
        OnBeforePrint = 'MasterData1OnBeforePrint'
        DataSet = frxrekdafkredittrx2
        DataSetName = 'frxdbdafrekapkredittrx'
        RowCount = 0
        Stretched = True
        object Memo20: TfrxMemoView
          AllowVectorExport = True
          Width = 26.456710000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[line#]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          AllowVectorExport = True
          Left = 28.031540000000000000
          Width = 64.252010000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'voucher_no'
          DataSet = frxrekdafkredittrx2
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."voucher_no"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line5: TfrxLineView
          AllowVectorExport = True
          Left = 28.393700790000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo29: TfrxMemoView
          AllowVectorExport = True
          Left = 91.929190000000000000
          Width = 49.488250000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'tanggal'
          DataSet = frxrekdafkredittrx2
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."tanggal"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line7: TfrxLineView
          AllowVectorExport = True
          Left = 92.669291340000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line24: TfrxLineView
          AllowVectorExport = True
          Left = 142.173228350000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line55: TfrxLineView
          AllowVectorExport = True
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line17: TfrxLineView
          AllowVectorExport = True
          Left = 565.401574800000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line18: TfrxLineView
          AllowVectorExport = True
          Left = 420.370078740000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo23: TfrxMemoView
          AllowVectorExport = True
          Left = 646.504330000000000000
          Width = 91.063080000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'pph'
          DataSet = frxrekdafkredittrx2
          DataSetName = 'frxdbdafrekapkredittrx'
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."pph"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo24: TfrxMemoView
          AllowVectorExport = True
          Left = 227.614410000000000000
          Width = 193.110390000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'deskripsi'
          DataSet = frxrekdafkredittrx2
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."deskripsi"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo8: TfrxMemoView
          AllowVectorExport = True
          Left = 422.118430000000000000
          Width = 140.196970000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'kepada'
          DataSet = frxrekdafkredittrx2
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."kepada"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line2: TfrxLineView
          AllowVectorExport = True
          Left = 740.228820000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line13: TfrxLineView
          AllowVectorExport = True
          Left = 646.520100000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo22: TfrxMemoView
          AllowVectorExport = True
          Left = 565.897960000000000000
          Width = 78.614173230000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataField = 'dpp'
          DataSet = frxrekdafkredittrx2
          DataSetName = 'frxdbdafrekapkredittrx'
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."dpp"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line22: TfrxLineView
          AllowVectorExport = True
          Left = 226.771653540000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo28: TfrxMemoView
          AllowVectorExport = True
          Left = 143.622140000000000000
          Width = 83.504020000000000000
          Height = 49.133858270000000000
          StretchMode = smMaxHeight
          DataSet = frxrekdafkredittrx2
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."nofaktur"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line20: TfrxLineView
          AllowVectorExport = True
          Top = 49.133890000000000000
          Width = 740.409448818898000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
      object ColumnHeader1: TfrxColumnHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 36.236191180000000000
        Top = 113.385900000000000000
        Width = 740.409927000000000000
        object Shape1: TfrxShapeView
          AllowVectorExport = True
          Top = 1.220470000000000000
          Width = 739.968770000000000000
          Height = 34.015770000000000000
          Frame.Typ = []
        end
        object Memo2: TfrxMemoView
          AllowVectorExport = True
          Left = 0.889765000000000000
          Top = 5.000000000000000000
          Width = 22.677180000000000000
          Height = 26.456710000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'NO.')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line1: TfrxLineView
          AllowVectorExport = True
          Left = 28.574830000000000000
          Top = 1.220470000000000000
          Height = 34.015770000000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line6: TfrxLineView
          AllowVectorExport = True
          Left = 92.645950000000000000
          Top = 2.220470000000000000
          Height = 34.015721180000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo92: TfrxMemoView
          AllowVectorExport = True
          Left = 29.574830000000000000
          Top = 5.000000000000000000
          Width = 61.984251970000000000
          Height = 26.456710000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'NO. BK')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo5: TfrxMemoView
          AllowVectorExport = True
          Left = 93.401980000000000000
          Top = 3.220470000000000000
          Width = 47.606370000000000000
          Height = 27.354360000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'TANGGAL')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line10: TfrxLineView
          AllowVectorExport = True
          Left = 142.000000000000000000
          Top = 1.834570000000000000
          Height = 34.015770000000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 225.212740000000000000
          Top = 2.543290000000000000
          Width = 193.133880240000000000
          Height = 31.133890000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'DESKRIPSI')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line12: TfrxLineView
          AllowVectorExport = True
          Left = 420.346395670000000000
          Top = 1.440940000000000000
          Height = 34.015748030000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo16: TfrxMemoView
          AllowVectorExport = True
          Left = 650.134200000000000000
          Top = 3.000000000000000000
          Width = 86.929190000000000000
          Height = 31.133890000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'JUMLAH')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line4: TfrxLineView
          AllowVectorExport = True
          Left = 565.228346460000000000
          Top = 1.220470000000000000
          Height = 34.015748030000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo4: TfrxMemoView
          AllowVectorExport = True
          Left = 422.897960000000000000
          Top = 2.220470000000000000
          Width = 139.842610000000000000
          Height = 31.133890000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'NAMA KENA PPH')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line14: TfrxLineView
          AllowVectorExport = True
          Left = 646.520505200000000000
          Top = 1.000000000000000000
          Height = 34.015748031496100000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo19: TfrxMemoView
          AllowVectorExport = True
          Left = 565.897960000000000000
          Top = 1.779530000000000000
          Width = 79.370076300000000000
          Height = 31.133890000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'DPP')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line21: TfrxLineView
          AllowVectorExport = True
          Left = 226.771800000000000000
          Top = 1.889763780000000000
          Height = 34.015748030000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo27: TfrxMemoView
          AllowVectorExport = True
          Left = 145.181200000000000000
          Top = 3.779530000000000000
          Width = 74.063080000000000000
          Height = 27.354360000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Courier New'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'NO FAKTUR/ INVOICE')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 26.456710000000000000
        Top = 411.968770000000000000
        Width = 740.409927000000000000
        object Shape2: TfrxShapeView
          AllowVectorExport = True
          Width = 740.031496060000000000
          Height = 26.456710000000000000
          Frame.Typ = []
        end
        object Memo3: TfrxMemoView
          AllowVectorExport = True
          Left = 429.811380000000000000
          Top = 0.519480000000000000
          Width = 109.606370000000000000
          Height = 22.677180000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            'TOTAL :')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line3: TfrxLineView
          AllowVectorExport = True
          Left = 645.921259842520000000
          Height = 26.456692910000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Memo7: TfrxMemoView
          AllowVectorExport = True
          Left = 647.165740000000000000
          Top = 2.000000000000000000
          Width = 91.063080000000000000
          Height = 22.677180000000000000
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[sum(<frxdbdafrekapkredittrx."pph">)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo25: TfrxMemoView
          AllowVectorExport = True
          Left = 572.488560000000000000
          Width = 72.165430000000000000
          Height = 26.456710000000000000
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[sum(<frxdbdafrekapkredittrx."dpp">)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line19: TfrxLineView
          AllowVectorExport = True
          Left = 565.417322834646000000
          Height = 26.456692910000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
      end
      object GroupFooter1: TfrxGroupFooter
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677165350000000000
        Top = 328.819110000000000000
        Width = 740.409927000000000000
        object Memo31: TfrxMemoView
          AllowVectorExport = True
          Width = 740.031496060000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo26: TfrxMemoView
          AllowVectorExport = True
          Left = 645.819316670000000000
          Top = 0.740156670000000000
          Width = 90.708661420000000000
          Height = 21.417336670000000000
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxdbdafrekapkredittrx."pph">)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo32: TfrxMemoView
          AllowVectorExport = True
          Left = 465.354631250000000000
          Top = 3.055117500000000000
          Width = 96.378015000000000000
          Height = 16.535443750000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'SUB TOTAL  :                                      ')
          ParentFont = False
        end
        object Memo33: TfrxMemoView
          AllowVectorExport = True
          Left = 565.417322830000000000
          Width = 79.370071420000000000
          Height = 21.417336670000000000
          DisplayFormat.FormatStr = '%0.n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxdbdafrekapkredittrx."dpp">)]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object GroupHeader1: TfrxGroupHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 211.653680000000000000
        Visible = False
        Width = 740.409927000000000000
        Condition = 'frxdbdafrekapkredittrx."kepada"'
        object Memo30: TfrxMemoView
          AllowVectorExport = True
          Top = 1.000000000000000000
          Width = 64.252010000000000000
          Height = 15.118088270000000000
          Visible = False
          StretchMode = smMaxHeight
          DataField = 'voucher'
          DataSet = frxrekdafkredittrx2
          DataSetName = 'frxdbdafrekapkredittrx'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxdbdafrekapkredittrx."voucher"]')
          ParentFont = False
        end
      end
    end
  end
  object unirekapkredittrx2: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      ''
      
        'SELECT  a.voucher_no,a.trans_date tanggal,a.kodesp,x.deskripsi,k' +
        'epada,a.inv_supp_no as nofaktur ,y.dpp as dpp,a.pph as pph,'#39#39' as' +
        ' nodo FROM '
      '(SELECT DISTINCT a.*,b.inv_supp_no,c.saldo as pph FROM '
      
        '(SELECT DISTINCT voucher_no,trans_date,to_ kepada,code_account k' +
        'odesp FROM '
      
        '(SELECT a.voucher_no,b.trans_date,a.to_ ,b.code_account,b."posit' +
        'ion" from t_cash_bank_expenditure a INNER JOIN t_cash_bank_expen' +
        'diture_det b on a.voucher_no=b.no_voucher)r where trans_date bet' +
        'ween '#39'2022-11-15'#39' and '#39'2022-11-15'#39' and code_account='#39'2142'#39' and "' +
        'position"='#39'K'#39')a  LEFT JOIN t_buy_pay b on a.voucher_no=b.voucher' +
        '_no LEFT JOIN t_credit_trx_real c on a.voucher_no=c.voucher and ' +
        'b.inv_supp_no=c.inv_supp_no WHERE (c.voucher is not null ) and (' +
        'b.inv_supp_no is not null)) a LEFT JOIN  t_item_receive f ON f.f' +
        'aktur_no=a.inv_supp_no LEFT JOIN (SELECT  a.receive_no,string_ag' +
        'g (b.item_name, '#39', '#39' ) AS deskripsi FROM  t_item_receive_det a L' +
        'EFT JOIN t_item_stock b ON a.item_stock_code::text = b.item_stoc' +
        'k_code::text GROUP BY a.receive_no ) x On x.receive_no=f.receive' +
        '_no LEFT JOIN (SELECT receive_no,sum(subtotal) as dpp FROM t_ite' +
        'm_receive_det GROUP BY  receive_no) y on f.receive_no=y.receive_' +
        'no '
      'UNION ALL '
      
        'SELECT aa.voucher_no,aa.trans_date tanggal,aa.kodesp,b.descripti' +
        'on1,aa.kepada,b.inv_supp_no as nofaktur ,b.dpp as dpp,b.saldo as' +
        ' pph,'#39#39' as nodo  FROM (SELECT a.*,c.inv_supp_no,b.inv_supp_no FR' +
        'OM '
      
        '(SELECT distinct voucher_no,trans_date,kodesp,kepada FROM (SELEC' +
        'T a.voucher_no,b.trans_date,b.code_account kodesp,a.to_ kepada,b' +
        '."position" from t_cash_bank_expenditure a INNER JOIN t_cash_ban' +
        'k_expenditure_det b on a.voucher_no=b.no_voucher)j where trans_d' +
        'ate between '#39'2022-11-15'#39' and '#39'2022-11-15'#39' and kodesp='#39'2142'#39' and ' +
        '"position"='#39'K'#39')a LEFT JOIN t_credit_trx_real b on a.voucher_no=b' +
        '.voucher LEFT JOIN t_buy_pay c on a.voucher_no=c.voucher_no LEFT' +
        ' JOIN t_payment_detail_real d on a.voucher_no=d.voucher_no WHERE' +
        ' (b.voucher is not null) and (b.voucher is null)and (c.voucher_n' +
        'o is null))aa LEFT JOIN t_credit_trx_real b ON aa.voucher_no=b.v' +
        'oucher'
      'UNION ALL '
      
        'SELECT DISTINCT m.voucher_no voucher,m.trans_date as tanggal,n.a' +
        'ccount_code as kodesp,n.description1 deskripsi,m.to_ kepada,n.in' +
        'v_supp_no nofaktur,n.dpp,n.saldo,'#39#39' as nodo from (SELECT a.vouch' +
        'er_no,b.trans_date,a.to_ from t_cash_bank_expenditure a INNER JO' +
        'IN t_cash_bank_expenditure_det b on a.voucher_no=b.no_voucher) m' +
        ' ,t_credit_trx_real n where m.voucher_no=n.voucher and n.account' +
        '_code='#39'2142'#39' and description1<>'#39#39' and m.trans_date between '#39'2022' +
        '-11-15'#39' and '#39'2022-11-15'#39
      ''
      'UNION ALL '
      'SELECT   '#39#39' as voucher,b.receive_date,a.account_pph_code,'
      
        '(case when a.account_pph_code='#39'2130.02'#39' then '#39'Hutang Pajak - PPh' +
        '.Ps.23'#39' when a.account_pph_code='#39'2130.01'#39' then '#39'Hutang Pajak - P' +
        'Ph.Ps.21'#39' when a.account_pph_code='#39'2130.03'#39' then '#39'Hutang Pajak -' +
        ' PPh.Ps.25'#39' when a.account_pph_code='#39'2130.04'#39' then '#39'Hutang Pajak' +
        ' - PPh.Ps.4(2)'#39' when a.account_pph_code='#39'2130.07'#39' then '#39'Hutang P' +
        'ajak - PPh.Ps.15'#39' when a.account_pph_code='#39'2130.06'#39' then '#39'Hutang' +
        ' Pajak - PPh.Ps.22'#39' else a.account_pph_code end) as deskripsi,'
      
        'supplier_name as kepada,b.faktur_no,a.subtotalrp as dpp,a.pph_rp' +
        ' as pph,'#39#39' as nodo'
      ' from t_item_receive_det a '
      'left join t_item_receive b on a.receive_no=b.receive_no'
      'LEFT JOIN t_supplier d on b.supplier_code=d.supplier_code'
      'where '
      'b.receive_date >'#39'2024-01-01'#39' '
      'and b.receive_date BETWEEN '#39'2023-03-01'#39' and '#39'2023-03-31'#39' '
      
        'and a.account_pph_code='#39'2142'#39' and a.receive_no not in (select lp' +
        'b_no from t_buy_pay)')
    Left = 368
    Top = 8
  end
  object frxrekdafkredittrx2: TfrxDBDataset
    UserName = 'frxdbdafrekapkredittrx'
    CloseDataSource = False
    FieldAliases.Strings = (
      'voucher_no=voucher_no'
      'tanggal=tanggal'
      'kodesp=kodesp'
      'deskripsi=deskripsi'
      'kepada=kepada'
      'nofaktur=nofaktur'
      'dpp=dpp'
      'pph=pph'
      'nodo=nodo')
    DataSet = unirekapkredittrx2
    BCDToCurrency = True
    DataSetOptions = []
    Left = 270
    Top = 8
  end
end
