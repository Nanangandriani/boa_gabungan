unit ULaporanHarianSisaNotaPerTP;
...
uses
  ...
  cxCalendar, cxRadioGroup, cxCheckGroup, MemTableDataEh, Data.DB, MemTableEh,
  MemDS, DBAccess, Uni, frxClass, frxDBSet, dxBarExtItems, cxBarEditItem, dxBar,
  ...
  Vcl.StdCtrls, RzLabel, frxExportBaseDialog, frxExportPDF;

type
  TFLaporanHarianSisaNotaPerTP = class(TForm)
    RzLabel1: TRzLabel;
    dxRibbon1: TdxRibbon;
    ...
    DsLaporanHarianSisaNotaPerTP: TDataSource;
    Report: TfrxReport;
    frxDBDatasetLapHarianSisaNotaPerTp: TfrxDBDataset;
    QLaporanHarianSisaNotaPerTP: TUniQuery;
    Qreport: TUniQuery;
...
//var
//  FLaporanHarianSisaNotaPerTP: TFLaporanHarianSisaNotaPerTP;
...
var
  laporanhariansisanotapertp : TFLaporanHarianSisaNotaPerTP;

function FLaporanHarianSisaNotaPerTP: TFLaporanHarianSisaNotaPerTP;
begin
  if laporanhariansisanotapertp <> nil then
    FLaporanHarianSisaNotaPerTP:= laporanhariansisanotapertp
  else
    Application.CreateForm(TFLaporanHarianSisaNotaPerTP, Result);
end;
...
begin
  ...
  begin
    ...
    begin
      strReportName:='rpt_laporanhariansisanotapertp';
...
begin
  ...
  begin
    with QLaporanHarianSisaNotaPerTP do
    begin
      close;
      sql.Clear;
      sql.Text:='SELECT * FROM get_lhsn_sum('+QuotedStr(FormatDateTime('yyyy-mm-dd',dtTanggal1.Date))+','+QuotedStr(FormatDateTime('yyyy-mm-dd',dtTanggal2.Date))+','+QuotedStr(cbKaresidenan.EditValue)+',NULL,FALSE,FALSE)';
...
begin
  laporanhariansisanotapertp:= Self;
end;

procedure TFLaporanHarianSisaNotaPerTP.FormDestroy(Sender: TObject);
begin
  laporanhariansisanotapertp:=nil;
end;
...
initialization
  RegisterClass(TFLaporanHarianSisaNotaPerTP);
