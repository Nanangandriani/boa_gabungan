...
begin
   ...
   query1:=//'select tanggal,kodesup,nasup,no_inv,nofakturpajak,sj_no,tglfaktur,tgltempo,valas,valas_value,jum_dolar,ppn_rp,jumlah-nilai_um as jumlah,npph,''''::text as akun_pph,plan_stat,status,approval_status '+
           'select tanggal,kodesup,nasup,no_inv,nofakturpajak,sj_no,tglfaktur,tgltempo,valas,valas_value,jum_dolar,ppn_rp,jumlah-nilai_um as jumlah,npph,'''' as akun_pph,plan_stat,status,approval_status '+
           'from '+
           ...
           //'(case when npph is null then 0 else npph end)npph,''''::text as akun_pph,plan_stat,status,approval_status '+
           '(case when npph is null then 0 else npph end)npph,'''' as akun_pph,plan_stat,status,approval_status '+
           'FROM '+
           ...
           '(case when do1.harga is null then 0 else do1.harga end)harga_do1,status,approval_status '+
           'FROM '+
           ...
           '(case when sj_status=1 and fk_status=1 and invoice_status=1 then 1 else 0 end)status,0 as ppnrp,id,approval_status  from purchase.t_purchase_invoice where (faktur_date + due_date) between '+QuotedStr(formatdatetime('yyyy-mm-dd',DtMulai.Date))+' and '+QuotedStr(formatdatetime('yyyy-mm-dd',DtSelesai.Date))+' '+
           ...
           //' ''''::character varying as nosj,''Rp''::character varying as valas,0 as valas_value,  0 as  due_date,false as stat_rencana,1 as status,0 as ppnrp,0 as id,0 as status_approval '+
           ' '''' as nosj,''Rp'' as valas,0 as valas_value,  0 as  due_date,false as stat_rencana,1 as status,0 as ppnrp,0 as id,0 as status_approval '+
           ...
           ')a '+
           'left join '+
           '(select a.trans_no,b.supplier_code,b.valas,sum(a.grandtotal)as jumlah,(case when b.valas=''USD'' then sum(a.subtotalrp) else sum(a.grandtotal) end)as hutang,a.subtotal,a.ppn_rp ,sum(a.pph_rp)as npph '+
           'from purchase.t_purchase_invoice_det a,purchase.t_purchase_invoice b  where a.trans_no=b.trans_no group by a.trans_no,b.supplier_code,b.valas,a.subtotal,a.ppn_rp order by trans_no,supplier_code)b on a.trans_no=b.trans_no and a.kodesup=b.supplier_code '+
           'left join '+
    ...
    begin
      ...
      if length(cbsupp.Text)<>0 then
       ...
       sql.Add('ORDER BY tglfaktur,nofakturpajak ASC ');
      open;
    end;
    qdaftarhutang.Close;
    qdaftarhutang.Open;
...
begin
  ...
  {with frmrencanalunashutang.qtmplunas do
  begin
    close;
    sql.clear;
    sql.add('select * from tbl_tmp_det_lunas_hutang where username=:usr');
