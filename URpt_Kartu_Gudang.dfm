object FRpt_Kartu_Gudang: TFRpt_Kartu_Gudang
  Left = 0
  Top = 0
  Caption = 'Form Laporan Kartu Gudang'
  ClientHeight = 454
  ClientWidth = 1092
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  Position = poMainFormCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  TextHeight = 13
  object Label2: TLabel
    Left = 43
    Top = 286
    Width = 26
    Height = 13
    Caption = 'Bulan'
    Visible = False
  end
  object Label3: TLabel
    Left = 119
    Top = 286
    Width = 4
    Height = 13
    Caption = ':'
    Visible = False
  end
  object Label5: TLabel
    Left = 43
    Top = 315
    Width = 37
    Height = 13
    Caption = 'Gudang'
    Visible = False
  end
  object Label6: TLabel
    Left = 119
    Top = 315
    Width = 4
    Height = 13
    Caption = ':'
    Visible = False
  end
  object Label7: TLabel
    Left = 43
    Top = 229
    Width = 68
    Height = 13
    Caption = 'Nama Supplier'
    Visible = False
  end
  object Label8: TLabel
    Left = 119
    Top = 229
    Width = 4
    Height = 13
    Caption = ':'
    Visible = False
  end
  object Label1: TLabel
    Left = 119
    Top = 257
    Width = 4
    Height = 13
    Caption = ':'
    Visible = False
  end
  object Label4: TLabel
    Left = 43
    Top = 257
    Width = 64
    Height = 13
    Caption = 'Nama Barang'
    Visible = False
  end
  object CbBulan: TRzComboBox
    Left = 133
    Top = 283
    Width = 142
    Height = 21
    TabOrder = 0
    Visible = False
    OnChange = CbBulanChange
    Items.Strings = (
      ''
      'January'
      'February'
      'Maret'
      'April'
      'Mey'
      'Juni'
      'July'
      'Agustus'
      'September'
      'Oktober'
      'November'
      'Desember')
  end
  object CbGudang: TRzComboBox
    Left = 133
    Top = 312
    Width = 239
    Height = 21
    TabOrder = 1
    Visible = False
    Items.Strings = (
      'Bongkar')
  end
  object EdTahun: TSpinEdit
    Left = 287
    Top = 283
    Width = 85
    Height = 22
    MaxValue = 10000
    MinValue = 0
    TabOrder = 2
    Value = 2020
    Visible = False
    OnChange = EdTahunChange
  end
  object Panel1: TPanel
    Left = 0
    Top = 413
    Width = 1092
    Height = 41
    Align = alBottom
    TabOrder = 3
    OnClick = Panel1Click
    ExplicitTop = 404
    ExplicitWidth = 1086
    object BBatal: TRzBitBtn
      Left = 1016
      Top = 1
      Height = 39
      Align = alRight
      Caption = 'Batal'
      TabOrder = 0
      OnClick = BBatalClick
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 1010
    end
    object BPrint: TRzBitBtn
      Left = 937
      Top = 1
      Width = 79
      Height = 39
      Align = alRight
      Caption = 'Print'
      TabOrder = 1
      Visible = False
      OnClick = BPrintClick
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 931
    end
    object RzBitBtn1: TRzBitBtn
      Left = 858
      Top = 1
      Width = 79
      Height = 39
      Align = alRight
      Caption = 'Print'
      TabOrder = 2
      OnClick = RzBitBtn1Click
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 852
    end
  end
  object EdKd_supp: TEdit
    Left = 133
    Top = 226
    Width = 73
    Height = 21
    CharCase = ecUpperCase
    Color = clInfoBk
    ReadOnly = True
    TabOrder = 4
    Visible = False
  end
  object EdNm_supp: TRzButtonEdit
    Left = 210
    Top = 226
    Width = 213
    Height = 21
    Text = ''
    CharCase = ecUpperCase
    TabOrder = 5
    Visible = False
    AltBtnNumGlyphs = 1
    ButtonNumGlyphs = 1
    OnButtonClick = EdNm_suppButtonClick
  end
  object Edkd: TEdit
    Left = 133
    Top = 254
    Width = 73
    Height = 21
    CharCase = ecUpperCase
    Color = clInfoBk
    ReadOnly = True
    TabOrder = 6
    Visible = False
  end
  object EdNm: TRzButtonEdit
    Left = 210
    Top = 254
    Width = 213
    Height = 21
    Text = ''
    TabOrder = 7
    Visible = False
    AltBtnNumGlyphs = 1
    ButtonNumGlyphs = 1
    OnButtonClick = EdNmButtonClick
  end
  object dxRibbon1: TdxRibbon
    Left = 0
    Top = 0
    Width = 1092
    Height = 127
    BarManager = dxBarManager1
    Style = rs2010
    ColorSchemeName = 'Blue'
    Contexts = <>
    TabOrder = 8
    TabStop = False
    object dxRibbon1Tab1: TdxRibbonTab
      Active = True
      Caption = 'Home'
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar1'
        end>
      Index = 0
    end
  end
  object DBGridEh1: TDBGridEh
    Left = 0
    Top = 127
    Width = 1092
    Height = 286
    Align = alClient
    AutoFitColWidths = True
    DataSource = DsKartu_Gudang
    DynProps = <>
    TabOrder = 9
    Columns = <
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'kd_barang'
        Footers = <>
        Title.Caption = 'Kode Barang'
        Width = 150
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'item_name'
        Footers = <>
        Title.Caption = 'Nama Barang'
        Width = 300
      end
      item
        CellButtons = <>
        DisplayFormat = '0.00#,##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'saldo_awal_periode'
        Footers = <>
        Title.Caption = 'Saldo Awal Periode'
        Width = 100
        WordWrap = True
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'satuan'
        Footers = <>
        Title.Caption = 'Satuan'
        Width = 100
      end>
    object RowDetailData: TRowDetailPanelControlEh
    end
  end
  object RptKartu_Gudang: TfrxReport
    Version = '2022.1.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 44211.454024247700000000
    ReportOptions.LastChange = 44726.600623449100000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 8
    Top = 344
    Datasets = <
      item
        DataSet = DbKartu_gudang
        DataSetName = 'DBKartu_Gudang'
      end
      item
        DataSet = frxDBDataset1
        DataSetName = 'frxDBDataset1'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 215.900000000000000000
      PaperHeight = 279.400000000000000000
      PaperSize = 1
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 5.000000000000000000
      BottomMargin = 5.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 117.165354330000000000
        Top = 18.897650000000000000
        Width = 740.409927000000000000
        object Memo1: TfrxMemoView
          Align = baCenter
          AllowVectorExport = True
          Left = 223.960838500000000000
          Top = 1.102350000000000000
          Width = 292.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'KARTU GUDANG')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo2: TfrxMemoView
          AllowVectorExport = True
          Left = 46.795275590000000000
          Top = 22.677165350000000000
          Width = 94.488188980000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Nama Barang')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo3: TfrxMemoView
          AllowVectorExport = True
          Left = 46.795275590000000000
          Top = 62.472440940000000000
          Width = 94.488188980000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Satuan ')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo4: TfrxMemoView
          AllowVectorExport = True
          Left = 46.795275590000000000
          Top = 42.708661420000000000
          Width = 94.488188980000000000
          Height = 18.897637800000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Supplier')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo5: TfrxMemoView
          AllowVectorExport = True
          Top = 86.929133860000000000
          Width = 75.590551180000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Tanggal')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo6: TfrxMemoView
          AllowVectorExport = True
          Left = 75.590551180000000000
          Top = 86.929133860000000000
          Width = 151.181102360000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Kode Stok')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo8: TfrxMemoView
          AllowVectorExport = True
          Left = 226.771653540000000000
          Top = 86.929133860000000000
          Width = 113.385826771654000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Terima')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo9: TfrxMemoView
          AllowVectorExport = True
          Left = 340.157480310000000000
          Top = 86.929133860000000000
          Width = 113.385826770000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Keluar')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo10: TfrxMemoView
          AllowVectorExport = True
          Left = 453.543307086614000000
          Top = 86.929133860000000000
          Width = 113.385826771654000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Sisa')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo12: TfrxMemoView
          AllowVectorExport = True
          Left = 566.929133858268000000
          Top = 86.929133860000000000
          Width = 94.488250000000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Paraf')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          AllowVectorExport = True
          Left = 661.417322834646000000
          Top = 86.929133860000000000
          Width = 79.370078740157500000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            'Keterangan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo22: TfrxMemoView
          AllowVectorExport = True
          Left = 144.469696670000000000
          Top = 22.677165350000000000
          Width = 245.669291340000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            ':  [frxDBDataset1."nm_material"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo23: TfrxMemoView
          Description = ':'
          AllowVectorExport = True
          Left = 145.136363340000000000
          Top = 42.708661420000000000
          Width = 245.669291340000000000
          Height = 18.897637800000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            ':  [frxDBDataset1."nm_supplier"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo24: TfrxMemoView
          Description = ':'
          AllowVectorExport = True
          Left = 145.097201830000000000
          Top = 62.472440940000000000
          Width = 245.669291340000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            ':  [DBKartu_Gudang."satuan"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo33: TfrxMemoView
          AllowVectorExport = True
          Left = 395.409710000000000000
          Top = 22.677165350000000000
          Width = 91.821583340000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Gudang')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo35: TfrxMemoView
          AllowVectorExport = True
          Left = 396.037215160000000000
          Top = 42.708661420000000000
          Width = 91.154916670000000000
          Height = 18.897637800000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Bulan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo36: TfrxMemoView
          AllowVectorExport = True
          Left = 489.897637800000000000
          Top = 22.677165350000000000
          Width = 226.771653540000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            ':  [frxDBDataset1."gudang"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo38: TfrxMemoView
          Description = ':'
          AllowVectorExport = True
          Left = 500.370548500000000000
          Top = 22.677180000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            ':')
          ParentFont = False
          VAlign = vaCenter
        end
        object MBln: TfrxMemoView
          Description = ':'
          AllowVectorExport = True
          Left = 489.897637800000000000
          Top = 42.708661420000000000
          Width = 226.771653540000000000
          Height = 18.897637800000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            ':')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677165350000000000
        Top = 196.535560000000000000
        Width = 740.409927000000000000
        DataSet = frxDBDataset1
        DataSetName = 'frxDBDataset1'
        PrintIfDetailEmpty = True
        RowCount = 0
        Stretched = True
        object Memo30: TfrxMemoView
          AllowVectorExport = True
          Left = 552.133858270000000000
          Top = 1.000000000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Width = 75.590551180000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          DisplayFormat.FormatStr = 'dd mmm yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo25: TfrxMemoView
          AllowVectorExport = True
          Left = 75.590551180000000000
          Width = 151.181102360000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataField = 'kd_stok'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          Memo.UTF8W = (
            '[frxDBDataset1."kd_stok"]')
          ParentFont = False
          VAlign = vaCenter
          Formats = <
            item
            end
            item
            end>
        end
        object Memo27: TfrxMemoView
          AllowVectorExport = True
          Left = 226.771653540000000000
          Width = 113.385826770000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo28: TfrxMemoView
          AllowVectorExport = True
          Left = 340.157480310000000000
          Width = 113.385826770000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo29: TfrxMemoView
          AllowVectorExport = True
          Left = 453.543307086614000000
          Width = 113.385826771654000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataField = 'stokakhir'
          DataSet = frxDBDataset1
          DataSetName = 'frxDBDataset1'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDataset1."stokakhir"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo31: TfrxMemoView
          AllowVectorExport = True
          Left = 661.417322834646000000
          Width = 79.370078740157500000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo32: TfrxMemoView
          AllowVectorExport = True
          Left = 566.929133858268000000
          Width = 94.488250000000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object DetailData1: TfrxDetailData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 241.889920000000000000
        Width = 740.409927000000000000
        DataSet = DbKartu_gudang
        DataSetName = 'DBKartu_Gudang'
        RowCount = 0
        object Memo14: TfrxMemoView
          AllowVectorExport = True
          Width = 75.590551180000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          DisplayFormat.FormatStr = 'dd mmm yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          Memo.UTF8W = (
            '[DBKartu_Gudang."tgl_trans"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo15: TfrxMemoView
          AllowVectorExport = True
          Left = 75.590551180000000000
          Width = 151.181102360000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataField = 'kd_stok'
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          Memo.UTF8W = (
            '[DBKartu_Gudang."kd_stok"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          AllowVectorExport = True
          Left = 226.771653540000000000
          Width = 113.385826771654000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataField = 'ms'
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[DBKartu_Gudang."ms"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo18: TfrxMemoView
          AllowVectorExport = True
          Left = 340.157480310000000000
          Width = 113.385826770000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataField = 'kl'
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[DBKartu_Gudang."kl"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo19: TfrxMemoView
          AllowVectorExport = True
          Left = 453.543307086614000000
          Width = 113.385826771654000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataField = 'sa'
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[DBKartu_Gudang."sa"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo20: TfrxMemoView
          AllowVectorExport = True
          Left = 566.929133858268000000
          Width = 94.488250000000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          AllowVectorExport = True
          Left = 661.417322834646000000
          Width = 79.370078740157500000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataSet = DbKartu_gudang
          DataSetName = 'DBKartu_Gudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
      end
    end
  end
  object QKartu_Gudang: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'SELECT m.kd_barang, i.item_name, COALESCE(m.saldo_awal ,0) + COA' +
        'LESCE(SUM(t.jum_trans_in - jum_trans_out), 0) AS saldo_awal_peri' +
        'ode, satuan  FROM  (SELECT kd_barang,  saldo_awal_lev1 AS  saldo' +
        '_awal,  satuan_lev1 AS  satuan from tbl_transaksi_stock_sa where' +
        ' kd_gudang = '#39'GD-BD01'#39')  m    LEFT JOIN ( SELECT item_code, SUM(' +
        'level1_qty_in) AS   jum_trans_in,  SUM(level1_qty_out) AS jum_tr' +
        'ans_out  from tbl_transaksi_stock  WHERE type IN ('#39'IN'#39','#39'OUT'#39') an' +
        'd tgltrans BETWEEN '#39'2025-06-30'#39'::date AND ('#39'2025-11-19'#39'::date - ' +
        'INTERVAL '#39'1 day'#39')AND kd_gudang = '#39'GD-BD01'#39' GROUP BY item_code OR' +
        'DER BY item_code) t  ON m.kd_barang = t.item_code '
      'LEFT JOIN t_item i ON m.kd_barang= i.item_code'
      
        ' GROUP BY m.kd_barang,i.item_name,m.saldo_awal,satuan ORDER BY m' +
        '.kd_barang')
    MasterFields = 'kd_stok'
    DetailFields = 'kd_stok'
    Left = 8
    Top = 304
    object QKartu_Gudangkd_barang: TStringField
      FieldName = 'kd_barang'
      ReadOnly = True
      Size = 50
    end
    object QKartu_Gudangitem_name: TStringField
      FieldName = 'item_name'
      Size = 255
    end
    object QKartu_Gudangsaldo_awal_periode: TFloatField
      FieldName = 'saldo_awal_periode'
      ReadOnly = True
      DisplayFormat = '0.00#,##'
      EditFormat = '0.00#,##'
    end
    object QKartu_Gudangsatuan: TStringField
      FieldName = 'satuan'
      ReadOnly = True
      Size = 50
    end
  end
  object DsKartu_Gudang: TDataSource
    DataSet = QKartu_Gudang
    Left = 16
    Top = 261
  end
  object DbKartu_gudang: TfrxDBDataset
    UserName = 'DBKartu_Gudang'
    CloseDataSource = False
    FieldAliases.Strings = (
      'item_code=item_code'
      'item_name=item_name'
      'unit=unit'
      'wh_code=wh_code'
      'to_char=to_char'
      'trans_date=trans_date'
      'stock_code=stock_code'
      'sa_first=sa_first'
      'in=in'
      'out=out'
      'sa_end=sa_end'
      'trans_code=trans_code')
    DataSet = QKartu_Gudang
    BCDToCurrency = False
    DataSetOptions = []
    Left = 9
    Top = 216
  end
  object Qstok: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'Select * from (select a.kd_stok,a.kd_material_stok,a.stokakhir,c' +
        '.nm_supplier,b.nm_material,d.gudang FROM t_trans_material a INNE' +
        'R JOIN t_material_stok b on a.kd_material_stok=b.kd_material_sto' +
        'k INNER JOIN t_supplier c on b.kd_supplier=c.kd_supplier INNER J' +
        'OIN  t_material_stok_det AS d ON a.kd_material_stok=d.kd_materia' +
        'l_stok and a.kd_stok=d.kd_stok where idstok in (select max(idsto' +
        'k) ids from (SELECT'#9'a.nm_material, c.nm_supplier,b.kd_stok,  b.s' +
        'tokawal,b.masuk, b.keluar,b.stokakhir,b.satuan, b.ket, b.tgl_tra' +
        'ns, b.kd_material_stok, b.kd_material, d.gudang,b.idstok FROM'#9't_' +
        'material_stok AS "a" INNER JOIN t_trans_material AS b ON  a.kd_m' +
        'aterial_stok = b.kd_material_stok  LEFT JOIN t_supplier AS "c" O' +
        'N a.kd_supplier = c.kd_supplier INNER JOIN  t_material_stok_det ' +
        'AS d ON  b.kd_material_stok = d.kd_material_stok and d.kd_stok=b' +
        '.kd_stok  where  a.nm_material='#39'TEPUNG ONGGOK'#39' and  d.gudang='#39'GU' +
        'DANG TEPUNG A1'#39' and a.kd_supplier='#39'MO'#39' and to_char(b.tgl_trans, ' +
        #39'mm-yyyy'#39')<'#39'04-2022'#39' GROUP BY a.nm_material, c.nm_supplier,b.kd_' +
        'stok, b.stokawal, b.masuk,b.keluar, b.stokakhir,b.satuan,b.ket,b' +
        '.tgl_trans,b.kd_material_stok, b.kd_material,d.gudang,b.idstok)a' +
        '  GROUP BY kd_stok,kd_material_stok order by kd_stok asc) and st' +
        'okakhir > 0'
      'UNION all'
      
        'select a.kd_stok,a.kd_material_stok,'#39'0'#39',c.nm_supplier,b.nm_mater' +
        'ial,d.gudang FROM t_trans_material a INNER JOIN t_material_stok ' +
        'b on a.kd_material_stok=b.kd_material_stok INNER JOIN t_supplier' +
        ' c on b.kd_supplier=c.kd_supplier LEFT JOIN  t_material_stok_det' +
        ' AS d ON a.kd_material_stok=d.kd_material_stok and a.kd_stok=d.k' +
        'd_stok where idstok in (select min(idstok) ids from (SELECT'#9'a.nm' +
        '_material, c.nm_supplier,b.kd_stok,  b.stokawal,b.masuk, b.kelua' +
        'r,b.stokakhir,b.satuan, b.ket, b.tgl_trans, b.kd_material_stok, ' +
        'b.kd_material, d.gudang,b.idstok FROM'#9't_material_stok AS "a" INN' +
        'ER JOIN t_trans_material AS b ON  a.kd_material_stok = b.kd_mate' +
        'rial_stok  LEFT JOIN t_supplier AS "c" ON a.kd_supplier = c.kd_s' +
        'upplier INNER JOIN  t_material_stok_det AS d ON  b.kd_material_s' +
        'tok = d.kd_material_stok and d.kd_stok=b.kd_stok  where  a.nm_ma' +
        'terial='#39'TEPUNG ONGGOK'#39' and  d.gudang='#39'GUDANG TEPUNG A1'#39' and a.kd' +
        '_supplier='#39'MO'#39' and to_char(b.tgl_trans, '#39'mm-yyyy'#39')='#39'04-2022'#39' GRO' +
        'UP BY a.nm_material, c.nm_supplier,b.kd_stok, b.stokawal, b.masu' +
        'k,b.keluar, b.stokakhir,b.satuan,b.ket,b.tgl_trans,b.kd_material' +
        '_stok, b.kd_material,d.gudang,b.idstok)a  GROUP BY kd_stok,kd_ma' +
        'terial_stok order by kd_stok asc) and masuk > 0) a')
    Left = 280
    Top = 168
  end
  object frxDBDataset1: TfrxDBDataset
    UserName = 'frxDBDataset1'
    CloseDataSource = False
    FieldAliases.Strings = (
      'item_code=item_code'
      'item_name=item_name'
      'unit=unit'
      'wh_code=wh_code'
      'to_char=to_char'
      'trans_date=trans_date'
      'stock_code=stock_code'
      'sa_first=sa_first'
      'in=in'
      'out=out'
      'sa_end=sa_end'
      'trans_code=trans_code')
    DataSet = QKartu_Gudang
    BCDToCurrency = False
    DataSetOptions = []
    Left = 224
    Top = 161
  end
  object DsStok: TDataSource
    DataSet = Qstok
    Left = 232
    Top = 248
  end
  object dxBarManager1: TdxBarManager
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    Categories.Strings = (
      'Default')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    PopupMenuLinks = <>
    UseSystemFont = True
    Left = 548
    Top = 160
    PixelsPerInch = 96
    object dxBarManager1Bar1: TdxBar
      Caption = 'Action'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 805
      FloatTop = 8
      FloatClientWidth = 100
      FloatClientHeight = 130
      ItemLinks = <
        item
          UserDefine = [udWidth]
          UserWidth = 204
          Visible = True
          ItemName = 'CbGudang2'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 204
          Visible = True
          ItemName = 'cxkategori_barang'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 204
          Visible = True
          ItemName = 'cxnm_barang'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 192
          Visible = True
          ItemName = 'tgl_awal'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 192
          Visible = True
          ItemName = 'tgl_akhir'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 192
          Visible = True
          ItemName = 'cbLevelSatuan'
        end
        item
          BeginGroup = True
          Visible = True
          ItemName = 'DxRefresh'
        end
        item
          Visible = True
          ItemName = 'dxBarLargeButton1'
        end>
      OneOnRow = True
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object dxBUpdate: TdxBarButton
      Caption = 'Update  '
      Category = 0
      Visible = ivNever
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFCF7ECFFEDC36AFFF1D08BFFFFFFFFFFFFFFFFFF}
    end
    object dxBDelete: TdxBarButton
      Caption = 'Delete  '
      Category = 0
      Visible = ivNever
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFBFBFFFFF0C0CFFFFABABFFFFFFFFFFFFFFFFFFFF}
    end
    object dxBbaru: TdxBarLargeButton
      Caption = 'Baru  '
      Category = 0
      Visible = ivNever
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        FFFFFFFFFFFF}
    end
    object dxBarSubItem1: TdxBarSubItem
      Caption = 'Print  '
      Category = 0
      Enabled = False
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ItemLinks = <>
    end
    object dxBarLargeButton1: TdxBarLargeButton
      Caption = 'Print'
      Category = 0
      Hint = 'Print'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        }
      OnClick = dxBarLargeButton1Click
    end
    object DxRefresh: TdxBarLargeButton
      Caption = 'Cari'
      Category = 0
      Hint = 'Cari'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        2F673E0D0A09093C2F673E0D0A093C2F673E0D0A3C2F7376673E0D0A}
      OnClick = DxRefreshClick
    end
    object cxBarEditItem1: TcxBarEditItem
      Caption = 'Bulan'
      Category = 0
      Hint = 'Bulan'
      Visible = ivAlways
      PropertiesClassName = 'TcxLabelProperties'
    end
    object SpTahun: TcxBarEditItem
      Caption = 'Tahun  '
      Category = 0
      Hint = 'Tahun  '
      Visible = ivAlways
      PropertiesClassName = 'TcxSpinEditProperties'
      Properties.MaxValue = 9999.000000000000000000
      Properties.MinValue = 2000.000000000000000000
    end
    object cxnm_barang: TcxBarEditItem
      Caption = 'Nama Barang      '
      Category = 0
      Hint = 'Nama Barang      '
      Visible = ivAlways
      OnChange = cxnm_barangChange
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = cxBarEditItem2PropertiesButtonClick
    end
    object cxkd_barang: TcxBarEditItem
      Caption = 'Nama Barang'
      Category = 0
      Hint = 'Nama Barang'
      Visible = ivAlways
      PropertiesClassName = 'TcxTextEditProperties'
      Properties.ReadOnly = True
    end
    object CbGudang2: TcxBarEditItem
      Caption = 'Gudang                '
      Category = 0
      Hint = 'Gudang                '
      Visible = ivAlways
      OnChange = CbGudang2Change
      PropertiesClassName = 'TcxComboBoxProperties'
    end
    object cxBarEditItem2: TcxBarEditItem
      Caption = 'New Item'
      Category = 0
      Hint = 'New Item'
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object dxBarEdit1: TdxBarEdit
      Caption = 'New Item'
      Category = 0
      Hint = 'New Item'
      Visible = ivAlways
    end
    object cxkategori_barang: TcxBarEditItem
      Caption = 'Kategori Barang  '
      Category = 0
      Hint = 'Kategori'
      Visible = ivAlways
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
      Properties.OnButtonClick = cxkategori_barangPropertiesButtonClick
    end
    object tgl_awal: TcxBarEditItem
      Caption = 'Periode Awal   '
      Category = 0
      Hint = 'Periode Awal   '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object dxBarSubItem2: TdxBarSubItem
      Caption = 'New SubItem'
      Category = 0
      Visible = ivAlways
      ItemLinks = <>
    end
    object tgl_akhir: TcxBarEditItem
      Caption = 'Periode  Akhir  '
      Category = 0
      Hint = 'Periode  Akhir  '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object cbLevelSatuan: TcxBarEditItem
      Caption = 'Level Satuan    '
      Category = 0
      Hint = 'Level Satuan    '
      Visible = ivAlways
      PropertiesClassName = 'TcxComboBoxProperties'
      Properties.Items.Strings = (
        'SATUAN TERBESAR'
        'SATUAN TERKECIL')
    end
    object cxBarEditItem3: TcxBarEditItem
      Caption = 'New Item'
      Category = 0
      Hint = 'New Item'
      Visible = ivAlways
      PropertiesClassName = 'TcxLabelProperties'
    end
  end
  object QKartuGUdang: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'SELECT aa.*, i.item_name as itemProduct_name from (  SELECT '#39'-'#39' ' +
        'as notrans, kd_barang, '#39#39' as type,  to_char(to_date('#39'19-11-2025'#39 +
        ', '#39'DD-MM-YYYY'#39'), '#39'DD/MM/YYYY'#39') AS tgltrans,  '#39'SALDO AWAL'#39' as ket' +
        'erangan, 0 as level1_qty_in, 0 as level1_qty_out, saldo_awal_per' +
        'iode as saldo_awal, 0 as urut, satuan from (SELECT m.kd_barang, ' +
        'i.item_name, COALESCE(m.saldo_awal ,0) +  COALESCE(SUM(t.jum_tra' +
        'ns_in - jum_trans_out), 0) AS saldo_awal_periode,  satuan  FROM ' +
        ' (SELECT kd_barang,   saldo_awal_lev1 AS  saldo_awal,    satuan_' +
        'lev1 AS  satuan from tbl_transaksi_stock_sa where kd_gudang = '#39'G' +
        'D-BD01'#39')  m  LEFT JOIN ( SELECT item_code,  SUM(level1_qty_in) A' +
        'S  jum_trans_in,    SUM(level1_qty_out) AS jum_trans_out  from t' +
        'bl_transaksi_stock  WHERE type  IN ('#39'IN'#39','#39'OUT'#39') and tgltrans  BE' +
        'TWEEN '#39'2025-09-30'#39'::date AND ('#39'2025-12-10'#39'::date - INTERVAL '#39'1 d' +
        'ay'#39')  AND kd_gudang = '#39'GD-BD01'#39' GROUP BY item_code ORDER BY item' +
        '_code) t   ON m.kd_barang = t.item_code  LEFT JOIN t_item i ON m' +
        '.kd_barang= i.item_code  where i.category_id=1 '
      ' '
      
        ' GROUP BY m.kd_barang,i.item_name,m.saldo_awal,satuan ORDER BY m' +
        '.kd_barang) sa  UNION ALL  '
      ''
      
        'SELECT notrans, t.item_code as kd_barang, t.type, to_char(tgltra' +
        'ns, '#39'DD/MM/YYYY'#39') AS tgltrans,  case when t.sumber_table ='#39't_pur' +
        'chase_invoice_det'#39' then concat('#39'Pembelian '#39',notrans)  when t.sum' +
        'ber_table ='#39't_selling_det'#39' then concat('#39'Penjualan '#39',notrans)  wh' +
        'en t.sumber_table ='#39't_purchase_return_det'#39' then concat('#39'Retur Pe' +
        'mbelian '#39',notrans) when t.sumber_table ='#39't_sales_returns_det'#39' th' +
        'en concat('#39'Retur Penjualan '#39',notrans) end keterangan,  t.level1_' +
        'qty_in AS level1_qty_in,  t.level1_qty_out AS level1_qty_out, 0 ' +
        'as saldo_awal,  1 as urut,  level1_satuan AS  satuan  from tbl_t' +
        'ransaksi_stock t  '
      'LEFT JOIN t_purchase_invoice beli on t.notrans=beli.trans_no  '
      'LEFT JOIN t_selling jual on t.notrans=jual.trans_no   '
      
        'LEFT JOIN t_purchase_return retbeli on t.notrans=retbeli.return_' +
        'no  '
      
        'LEFT JOIN t_sales_returns retjual on t.notrans=retjual.trans_no ' +
        '  '
      
        'where t.kd_gudang = '#39'GD-BD01'#39' AND  (t.tgltrans between '#39'2025-11-' +
        '19'#39' and  '#39'2025-11-19'#39') and  t.type IN ('#39'IN'#39','#39'OUT'#39')  )aa  '
      ''
      'LEFT JOIN t_item i ON aa.kd_barang= i.item_code'
      ' --where jenisproduct_id=1 '
      
        ' GROUP BY "notrans", "kd_barang", "type", "tgltrans", "keteranga' +
        'n", "level1_qty_in", "level1_qty_out", "saldo_awal", "urut", "sa' +
        'tuan", "itemproduct_name"'
      ' ORDER BY tgltrans,kd_barang,urut,type,notrans asc')
    Left = 256
    Top = 408
    object QKartuGUdangnotrans: TMemoField
      FieldName = 'notrans'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QKartuGUdangkd_barang: TMemoField
      FieldName = 'kd_barang'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QKartuGUdangtype: TMemoField
      FieldName = 'type'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QKartuGUdangtgltrans: TMemoField
      FieldName = 'tgltrans'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QKartuGUdangketerangan: TMemoField
      FieldName = 'keterangan'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QKartuGUdanglevel1_qty_in: TFloatField
      FieldName = 'level1_qty_in'
      ReadOnly = True
    end
    object QKartuGUdanglevel1_qty_out: TFloatField
      FieldName = 'level1_qty_out'
      ReadOnly = True
    end
    object QKartuGUdangsaldo_awal: TFloatField
      FieldName = 'saldo_awal'
      ReadOnly = True
    end
    object QKartuGUdangurut: TIntegerField
      FieldName = 'urut'
      ReadOnly = True
    end
    object QKartuGUdangsatuan: TMemoField
      FieldName = 'satuan'
      ReadOnly = True
      BlobType = ftMemo
    end
    object QKartuGUdangitemproduct_name: TStringField
      FieldName = 'itemproduct_name'
      Size = 255
    end
  end
  object frxDBDKartuGUdang: TfrxDBDataset
    UserName = 'frxDBDKartuGudang'
    CloseDataSource = False
    FieldAliases.Strings = (
      'notrans=notrans'
      'kd_barang=kd_barang'
      'type=type'
      'tgltrans=tgltrans'
      'keterangan=keterangan'
      'level1_qty_in=level1_qty_in'
      'level1_qty_out=level1_qty_out'
      'saldo_awal=saldo_awal'
      'urut=urut'
      'satuan=satuan'
      'itemproduct_name=itemProduct_name')
    DataSet = QKartuGUdang
    BCDToCurrency = False
    DataSetOptions = []
    Left = 160
    Top = 408
  end
  object Report: TfrxReport
    Version = '2022.1.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PreviewOptions.ZoomMode = zmWholePage
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 45838.977766886600000000
    ReportOptions.LastChange = 45842.391858252300000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      ''
      
        'procedure MasterData1OnBeforePrint(Sender: TfrxComponent);      ' +
        ' '
      'var'
      '  RupiahValue: Double;'
      '  FormattedRupiah: String;  '
      'begin      '
      '  if <frxDBDKartuGudang."urut">='#39'0'#39' then'
      '  begin'
      '   frxDBDKartuGudangtgltrans.visible:=false;'
      '   frxDBDKartuGudanglevel1_qty_in.visible:=false;'
      '   frxDBDKartuGudanglevel1_qty_out.visible:=false;       '
      '   //lbketerangan.Text:='#39'SALDO AWAL'#39' ;'
      '     '
      
        '   Report.Variables['#39'Vsaldo'#39'] :=<frxDBDKartuGudang."saldo_awal">' +
        ';'
      
        '   saldo.Text:=FormatFloat('#39'#,###,###,###0.####'#39', Report.Variabl' +
        'es['#39'Vsaldo'#39']) ;                        '
      
        '   Report.Variables['#39'Vtotaldebit'#39'] := FormatFloat('#39'#,###,###,###' +
        '0.####'#39', 0) ;'
      
        '   Report.Variables['#39'Vtotalkredit'#39'] := FormatFloat('#39'#,###,###,##' +
        '#0.####'#39', 0) ;                                                  ' +
        '                                         '
      '  end;     '
      '                                              '
      
        '  if (<frxDBDKartuGudang."urut"><>'#39'0'#39') and (<frxDBDKartuGudang."' +
        'level1_qty_in">>=0) then'
      '  begin                         '
      '   frxDBDKartuGudangtgltrans.visible:=true;'
      '   frxDBDKartuGudanglevel1_qty_in.visible:=true;'
      '   frxDBDKartuGudanglevel1_qty_out.visible:=true;'
      '     '
      
        '   Report.Variables['#39'Vsaldo'#39'] := Report.Variables['#39'Vsaldo'#39']+<frx' +
        'DBDKartuGudang."level1_qty_in">;                  '
      
        '   saldo.Text:=FormatFloat('#39'#,###,###,###0.####'#39', Report.Variabl' +
        'es['#39'Vsaldo'#39']);'
      ''
      
        '   Report.Variables['#39'Vtotaldebit'#39'] := FormatFloat('#39'#,###,###,###' +
        '0.####'#39',Report.Variables['#39'Vtotaldebit'#39']+<frxDBDKartuGudang."leve' +
        'l1_qty_in">);       '
      
        '                                                                ' +
        '                                    '
      '  end;'
      '    '
      
        '  if (<frxDBDKartuGudang."urut"><>'#39'0'#39') and (<frxDBDKartuGudang."' +
        'level1_qty_out">>0) then'
      '  begin                         '
      '   frxDBDKartuGudangtgltrans.visible:=true;'
      '   frxDBDKartuGudanglevel1_qty_in.visible:=true;'
      '   frxDBDKartuGudanglevel1_qty_out.visible:=true;       '
      ''
      
        '   Report.Variables['#39'Vsaldo'#39'] := Report.Variables['#39'Vsaldo'#39']-<frx' +
        'DBDKartuGudang."level1_qty_out">;'
      
        '   saldo.Text:=FormatFloat('#39'#,###,###,###0.####'#39', Report.Variabl' +
        'es['#39'Vsaldo'#39']);'
      ''
      
        '   Report.Variables['#39'Vtotalkredit'#39'] := FormatFloat('#39'#,###,###,##' +
        '#0.####'#39',Report.Variables['#39'Vtotalkredit'#39']+<frxDBDKartuGudang."le' +
        'vel1_qty_out">);'
      
        '                                                                ' +
        '                                      '
      '  end;'
      '                 '
      'end;'
      '  '
      'begin'
      ''
      'end.')
    Left = 80
    Top = 408
    Datasets = <
      item
        DataSet = frxDBDKartuGUdang
        DataSetName = 'frxDBDKartuGudang'
      end>
    Variables = <
      item
        Name = ' New Category1'
        Value = Null
      end
      item
        Name = 'Vsaldo'
        Value = Null
      end
      item
        Name = 'Vsaldoakhir'
        Value = Null
      end
      item
        Name = 'vKodeOutlet'
        Value = Null
      end
      item
        Name = 'Vtotaldebit'
        Value = Null
      end
      item
        Name = 'Vtotalkredit'
        Value = Null
      end>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
      MirrorMode = []
      object GroupHeader1: TfrxGroupHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 222.992270000000000000
        Top = 102.047310000000000000
        Width = 718.110700000000000000
        Condition = 'frxDBDKartuGudang."kd_barang"'
        StartNewPage = True
        object Shape4: TfrxShapeView
          Align = baClient
          AllowVectorExport = True
          Width = 718.110700000000000000
          Height = 222.992270000000000000
          Frame.Typ = []
        end
        object Shape2: TfrxShapeView
          AllowVectorExport = True
          Width = 718.110700000000000000
          Height = 56.692950000000000000
          Frame.Typ = []
        end
        object Memo3: TfrxMemoView
          AllowVectorExport = True
          Left = 11.338590000000000000
          Top = 71.811070000000000000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'SKU')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo4: TfrxMemoView
          AllowVectorExport = True
          Left = 11.338590000000000000
          Top = 90.708720000000000000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'NAMA BARANG')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo5: TfrxMemoView
          AllowVectorExport = True
          Left = 11.338590000000000000
          Top = 109.606370000000000000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'GUDANG')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo6: TfrxMemoView
          AllowVectorExport = True
          Left = 11.338590000000000000
          Top = 128.504020000000000000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'SATUAN')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo7: TfrxMemoView
          AllowVectorExport = True
          Left = 11.338590000000000000
          Top = 147.401670000000000000
          Width = 102.047310000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'PERIODE')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo8: TfrxMemoView
          AllowVectorExport = True
          Left = 113.385900000000000000
          Top = 71.811070000000000000
          Width = 7.559060000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            ':')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo9: TfrxMemoView
          AllowVectorExport = True
          Left = 113.385900000000000000
          Top = 90.708720000000000000
          Width = 7.559060000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            ':')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo10: TfrxMemoView
          AllowVectorExport = True
          Left = 113.385900000000000000
          Top = 109.606370000000000000
          Width = 7.559060000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            ':')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 113.385900000000000000
          Top = 128.504020000000000000
          Width = 7.559060000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            ':')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo12: TfrxMemoView
          AllowVectorExport = True
          Left = 113.385900000000000000
          Top = 147.401670000000000000
          Width = 7.559060000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            ':')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDKartuGudangkd_barang: TfrxMemoView
          AllowVectorExport = True
          Left = 120.944960000000000000
          Top = 71.811070000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          DataField = 'kd_barang'
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDKartuGudang."kd_barang"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          AllowVectorExport = True
          Left = 120.944960000000000000
          Top = 90.708720000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDKartuGudang."itemProduct_name"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object periode: TfrxMemoView
          AllowVectorExport = True
          Left = 120.944960000000000000
          Top = 147.401670000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'periode')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo2: TfrxMemoView
          AllowVectorExport = True
          Left = 75.590600000000000000
          Top = 4.000000000000000000
          Width = 638.740570000000000000
          Height = 22.677180000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold, fsUnderline]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'KARTU GUDANG')
          ParentFont = False
          VAlign = vaCenter
        end
        object nama_pt: TfrxMemoView
          AllowVectorExport = True
          Width = 71.811070000000000000
          Height = 52.913420000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'nama_pt')
          ParentFont = False
          VAlign = vaCenter
        end
        object satuan: TfrxMemoView
          AllowVectorExport = True
          Left = 75.590600000000000000
          Top = 30.236240000000000000
          Width = 638.740570000000000000
          Height = 22.677180000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '(SATUAN)')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line2: TfrxLineView
          AllowVectorExport = True
          Left = 71.590600000000000000
          Top = 30.236240000000000000
          Width = 646.299166220000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Line3: TfrxLineView
          AllowVectorExport = True
          Left = 71.811070000000000000
          Height = 56.692950000000000000
          Color = clBlack
          Frame.Typ = [ftLeft]
        end
        object Line1: TfrxLineView
          AllowVectorExport = True
          Top = 60.472480000000000000
          Width = 718.110236220000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Shape1: TfrxShapeView
          AllowVectorExport = True
          Top = 177.637910000000000000
          Width = 718.110700000000000000
          Height = 45.354330710000000000
          Frame.Typ = []
        end
        object Line4: TfrxLineView
          AllowVectorExport = True
          Top = 200.315090000000000000
          Width = 718.110236220000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
        object Memo14: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Top = 200.315090000000000000
          Width = 98.267780000000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'TANGGAL')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo15: TfrxMemoView
          AllowVectorExport = True
          Left = 102.047310000000000000
          Top = 200.315090000000000000
          Width = 173.858380000000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'NO TRANSAKSI')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo16: TfrxMemoView
          AllowVectorExport = True
          Left = 275.905690000000000000
          Top = 200.315090000000000000
          Width = 170.078850000000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'KETERANGAN')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          AllowVectorExport = True
          Left = 445.984540000000000000
          Top = 200.315090000000000000
          Width = 90.708720000000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'DEBET')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo18: TfrxMemoView
          AllowVectorExport = True
          Left = 536.693260000000000000
          Top = 200.315090000000000000
          Width = 90.708720000000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'KREDIT')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo19: TfrxMemoView
          AllowVectorExport = True
          Left = 627.401980000000000000
          Top = 200.315090000000000000
          Width = 90.708720000000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'SALDO')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line5: TfrxLineView
          AllowVectorExport = True
          Left = 445.984540000000000000
          Top = 177.637910000000000000
          Height = 45.354360000000000000
          Color = clBlack
          Frame.Typ = [ftLeft]
        end
        object Line6: TfrxLineView
          AllowVectorExport = True
          Left = 102.047310000000000000
          Top = 201.094620000000000000
          Height = 22.677165350000000000
          Color = clBlack
          Frame.Typ = [ftLeft]
        end
        object Line7: TfrxLineView
          AllowVectorExport = True
          Left = 275.905690000000000000
          Top = 200.315090000000000000
          Height = 22.677165350000000000
          Color = clBlack
          Frame.Typ = [ftLeft]
        end
        object Line8: TfrxLineView
          AllowVectorExport = True
          Left = 536.693260000000000000
          Top = 200.315090000000000000
          Height = 22.677165350000000000
          Color = clBlack
          Frame.Typ = [ftLeft]
        end
        object Line9: TfrxLineView
          AllowVectorExport = True
          Left = 627.401980000000000000
          Top = 200.315090000000000000
          Height = 22.677165350000000000
          Color = clBlack
          Frame.Typ = [ftLeft]
        end
        object nama_gudang: TfrxMemoView
          AllowVectorExport = True
          Left = 120.944960000000000000
          Top = 109.606370000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            'nama_gudang')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDKartuGudangsatuan: TfrxMemoView
          AllowVectorExport = True
          Left = 120.944960000000000000
          Top = 128.504020000000000000
          Width = 400.630180000000000000
          Height = 18.897650000000000000
          DataField = 'satuan'
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDKartuGudang."satuan"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Top = 18.897650000000000000
        Width = 718.110700000000000000
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 347.716760000000000000
        Width = 718.110700000000000000
        OnBeforePrint = 'MasterData1OnBeforePrint'
        DataSet = frxDBDKartuGUdang
        DataSetName = 'frxDBDKartuGudang'
        RowCount = 0
        Stretched = True
        object frxDBDKartuGudangnotrans: TfrxMemoView
          AllowVectorExport = True
          Left = 102.047310000000000000
          Width = 173.858380000000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataField = 'notrans'
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDKartuGudang."notrans"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDKartuGudangtgltrans: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Width = 98.267780000000000000
          Height = 22.677165350000000000
          DataField = 'tgltrans'
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDKartuGudang."tgltrans"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo20: TfrxMemoView
          AllowVectorExport = True
          Left = 275.905690000000000000
          Width = 170.078850000000000000
          Height = 22.677165350000000000
          StretchMode = smMaxHeight
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDKartuGudang."keterangan"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDKartuGudanglevel1_qty_in: TfrxMemoView
          AllowVectorExport = True
          Left = 445.984540000000000000
          Width = 90.708720000000000000
          Height = 22.677165350000000000
          DataField = 'level1_qty_in'
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          DisplayFormat.FormatStr = '#,###,###,###0.####'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDKartuGudang."level1_qty_in"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object frxDBDKartuGudanglevel1_qty_out: TfrxMemoView
          AllowVectorExport = True
          Left = 536.693260000000000000
          Width = 90.708720000000000000
          Height = 22.677165350000000000
          DataField = 'level1_qty_out'
          DataSet = frxDBDKartuGUdang
          DataSetName = 'frxDBDKartuGudang'
          DisplayFormat.FormatStr = '#,###,###,###0.####'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDKartuGudang."level1_qty_out"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object saldo: TfrxMemoView
          AllowVectorExport = True
          Left = 627.401980000000000000
          Width = 86.929126540000000000
          Height = 22.677165350000000000
          DisplayFormat.FormatStr = #39'#,###,###,###0.####'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          ParentFont = False
          VAlign = vaCenter
        end
        object Line15: TfrxLineView
          AllowVectorExport = True
          Left = 718.551640000000000000
          Height = 22.677180000000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
        object Line16: TfrxLineView
          AllowVectorExport = True
          Height = 22.677180000000000000
          StretchMode = smMaxHeight
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
      end
      object PageFooter1: TfrxPageFooter
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 476.220780000000000000
        Width = 718.110700000000000000
        object Memo1: TfrxMemoView
          AllowVectorExport = True
          Left = 642.520100000000000000
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            'Page : [Page#]')
        end
        object Line13: TfrxLineView
          AllowVectorExport = True
          Width = 718.110700000000000000
          Color = clBlack
          Frame.Typ = []
          Diagonal = True
        end
      end
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Top = 41.574830000000000000
        Width = 718.110700000000000000
      end
      object GroupFooter1: TfrxGroupFooter
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 393.071120000000000000
        Width = 718.110700000000000000
        object Shape3: TfrxShapeView
          Align = baClient
          AllowVectorExport = True
          Width = 718.110700000000000000
          Height = 22.677180000000000000
          Frame.Typ = []
        end
        object Memo22: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Width = 442.205010000000000000
          Height = 22.677165350000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'JUMLAH')
          ParentFont = False
          VAlign = vaCenter
        end
        object SysMemo1: TfrxSysMemoView
          AllowVectorExport = True
          Left = 445.984540000000000000
          Width = 90.708720000000000000
          Height = 22.677165350000000000
          DisplayFormat.FormatStr = '#,###,###,###0.####'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDKartuGudang."level1_qty_in">,MasterData1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object SysMemo2: TfrxSysMemoView
          AllowVectorExport = True
          Left = 536.693260000000000000
          Width = 90.708720000000000000
          Height = 22.677165350000000000
          DisplayFormat.FormatStr = '#,###,###,###0.####'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDKartuGudang."level1_qty_out">,MasterData1)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Line10: TfrxLineView
          AllowVectorExport = True
          Left = 445.984540000000000000
          Height = 22.677165350000000000
          Color = clBlack
          Frame.Typ = [ftLeft]
        end
        object Line11: TfrxLineView
          AllowVectorExport = True
          Left = 536.693260000000000000
          Height = 22.677165350000000000
          Color = clBlack
          Frame.Typ = [ftLeft]
        end
        object Line12: TfrxLineView
          AllowVectorExport = True
          Left = 627.401980000000000000
          Height = 22.677165350000000000
          Color = clBlack
          Frame.Typ = [ftLeft]
        end
      end
    end
  end
end
