object FDaftar_Perkiraan: TFDaftar_Perkiraan
  Left = 0
  Top = 0
  BorderStyle = bsSingle
  Caption = 'Daftar Perkiraan'
  ClientHeight = 491
  ClientWidth = 781
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  Position = poDesktopCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  TextHeight = 13
  object dxRibbon1: TdxRibbon
    Left = 0
    Top = 0
    Width = 781
    Height = 124
    BarManager = dxBarManager1
    ColorSchemeName = 'DevExpressStyle'
    Contexts = <>
    TabOrder = 0
    TabStop = False
    object dxRibbon1Tab1: TdxRibbonTab
      Active = True
      Caption = 'Daftar Perkiraan'
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar1'
        end>
      Index = 0
    end
  end
  object DBGridEh1: TDBGridEh
    Left = 0
    Top = 124
    Width = 781
    Height = 367
    Align = alClient
    DataSource = Dsdaftar_perkiraan
    DrawMemoText = True
    DynProps = <>
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
    RowDetailPanel.Active = True
    SearchPanel.Enabled = True
    TabOrder = 1
    OnDblClick = DBGridEh1DblClick
    Columns = <
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'code'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = 'Kode'
        Width = 140
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'account_name'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = 'Nama Perkiraan'
        Width = 291
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'header_name'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = 'Header'
        Width = 263
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##'
        DynProps = <>
        EditButtons = <>
        FieldName = 'balance'
        Footers = <>
        Title.Alignment = taCenter
        Title.Caption = 'Balance'
        Visible = False
        Width = 134
      end>
    object RowDetailData: TRowDetailPanelControlEh
      object DBGridEh2: TDBGridEh
        Left = 0
        Top = 0
        Width = 676
        Height = 118
        Align = alClient
        DataSource = DSdetail_perkiraan
        DynProps = <>
        TabOrder = 0
        Columns = <
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'id'
            Footers = <>
            Visible = False
          end
          item
            CellButtons = <>
            DynProps = <>
            EditButtons = <>
            FieldName = 'module_name'
            Footers = <>
            Title.Alignment = taCenter
            Title.Caption = 'Modul'
          end>
        object RowDetailData: TRowDetailPanelControlEh
        end
      end
    end
  end
  object dxBarManager1: TdxBarManager
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    Categories.Strings = (
      'Default')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    PopupMenuLinks = <>
    UseSystemFont = True
    Left = 609
    Top = 24
    PixelsPerInch = 96
    object dxBarManager1Bar1: TdxBar
      Caption = 'Action'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 775
      FloatTop = 8
      FloatClientWidth = 0
      FloatClientHeight = 0
      ItemLinks = <
        item
          Visible = True
          ItemName = 'dxBuBaru'
        end
        item
          Visible = True
          ItemName = 'dxBUpdate'
        end
        item
          Visible = True
          ItemName = 'dxBRefresh'
        end
        item
          Visible = True
          ItemName = 'dxBDelete'
        end>
      OneOnRow = True
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object dxBUpdate: TdxBarButton
      Caption = 'Update     '
      Category = 0
      Hint = 'Update     '
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        82}
      OnClick = dxBUpdateClick
    end
    object dxBRefresh: TdxBarButton
      Caption = 'Refresh     '
      Category = 0
      Hint = 'Refresh     '
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        02FE19007E4E40427BAAAF350000000049454E44AE426082}
      OnClick = dxBRefreshClick
    end
    object dxBDelete: TdxBarButton
      Caption = 'Delete       '
      Category = 0
      Enabled = False
      Hint = 'Delete       '
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        0000000049454E44AE426082}
      OnClick = dxBDeleteClick
    end
    object dxBarButton2: TdxBarButton
      Caption = 'Print Order Handling'
      Category = 0
      Hint = 'Print Order Handling'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FDFFFDFDFDFFFDFDFDFFFCFCFCFFFCFCFCFFFDFDFDFF}
    end
    object dxBuBaru: TdxBarLargeButton
      Caption = 'Baru'
      Category = 0
      Hint = 'Baru'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        0F208A3B0000000049454E44AE426082}
      OnClick = dxBuBaruClick
    end
    object dxBprint: TdxBarLargeButton
      Caption = 'Rencana Pelunasan Piutang'
      Category = 0
      Hint = 'Rencana Pelunasan Piutang'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        FDFFFDFDFDFFFDFDFDFFFCFCFCFFFCFCFCFFFDFDFDFF}
    end
    object dxBarLargeButton1: TdxBarLargeButton
      Caption = 'Status Selesai'
      Category = 0
      Hint = 'Status Selesai'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
  end
  object Qdaftar_perkiraan: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'select a.*, b.header_name,c.posting,d.category,e."type",case whe' +
        'n "group_id" = '#39'1'#39' then '#39'NERACA'#39' ELSE '#39'L/R'#39' END "group",f.type_b' +
        'alance,g.post_name,'
      
        'h.cost_type,i.lr2,case when posisi_dk='#39'D'#39' then '#39'DEBIT'#39' ELSE '#39'KRE' +
        'DIT'#39' END posisi_dk2  from t_ak_account a'
      'left join t_ak_header b on a.header_code=b.header_code'
      'left join t_ak_posting_type c on a.posting_id=c."id" '
      'left join t_ak_category d on a.category_code=d.category_code'
      'left join t_ak_type e on a.type_id=e."id"'
      'left join t_ak_type_balance f on a.balance_st_id=f."id"'
      
        'left join t_ak_type_balance_post g on a.balance_post_st_id=g."id' +
        '" '
      'left join t_ak_type_cost h on a.cost_st_id=h."id"'
      'left join t_ak_type_lr i on a.lr_st_id=i."id"'
      '-- WHERE code='#39'70000.03'#39
      'order by code ASC')
    Active = True
    Left = 240
    Top = 64
  end
  object Dsdaftar_perkiraan: TDataSource
    DataSet = MemDaftar_Perk
    Left = 387
  end
  object MemDaftar_Perk: TMemTableEh
    Active = True
    FetchAllOnOpen = True
    Params = <>
    DataDriver = DataSetDriverEh1
    Left = 384
    Top = 56
  end
  object DataSetDriverEh1: TDataSetDriverEh
    ProviderDataSet = Qdaftar_perkiraan
    Left = 608
    Top = 368
  end
  object DSdetail_perkiraan: TDataSource
    DataSet = Qdetail_perkiraan
    Left = 240
    Top = 8
  end
  object Qdetail_perkiraan: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      'select a.*,b.module_name from t_ak_account_det a'
      'left join t_ak_module b on a.module_id=b.id')
    MasterSource = Dsdaftar_perkiraan
    MasterFields = 'code'
    DetailFields = 'account_code'
    Left = 310
    Top = 34
    ParamData = <
      item
        DataType = ftString
        Name = 'code'
        ParamType = ptInput
        Value = '1111'
      end>
  end
end
