...
uses
  ...
  Vcl.ExtCtrls, RzCmboBx, frxClass, RzPanel, RzRadChk;
...
type
  TFDataPenerimaanBank = class(TForm)
    ...
    gbDataSumberPenerimaan: TGroupBox;
    Label25: TLabel;
    Label26: TLabel;
    edNoRefSumberPenerimaan: TRzButtonEdit;
    edNamaPelangganSumber: TEdit;
    cbSumberPenerimaan: TRzComboBox;
    Label27: TLabel;
    Label28: TLabel;
    Label29: TLabel;
    Label30: TLabel;
    ...
    procedure edKodePelangganSumberChange(Sender: TObject);
    procedure MemDetailPiutangAfterPost(DataSet: TDataSet);
    ...
    procedure edNoTransChange(Sender: TObject);
  private
    vtotal_debit, vtotal_kredit,vpiutang_kredit, vtotal_piutang : Currency;
    const
    MAX_RETRIES = 3;
  ...
  public
    ...
    strtgl, strbulan, strtahun, strNoTransaksiDiGanti: string;
    Year, Month, Day: Word;
    next_proses:boolean;
    UangMuka: Currency;
    { Public declarations }
    ...
    procedure UpdateDPPPembayaranCek;
  end;
...
uses UDaftarTagihan, Ubrowse_pelanggan, UMasterData, UDataModule, UMy_Function,
  ...
  UbrowseUangMukaPenjualan, UListNoTransaksi;
...
begin
  cbSumberPenerimaan.Clear;
  with dm.Qtemp do
  begin
    Close;
    SQL.Clear;
    SQL.Text := 'select name from t_cash_bank_acceptance_source WHERE deleted_at IS NULL order by name ASC';
    // Use Open() for SELECT statement
    Open;
  end;
  cbSumberPenerimaan.Items.Add('TIDAK ADA SUMBER');
  ...
  begin
    ...
    begin
      cbSumberPenerimaan.Items.Add(Dm.Qtemp['name']);
      Dm.Qtemp.Next;
    end;
  end;
  cbSumberPenerimaan.ItemIndex:=0;
end;

procedure TFDataPenerimaanBank.RzButton1Click(Sender: TObject);
begin
  if (edKode_Pelanggan.Text='0') OR (Length(edKode_Pelanggan.Text)=0) then
  begin
    ShowMessage('Silahkan Pilih Pelanggan...!!!');
    Exit;
  end;
  ...
  begin
    ...
    if (edKodeSumberTagihan.Text='1') OR (vid_modul='4') then
    begin
...
var curCash,curReceipt,curChequeAmount1: Currency;
    ...
    strCash, strReceipt, strCheque1: String;
begin
  curCash:=0;
  curReceipt:=0;
  curChequeAmount1:=0;
...
procedure TFDataPenerimaanBank.UpdateDPPPembayaranCek;
var strTglDitunaikan,strCodeDitunaikanDiBank,strNameDitunaikanDiBank,
    strTanggalTandaTerima,strNoTandaTerima :String;
begin
  strTglDitunaikan:='';
  strCodeDitunaikanDiBank:='';
  strNameDitunaikanDiBank:='';
  strTanggalTandaTerima:='';
  strNoTandaTerima:='';
  ...
  begin
    ...
    strTglDitunaikan:=QuotedStr(FormatDateTime('yyyy-mm-dd',dtTrans.Date));
    strCodeDitunaikanDiBank:=QuotedStr(edNorek.Text);
    strNameDitunaikanDiBank:=QuotedStr(edNamaBank.Text);

    with dm.Qtemp2 do
    begin
      close;
      sql.Clear;
      sql.Text:='UPDATE t_register_cek SET code_bank_chanel='+strCodeDitunaikanDiBank+','+
                'bank_chanel='+strNameDitunaikanDiBank+',disbursment_date='+strTglDitunaikan+' '+
                'WHERE id_dpp='+QuotedStr(MemDetailPiutang['id_dpp']);
      ExecSQL;
    end;
    MemDetailPiutang.Next;
  end;
...
begin
  ...
  begin
    ...
    edNamaPelangganSumber.Clear;
    edNamaPKPSumber.Clear;
  ...
  begin
    ...
    gbDataSumberPenerimaan.Visible:=True; //Harus True
    cbSumberPenerimaan.ItemIndex:=0;
...
procedure TFDataPenerimaanBank.chkNomorPenggantiClick(Sender: TObject);
begin
  if chkNomorPengganti.Checked=True then btnNoTransaksi.Visible:=True
  else btnNoTransaksi.Visible:=False;
  if Status=0 then edNoTrans.Text:='';
end;

procedure TFDataPenerimaanBank.Update;
var terbilang,sCustCode,sCustName :String;
begin
//  terbilang:= ConvKeHuruf(floattostr(edJumlah.value))+' Rupiah';
  if (cbJenisTransaksi.Text = 'NON PIUTANG') then
  begin
    sCustCode := edKodePelangganSumber.Text;
    sCustName := edNamaPelangganSumber.Text;
  end else
  begin
    sCustCode := edKode_Pelanggan.Text;
    sCustName := edNama_Pelanggan.Text;
  end;
  ...
  begin
    ...
    sql.add(' UPDATE "public"."t_cash_bank_acceptance" SET '+
            ...
            ' code_cust='+QuotedStr(sCustCode)+','+
            ' name_cust='+QuotedStr(sCustName)+', '+
...
//            ' word_amount='+QuotedStr(terbilang)+', '+
            ' code_karesidenan='+QuotedStr(kd_kares)+' '+
  ...
  begin
    ...
    if edKodeSumberTagihan.Text='2' then
    UpdateDPP;
    if (edKodeSumberTagihan.Text='2') AND (edKodeJenisBayar.Text='3') then
    UpdateDPPPembayaranCek;
  end;
//  ShowMessage(cbSumberPenerimaan.Text);
...
procedure TFDataPenerimaanBank.InsertDetailUangMuka;
var
  strDescription,KodePelangganUM :String;
  SisaPiutang,Bayar: Currency;
begin
//  if edKodePelangganSumber.Text<>'' then KodePelangganUM:=edKodePelangganSumber.Text;
//  if edKode_Pelanggan.Text<>'' then KodePelangganUM:=edKode_Pelanggan.Text;
  ...
  begin
    ...
    sql.Text:=' SELECT * from ('+
            ' SELECT * from "public"."t_cash_bank_acceptance_down_payment" '+
            ' WHERE "voucher_no"='+QuotedStr(edNoTrans.Text)+' ) a '+
            ' Order By voucher_no desc';
    open;
  end;
  ...
  begin
    ...
    begin
      close;
      sql.clear;
      sql.Text:=' DELETE FROM  "public"."t_cash_bank_acceptance_down_payment" '+
                ' WHERE "voucher_no"='+QuotedStr(edNoTrans.Text)+';';
      ExecSQL;
    end;
  end;

  strDescription:='Pembayaran Uang Muka Penjualan';
  with dm.Qtemp do
  begin
    close;
    sql.clear;
    sql.Text:=' INSERT INTO "public"."t_cash_bank_acceptance_down_payment" ("voucher_no", '+
              ...
              ' '+QuotedStr(StringReplace(FloatToStr(UangMuka),',','.',[]))+', '+
              ' '+QuotedStr(strDescription)+', '+
              ' '+QuotedStr(formatdatetime('yyyy-mm-dd',dtTrans.Date))+','+
...
procedure TFDataPenerimaanBank.MemDetailPiutangAfterPost(DataSet: TDataSet);
begin
  GetKeterangan;
end;

procedure TFDataPenerimaanBank.Save;
var Stradditional_code2,Stradditional_code3,terbilang,sCustCode, sCustName,strIsNoReplacement:String;
begin
...
//  terbilang:= ConvKeHuruf(floattostr(edJumlah.value))+' Rupiah';
  ...
  begin
    ...
    sql.add(' Insert into "public"."t_cash_bank_acceptance" ("created_at", "created_by", '+
            ...
            ' "order_no", "trans_day", "trans_month", "trans_year",sbu_code,code_karesidenan,is_no_replacement) '+
            ' VALUES ( '+
            ' NOW(), '+
            ' '+QuotedStr(Nm)+', '+
            ' '+QuotedStr(edNoTrans.Text)+', '+
            ...
            ' '+QuotedStr(strtgl)+', '+
            ' '+QuotedStr(strbulan)+', '+
            ' '+QuotedStr(strtahun)+','+QuotedStr(FHomeLogin.vKodePRSH)+','+
//            ''+QuotedStr(terbilang)+','+
            ' '+QuotedStr(kd_kares)+', '+
  ...
  begin
    ...
    begin
      ...
      sql.Text:='UPDATE "public"."t_cash_bank_acceptance" SET'+
                ...
                ' Where voucher_no='+QuotedStr(strNoTransaksiDiGanti);
      ExecSQL;
    end;
  end;
...
begin
  ...
  if (edNoRefSumberPenerimaan.Text<>'') AND (kd_kares<>'') then
  begin
    additional_code2:=kd_kares;
  end;   }
  if kd_kares<>'' then
  ...
  begin
    edNoTrans.Text:=getNourut2(strday2,'public.t_cash_bank_acceptance',additional_code1,'0',additional_code3,'0')+'/'+additional_code2+'/'+additional_code3;
  end;
  if (additional_code1<>'') AND (additional_code2<>'') AND (additional_code3='')  then
  begin
...
begin
  ...
  begin
    ...
    FKoreksi.Status:=0;
    FKoreksi.vnotransaksi:=edNoTrans.Text;
    FKoreksi.ShowModal;
  end;
end;
...
var
  Year, Month, Day: Word;
  FoundUangMuka: Integer;
  TsUangMuka: Currency;
begin
  UangMuka:=0;
  //Rferesh hasil Kurs, cek posisi DK
//  if (cbJenisTransaksi.Text='PIUTANG') AND (MemDetailPiutang.RecordCount>0) then
//  begin
//    GetKeterangan;
//  end;
  FoundUangMuka:=0;
  ...
  while not MemDetailAkun.Eof do
  begin
    if MemDetailAkun['kd_akun'] = '2102.08' then
    begin
      FoundUangMuka:=1;
      UangMuka:=MemDetailAkun['kredit'];
    end;
  ...
  if FoundUangMuka=0 then
  begin
    edNoRefSumberPenerimaan.Text := '';
    cbSumberPenerimaan.Text := '';
    edKodePelangganSumber.Text:='';
    if (cbJenisTransaksi.Text='PIUTANG') then
    begin
      edNamaPelangganSumber.Text:='';
    end;
  end;
  ...
  begin
    ...
    begin
      close;
      sql.Clear;
      sql.Text:='SELECT grand_tot FROM t_down_payment_sales WHERE no_trans='+QuotedStr(edNoRefSumberPenerimaan.Text)+' AND deleted_at IS NULL';
      open;
    end;
    TsUangMuka:=dm.Qtemp3.FieldValues['grand_tot'];
  end;
...
//      end
    ...
    else if (MemDetailPiutang.RecordCount=0) and (Length(edKode_Pelanggan.Text)<>0) and (cbJenisTransaksi.Text='PIUTANG') then
    begin
      MessageDlg('Pastikan Detail Piutang Sudah Lengkap..!!',mtInformation,[mbRetry],0);
    end
    else if (cbSumberPenerimaan.Text='UANG MUKA PENJUALAN') AND (edNoRefSumberPenerimaan.Text='') then
    begin
      MessageDlg('No Uang Muka Wajib Diisi..!!',mtInformation,[mbRetry],0);
    end
    else if (cbSumberPenerimaan.Text='UANG MUKA PENJUALAN') AND (edNoRefSumberPenerimaan.Text<>'') AND (UangMuka<>TsUangMuka) then
    begin
      MessageDlg('Jumlah Penerimaan Harus Sama Dengan Uang Muka..!!',mtInformation,[mbRetry],0);
    end
    else if MemKeterangan.Text='' then
    begin
      MessageDlg('Keterangan Wajib Diisi..!!',mtInformation,[mbRetry],0);
    end
    else if FDataPenerimaanBank.Status = 0 then
    begin
      if chkNomorPengganti.Checked=False then FDataPenerimaanBank.Autonumber;
...
procedure TFDataPenerimaanBank.btnNoTransaksiClick(Sender: TObject);
var Stradditional_code3: String;
begin
  ...
  if (vid_modul='3') AND (edNoRek.Text='') then
  begin
    MessageDlg('No Rekening Wajib Diisi..!!',mtInformation,[mbRetry],0);
    Exit;
  end else
  begin
    Stradditional_code3:='additional_code3 IS NULL';
    if vid_modul='3' then
    begin
      additional_code3:=SelectRow('select bank_code from t_bank where rekening_no='+QuotedStr(edNoRek.Text)+' ');
      Stradditional_code3:='additional_code3='+QuotedStr(additional_code3);
    end;
    FListNoTransaksi.vcall:='penerimaanbankkas';
    FListNoTransaksi.update_grid('voucher_no','order_no','t_cash_bank_acceptance','WHERE trans_date='+QuotedStr(formatdatetime('yyyy-mm-dd',dtTrans.Date))+' and deleted_at is NOT NULL '+
    'AND (trans_no_replacement is NULL OR trans_no_replacement='''') '+
    'AND additional_code='+QuotedStr(additional_code1)+' AND '+Stradditional_code3+' ORDER BY voucher_no ASC');
    FListNoTransaksi.ShowModal;
  end;
end;

procedure TFDataPenerimaanBank.SimpanPelanggan;
var KodePelanggan: String;
begin
  KodePelanggan:='';
  if (MemDetailPiutang.RecordCount>0) then
  begin
    KodePelanggan:=edKode_Pelanggan.Text;
  end;
  if (cbSumberPenerimaan.Text='UANG MUKA PENJUALAN') AND (edNoRefSumberPenerimaan.Text<>'') then
  begin
    KodePelanggan:=edKodePelangganSumber.Text;
  end;
  ...
  begin
    close;
    sql.Clear;
    sql.Text:='UPDATE t_cash_bank_acceptance_customer SET deleted_at=NOW() where trans_no ='+QuotedStr(edNoTrans.Text) ;
    ExecSQL;
  end;
  ...
  begin
    ...
    sql.Text := 'INSERT INTO t_cash_bank_acceptance_customer (' +
               ...
               'cust_type_code_tax, cust_type_name_tax, country_code_tax, country_name_tax, no_nitku, no_passport,va_name,company_code_bank ' +
                'FROM t_customer ' +
                'WHERE customer_code = '+QuotedStr(KodePelanggan)+' ';
    ExecSQL;
  end;
end;
...
procedure TFDataPenerimaanBank.cbSumberPenerimaanChange(Sender: TObject);
begin
  if cbSumberPenerimaan.Text='TIDAK ADA SUMBER' then
  begin
    label33.Visible:=False;
    label34.Visible:=False;
    edNamaPKPSumber.Visible:=False;
    ...
    edJumlah.value:=0.00;
    edKaresidenan.Enabled:=True;
  end else if cbSumberPenerimaan.Text='UANG MUKA PENJUALAN' then
  begin
    ...
    edUntukPengiriman.Text:='UANG MUKA PENJUALAN '+edNamaPKPSumber.Text;
...
begin
  ...
  edNamaPelangganSumber.Clear;
  edNoRefSumberPenerimaan.Clear;
  edNamaPKPSumber.Clear;
  chkNomorPengganti.Checked:=False;
  btnNoTransaksi.Visible:=False;
  strNoTransaksiDiGanti:='';

  RefreshSumberPenerimaan;
end;
...
begin
  ...
  begin
    ...
    begin
      FDaftarTagihan.vcall:='edit_terima_bank';
      FDaftarTagihan.kd_outlet:=edKode_Pelanggan.Text;
      FDaftarTagihan.RefreshGrid;
      FDaftarTagihan.show;
    end;
    if edKodeSumberTagihan.Text='2' then
    begin
      FDaftarPenagihanPiutang.vcall:='edit_PenagihanPiutang';
      FDaftarPenagihanPiutang.kd_outlet:=edKode_Pelanggan.Text;
      FDaftarPenagihanPiutang.tglTagih1:=dtPeriode1.Date;
      FDaftarPenagihanPiutang.tglTagih2:=dtPeriode2.Date;
      FDaftarPenagihanPiutang.edKode_Pelanggan.Text:=edKode_Pelanggan.Text;
  ...
  if SelectRow('select value_parameter from t_parameter where key_parameter=''sumber_terima_bank'' ')= '1' then
  begin
    FDaftarRencanaLunasPiutang.vcall:='edit_rencana_lunas_piutang';
    FDaftarRencanaLunasPiutang.kd_outlet:=edKode_Pelanggan.Text;
    FDaftarRencanaLunasPiutang.periode1:=dtPeriode1.Date;
    FDaftarRencanaLunasPiutang.periode2:=dtPeriode2.Date;
    FDaftarRencanaLunasPiutang.edKode_Pelanggan.Text:=edKode_Pelanggan.Text;
...
procedure TFDataPenerimaanBank.DBGridTagihanExit(Sender: TObject);
begin
  GetKeterangan;
end;
...
procedure TFDataPenerimaanBank.edKaresidenanButtonClick(Sender: TObject);
begin
  if cbJenisTransaksi.Text='NON PIUTANG' then
  begin
    FMasterData.Caption:='Master Data Karesidenan';
    FMasterData.vcall:='penerimaankas_bank_kares';
    FMasterData.update_grid('code','name','description','t_region_karesidenan','WHERE	deleted_at IS NULL');
    FMasterData.ShowModal;
  end;
end;

procedure TFDataPenerimaanBank.edKodePelangganSumberChange(Sender: TObject);
begin
  if edKodePelangganSumber.Text<>'' then
  begin
    with dm.Qtemp3 do
    begin
      Close;
      Sql.Clear;
      SQl.Text:='SELECT COALESCE(code_karesidenan,'''') code_karesidenan,COALESCE(karesidenan,'''') karesidenan FROM get_customer() WHERE customer_code='+QuotedStr(edKodePelangganSumber.Text)+' ';
      Open;
    end;
    if dm.Qtemp3.FieldValues['karesidenan']='' then
    begin
      edNamaPelangganSumber.Text:='';
      edNamaPKPSumber.Text:='';
      edKodePelangganSumber.Text:='';
      ...
      kd_kares:=dm.Qtemp3.FieldValues['code_karesidenan'];
      edKaresidenan.Text:=dm.Qtemp3.FieldValues['karesidenan'];
...
begin
  if (edKode_Pelanggan.Text<>'') AND (chkNomorPengganti.Checked=False) then
  begin
    with dm.Qtemp1 do
    begin
      Close;
      Sql.Clear;
      SQl.Text:='SELECT COALESCE(code_karesidenan,'''') code_karesidenan,COALESCE(karesidenan,'''') karesidenan FROM get_customer() WHERE customer_code='+QuotedStr(edKode_Pelanggan.Text)+' ';
      Open;
    end;
    ...
    begin
      ...
      edKaresidenan.Text:=dm.Qtemp1.FieldValues['karesidenan'];
    end;
//    RefreshGridDetailAkun;
  end;
end;
...
procedure TFDataPenerimaanBank.edNoRefSumberPenerimaanButtonClick(Sender: TObject);
begin
  if cbSumberPenerimaan.Text='UANG MUKA PENJUALAN' then
  begin
    FbrowseUangMukaPenjualan.QListUangMuka.Close;
    FbrowseUangMukaPenjualan.ShowModal;
  end;
end;

procedure TFDataPenerimaanBank.edNoRekChange(Sender: TObject);
begin
  if (chkNomorPengganti.Checked=True) AND (Status=0) then edNoTrans.Text:='';
end;

procedure TFDataPenerimaanBank.edNoTransChange(Sender: TObject);
begin
  if (chkNomorPengganti.Checked=True) AND (edNoTrans.Text<>'') then
  begin
//    if cbJenisTransaksi.Text='PIUTANG' then
//    begin
//       kd_kares:=SelectRow('SELECT code_karesidenan from t_cash_bank_acceptance WHERE voucher_no='+QuotedStr(strNoTransaksiDiGanti)+';');
//    end;
//    edKode_Pelanggan.Text:='';
//    edNama_Pelanggan.Text:='';
//    edNamaPKP.Text:='';
...
//    edKaresidenan.Text:='';
    ...
    begin
      close;
      sql.Clear;
      sql.Text:='SELECT a.code_karesidenan, b.name karesidenan from t_cash_bank_acceptance a '+
...
begin
  ...
  begin
    ...
    dtTrans.Enabled := False;
    if cbJenisTransaksi.Text='NON PIUTANG' then edKaresidenan.Enabled := True
    else edKaresidenan.Enabled := False;
    btnNoTransaksi.Visible:=False;
