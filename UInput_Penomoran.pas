...
function TFInput_Penomoran.GiveZero(const aNumber, aMaxDigit: Integer): String;
var
...
function TFInput_Penomoran.Terbilang(sValue: string):string;
const
...
function TFInput_Penomoran.ConvKeHuruf(inp: string): string;
var
...
procedure TFInput_Penomoran.Ed_KompChange(Sender: TObject);
begin
...
procedure TFInput_Penomoran.eddigit_countChange(Sender: TObject);
var
  i:integer;
begin   i := 00000000;
   Inc(i);
...
{var aNumber: Integer;
begin
    if (TryStrToInt(eddigit_count.Text, aNumber)) then
    Edbut_counter.Text := GiveZero(aNumber,5)
    else
      ShowMessage('Value in Edit1 is not a valid integer value');
end;}

procedure TFInput_Penomoran.btnplus2Click(Sender: TObject);
var
  B: TrzButtonEdit;
begin
...
procedure TFInput_Penomoran.btnplusClick(Sender: TObject);
var
  B: TrzButtonEdit;
  i:integer;
begin
...
procedure TFInput_Penomoran.Button1Click(Sender: TObject);
begin
   LabelA.Caption:=Terbilang(Edit1.Text);
end;

procedure TFInput_Penomoran.Button2Click(Sender: TObject);
begin
