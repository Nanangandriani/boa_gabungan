...
begin
  close;
  sql.Clear;
  sql.Text:='Update t_result_formula set nospk='+QuotedStr(EdNo_spk.Text)+','+
            ' noformula='+QuotedStr(EdNo_Formula.Text)+','+
            ' nm_produk='+QuotedStr(EdProduk.Text)+','+
            ' kesimpulan='+QuotedStr(EdKesimpulan.Text)+','+
	          ' persetujuan='+QuotedStr(edPersetujuan.Text)+','+
            ' tanggal='+QuotedStr(FormatDateTime('mm-dd-yyy',DtTrial.date))+','+
            ' ket='+QuotedStr(Edket.Text)+', tahun='+QuotedStr(yr)+','+
            ' kd_sbu='+QuotedStr(loksbu)+''+
            ' where noresult='+QuotedStr(EditNo);
  ExecSQL;
end;
with dm.Qtemp2 do
begin
  close;
  sql.Clear;
  sql.Text:='Delete from t_result_formula_det where noresult='+QuotedStr(EditNo);
  ExecSQL;
end;
...
begin
  close;
  sql.Clear;
  sql.Text:='insert into t_result_formula_det(noresult,kode,deskripsi,standard,trial)values'+
...
begin
  close;
  sql.Clear;
  sql.Text:='insert into t_result_formula(noresult,nospk,noformula,nm_produk,kesimpulan,'+
	          ' persetujuan,tanggal,ket,tahun,kd_sbu,bln,tgl_no,nourut)values('+QuotedStr(EditNo)+','+
