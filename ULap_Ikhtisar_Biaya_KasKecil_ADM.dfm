object FLap_Ikhtisar_Biaya_KasKecil_ADM: TFLap_Ikhtisar_Biaya_KasKecil_ADM
  Left = 0
  Top = 0
  Caption = 'FLap_Ikhtisar_Biaya_KasKecil_ADM'
  ClientHeight = 442
  ClientWidth = 1399
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  TextHeight = 15
  object DBGridKasKecilADM: TDBGridEh
    Left = 0
    Top = 127
    Width = 1399
    Height = 274
    Align = alClient
    Color = clBtnFace
    DataGrouping.GroupLevels = <
      item
      end
      item
      end>
    DataSource = DSKasKecilADM
    DrawMemoText = True
    DynProps = <>
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
    SearchPanel.Enabled = True
    TabOrder = 0
    TitleParams.MultiTitle = True
    OnGetCellParams = DBGridKasKecilADMGetCellParams
    Columns = <
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'actors_name'
        Footers = <>
        Title.Caption = 'Pelaku'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'voucher_no'
        Footers = <>
        Title.Caption = 'No. Bukti'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'description'
        Footers = <>
        Title.Caption = 'Keterangan'
        Width = 150
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'total'
        Footers = <>
        Title.Caption = 'Total'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b1'
        Footers = <>
        Title.Caption = 'Sumbangan'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b2'
        Footers = <>
        Title.Caption = 'Asuransi'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b3'
        Footers = <>
        Title.Caption = 'Jamuan Tamu'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b4'
        Footers = <>
        Title.Caption = 'Perjalanan Dinas'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b5'
        Footers = <>
        Title.Caption = 'Alat Tulis Dan Foto Copy'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b6'
        Footers = <>
        Title.Caption = 'Listrik dan Air'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b7'
        Footers = <>
        Title.Caption = 'PTT dan Materai'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b8'
        Footers = <>
        Title.Caption = 'M/R Alat Kantor'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b9'
        Footers = <>
        Title.Caption = 'Pendidikan Karyawan'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b10'
        Footers = <>
        Title.Caption = 'Paajak Daerah dan Perijinan'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b11'
        Footers = <>
        Title.Caption = 'Keperluan Kantor'
        Width = 100
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'b12'
        Footers = <>
        Title.Caption = 'Humas'
      end>
    object RowDetailData: TRowDetailPanelControlEh
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 401
    Width = 1399
    Height = 41
    Align = alBottom
    TabOrder = 1
    Visible = False
    ExplicitTop = 392
    ExplicitWidth = 855
    object BBatal: TRzBitBtn
      Left = 1323
      Top = 1
      Height = 39
      Align = alRight
      Caption = 'Batal'
      TabOrder = 0
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 779
    end
    object BPrint: TRzBitBtn
      Left = 1244
      Top = 1
      Width = 79
      Height = 39
      Align = alRight
      Caption = 'Print'
      TabOrder = 1
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 700
    end
  end
  object dxRibbon1: TdxRibbon
    Left = 0
    Top = 0
    Width = 1399
    Height = 127
    BarManager = dxBarManager1
    Style = rs2010
    ColorSchemeName = 'Blue'
    Contexts = <>
    TabOrder = 2
    TabStop = False
    ExplicitWidth = 861
    object dxRibbon1Tab1: TdxRibbonTab
      Active = True
      Caption = 'Home'
      Groups = <
        item
          ToolbarName = 'dxBarManager1Bar1'
        end>
      Index = 0
    end
  end
  object DataSetDriverEh1: TDataSetDriverEh
    ProviderDataSet = QKasKecilADM
    Left = 176
    Top = 272
  end
  object frxReport1: TfrxReport
    Version = '2022.1.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick, pbCopy, pbSelection]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 44805.841268819400000000
    ReportOptions.LastChange = 46031.581273958330000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'procedure Memo21OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      ''
      'end;'
      ''
      'begin'
      ''
      'end.')
    Left = 734
    Top = 27
    Datasets = <
      item
        DataSet = frxDBDatasetPers
        DataSetName = 'frxDBDatasetPers'
      end
      item
        DataSet = frxDBDatasetADM2
        DataSetName = 'frxDBDatasetADM2'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 330.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 10000
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
      MirrorMode = []
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 171.456692920000000000
        Top = 18.897650000000000000
        Width = 1209.449600000000000000
        object Shape1: TfrxShapeView
          AllowVectorExport = True
          Top = 114.377952760000000000
          Width = 1209.448818900000000000
          Height = 57.078740160000000000
          Frame.Typ = []
        end
        object Memo50: TfrxMemoView
          AllowVectorExport = True
          Top = 12.094488190000000000
          Width = 1247.244094490000000000
          Height = 48.236240000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'IKHTISAR PENGGUNAAN DANA KAS KECIL '
            'BIAYA ADMINISTRASI DAN UMUM')
          ParentFont = False
        end
        object SysMemo8: TfrxSysMemoView
          AllowVectorExport = True
          Left = 1090.094930000000000000
          Top = 5.984230000000000000
          Width = 110.574830000000000000
          Height = 26.779530000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            'Hal. [PAGE#] dari [TOTALPAGES#]')
          ParentFont = False
        end
        object Memo1: TfrxMemoView
          AllowVectorExport = True
          Top = 12.204700000000000000
          Width = 354.488250000000000000
          Height = 18.897650000000000000
          DataField = 'company_name'
          DataSet = frxDBDatasetPers
          DataSetName = 'frxDBDatasetPers'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetPers."company_name"]')
          ParentFont = False
        end
        object Memo53: TfrxMemoView
          AllowVectorExport = True
          Top = 31.204700000000000000
          Width = 418.433070870000000000
          Height = 41.574830000000000000
          StretchMode = smMaxHeight
          DataField = 'address'
          DataSet = frxDBDatasetPers
          DataSetName = 'frxDBDatasetPers'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetPers."address"]')
          ParentFont = False
        end
        object Memo54: TfrxMemoView
          AllowVectorExport = True
          Top = 115.133858270000000000
          Width = 30.236220470000000000
          Height = 55.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'NO.')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo55: TfrxMemoView
          AllowVectorExport = True
          Left = 29.102362200000000000
          Top = 114.133858270000000000
          Width = 87.307086614173200000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'PELAKU')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memoperiode: TfrxMemoView
          AllowVectorExport = True
          Top = 70.102350000000000000
          Width = 1247.244094490000000000
          Height = 30.236240000000000000
          DisplayFormat.FormatStr = 'dd mmm yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Tanggal :')
          ParentFont = False
        end
        object Memo28: TfrxMemoView
          AllowVectorExport = True
          Left = 1151.244094488190000000
          Top = 112.133858270000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Keperluan Kantor')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo30: TfrxMemoView
          AllowVectorExport = True
          Left = 116.787401570000000000
          Top = 113.566929130000000000
          Width = 75.590551181102400000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'No. Bukti')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo32: TfrxMemoView
          AllowVectorExport = True
          Left = 192.000000000000000000
          Top = 113.566929130000000000
          Width = 112.629921259843000000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Keterangan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo31: TfrxMemoView
          AllowVectorExport = True
          Left = 304.629921260000000000
          Top = 116.133858270000000000
          Width = 56.692913390000000000
          Height = 54.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Total')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo33: TfrxMemoView
          AllowVectorExport = True
          Left = 360.566929133858000000
          Top = 117.133858270000000000
          Width = 56.692913385826800000
          Height = 53.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Gaji')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo34: TfrxMemoView
          AllowVectorExport = True
          Left = 416.125984251969000000
          Top = 115.133858270000000000
          Width = 56.692913385826800000
          Height = 55.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Sumbangan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo35: TfrxMemoView
          AllowVectorExport = True
          Left = 531.779527559055000000
          Top = 115.566929130000000000
          Width = 56.692913385826800000
          Height = 55.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'M/R Alat Kantor')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo36: TfrxMemoView
          AllowVectorExport = True
          Left = 588.472440944882000000
          Top = 114.566929130000000000
          Width = 56.692913385826800000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Alat Tulis dan Fotocopy')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo37: TfrxMemoView
          AllowVectorExport = True
          Left = 644.787401574803000000
          Top = 114.566929130000000000
          Width = 56.692913385826800000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Listrik dan Air')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo39: TfrxMemoView
          AllowVectorExport = True
          Left = 700.724409448819000000
          Top = 115.511811020000000000
          Width = 56.692913385826800000
          Height = 55.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'PTT dan Materai')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo41: TfrxMemoView
          AllowVectorExport = True
          Left = 757.795275590551000000
          Top = 114.511811020000000000
          Width = 56.692913385826800000
          Height = 56.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Perjalanan Dinas')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo42: TfrxMemoView
          AllowVectorExport = True
          Left = 814.488188980000000000
          Top = 114.511811020000000000
          Width = 56.692913385826800000
          Height = 55.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Pendidikan Karyawan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo43: TfrxMemoView
          AllowVectorExport = True
          Left = 870.803149606299000000
          Top = 114.511811020000000000
          Width = 56.692913385826800000
          Height = 56.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Pajak Daerah dan Perijinan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo47: TfrxMemoView
          AllowVectorExport = True
          Left = 474.330708661417000000
          Top = 114.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Asuransi')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo67: TfrxMemoView
          AllowVectorExport = True
          Left = 925.984251970000000000
          Top = 114.102350000000000000
          Width = 56.692913390000000000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'M/R Kendaraan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo70: TfrxMemoView
          AllowVectorExport = True
          Left = 981.921259842520000000
          Top = 114.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'ADM Bank')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo71: TfrxMemoView
          AllowVectorExport = True
          Left = 1038.992125984250000000
          Top = 113.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Jamuan Tamu')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo72: TfrxMemoView
          AllowVectorExport = True
          Left = 1094.929133858270000000
          Top = 113.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Managment Expense')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 24.692779130000000000
        Top = 249.448980000000000000
        Width = 1209.449600000000000000
        AllowSplit = True
        DataSet = frxDBDatasetADM2
        DataSetName = 'frxDBDatasetADM2'
        RowCount = 0
        object Memo10: TfrxMemoView
          AllowVectorExport = True
          Width = 30.236220470000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DataSetName = 'frxDBDatasetRekapKonfirmasi'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 30.102362200000000000
          Width = 87.307086610000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          Memo.UTF8W = (
            '[frxDBDatasetADM2."actors_name"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo12: TfrxMemoView
          AllowVectorExport = True
          Left = 531.779527560000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b4"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo19: TfrxMemoView
          AllowVectorExport = True
          Left = 304.629921260000000000
          Top = -0.333333340000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."total"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          AllowVectorExport = True
          Left = 116.787401570000000000
          Width = 75.590551180000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetADM2."voucher_no"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo25: TfrxMemoView
          AllowVectorExport = True
          Left = 700.724409450000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b7"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object SysMemo1: TfrxSysMemoView
          AllowVectorExport = True
          Width = 30.236220470000000000
          Height = 22.677180000000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[LINE#]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          AllowVectorExport = True
          Left = 588.472440940000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          OnBeforePrint = 'Memo21OnBeforePrint'
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b5"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo27: TfrxMemoView
          AllowVectorExport = True
          Left = 870.803149610000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b10"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo29: TfrxMemoView
          AllowVectorExport = True
          Left = 1151.244094490000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b12"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo44: TfrxMemoView
          AllowVectorExport = True
          Left = 192.000000000000000000
          Width = 112.629921260000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetADM2."description"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo45: TfrxMemoView
          AllowVectorExport = True
          Left = 360.566929130000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b1"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo46: TfrxMemoView
          AllowVectorExport = True
          Left = 416.125984250000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b2"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo48: TfrxMemoView
          AllowVectorExport = True
          Left = 644.787401574803000000
          Width = 56.692913385826800000
          Height = 24.566929130000000000
          OnBeforePrint = 'Memo21OnBeforePrint'
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b6"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo49: TfrxMemoView
          AllowVectorExport = True
          Left = 474.330708660000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b3"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo51: TfrxMemoView
          AllowVectorExport = True
          Left = 757.795275590000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b8"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo52: TfrxMemoView
          AllowVectorExport = True
          Left = 814.488188980000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b9"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo68: TfrxMemoView
          AllowVectorExport = True
          Left = 925.984251970000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b11"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo73: TfrxMemoView
          AllowVectorExport = True
          Left = 981.921259840000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b11"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo75: TfrxMemoView
          AllowVectorExport = True
          Left = 1038.992125980000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b11"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo77: TfrxMemoView
          AllowVectorExport = True
          Left = 1094.929133860000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b11"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 30.236220470000000000
        Top = 336.378170000000000000
        Width = 1209.449600000000000000
        object Memo16: TfrxMemoView
          AllowVectorExport = True
          Left = 1152.244094490000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b12">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          AllowVectorExport = True
          Width = 305.157480310000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            'Jumlah')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo57: TfrxMemoView
          AllowVectorExport = True
          Left = 361.566929130000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b1">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo58: TfrxMemoView
          AllowVectorExport = True
          Left = 417.125984250000000000
          Width = 55.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b2">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo60: TfrxMemoView
          AllowVectorExport = True
          Left = 530.779527560000000000
          Width = 57.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b4">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo61: TfrxMemoView
          AllowVectorExport = True
          Left = 588.472440940000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b5">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo62: TfrxMemoView
          AllowVectorExport = True
          Left = 644.787401570000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b6">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo63: TfrxMemoView
          AllowVectorExport = True
          Left = 700.724409450000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b7">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo64: TfrxMemoView
          AllowVectorExport = True
          Left = 756.795275590000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b8">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo65: TfrxMemoView
          AllowVectorExport = True
          Left = 815.154855650000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b9">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo66: TfrxMemoView
          AllowVectorExport = True
          Left = 870.803149610000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b10">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo59: TfrxMemoView
          AllowVectorExport = True
          Left = 473.330708660000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b3">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo69: TfrxMemoView
          AllowVectorExport = True
          Left = 925.984251970000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b11">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo56: TfrxMemoView
          AllowVectorExport = True
          Left = 304.629921260000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."total">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo74: TfrxMemoView
          AllowVectorExport = True
          Left = 981.921259840000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b11">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo76: TfrxMemoView
          AllowVectorExport = True
          Left = 1038.992125980000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b11">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo78: TfrxMemoView
          AllowVectorExport = True
          Left = 1094.929133860000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b11">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
    end
  end
  object frxDBDatasetPers: TfrxDBDataset
    UserName = 'frxDBDatasetPers'
    CloseDataSource = False
    FieldAliases.Strings = (
      'company_code=company_code'
      'company_name=company_name'
      'address=address'
      'telp=telp'
      'email=email'
      'npwp=npwp'
      'city=city'
      'address2=address2'
      'id=id'
      'created_at=created_at'
      'created_by=created_by'
      'updated_at=updated_at'
      'updated_by=updated_by'
      'deleted_at=deleted_at'
      'deleted_by=deleted_by'
      'type_of_business=type_of_business'
      'latitude=latitude'
      'longitude=longitude'
      'tax_status=tax_status'
      'currency=currency')
    DataSet = QPerusahaan
    BCDToCurrency = False
    DataSetOptions = []
    Left = 816
    Top = 88
  end
  object QPerusahaan: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      'select * from t_company')
    Left = 776
    Top = 88
    object QPerusahaancompany_code: TStringField
      FieldName = 'company_code'
      Required = True
      Size = 10
    end
    object QPerusahaancompany_name: TStringField
      FieldName = 'company_name'
      Size = 255
    end
    object QPerusahaanaddress: TStringField
      FieldName = 'address'
      Size = 255
    end
    object QPerusahaantelp: TStringField
      FieldName = 'telp'
    end
    object QPerusahaanemail: TStringField
      FieldName = 'email'
      Size = 255
    end
    object QPerusahaannpwp: TStringField
      FieldName = 'npwp'
      Size = 50
    end
    object QPerusahaancity: TStringField
      FieldName = 'city'
      Size = 255
    end
    object QPerusahaanaddress2: TStringField
      FieldName = 'address2'
      Size = 255
    end
    object QPerusahaanid: TGuidField
      FieldName = 'id'
      Required = True
      Size = 38
    end
    object QPerusahaancreated_at: TDateTimeField
      FieldName = 'created_at'
    end
    object QPerusahaancreated_by: TStringField
      FieldName = 'created_by'
      Size = 50
    end
    object QPerusahaanupdated_at: TDateTimeField
      FieldName = 'updated_at'
    end
    object QPerusahaanupdated_by: TStringField
      FieldName = 'updated_by'
      Size = 50
    end
    object QPerusahaandeleted_at: TDateTimeField
      FieldName = 'deleted_at'
    end
    object QPerusahaandeleted_by: TStringField
      FieldName = 'deleted_by'
      Size = 50
    end
    object QPerusahaantype_of_business: TStringField
      FieldName = 'type_of_business'
      Size = 255
    end
    object QPerusahaanlatitude: TStringField
      FieldName = 'latitude'
      Size = 255
    end
    object QPerusahaanlongitude: TStringField
      FieldName = 'longitude'
      Size = 255
    end
    object QPerusahaantax_status: TBooleanField
      FieldName = 'tax_status'
    end
    object QPerusahaancurrency: TStringField
      FieldName = 'currency'
      Size = 10
    end
  end
  object frxDBDataset1: TfrxDBDataset
    UserName = 'frxDBDatasetADM'
    CloseDataSource = False
    FieldAliases.Strings = (
      'actors_name=actors_name'
      'voucher_no=voucher_no'
      'description=description'
      'name=name'
      'total=total'
      'b1=b1'
      'b2=b2'
      'b3=b3'
      'b4=b4'
      'b5=b5'
      'b6=b6'
      'b7=b7'
      'b8=b8'
      'b9=b9'
      'b10=b10'
      'b11=b11'
      'b12=b12')
    DataSet = QCetak_KasKecil_ADM
    BCDToCurrency = False
    DataSetOptions = []
    Left = 672
    Top = 32
  end
  object QCetak_KasKecil_ADM: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'select a.actors_name,a.voucher_no,a.description,a."name",b.total' +
        ','
      '(case when b1.amount is null then 0 else b1.amount end)b1,'
      '(case when b2.amount is null then 0 else b2.amount end)b2, '
      '(case when b3.amount is null then 0 else b3.amount end)b3,'
      '(case when b4.amount is null then 0 else b4.amount end)b4,'
      '(case when b5.amount is null then 0 else b5.amount end)b5, '
      '(case when b6.amount is null then 0 else b6.amount end)b6,'
      '(case when b7.amount is null then 0 else b7.amount end)b7,'
      '(case when b8.amount is null then 0 else b8.amount end)b8,'
      '(case when b9.amount is null then 0 else b9.amount end)b9,'
      '(case when b10.amount is null then 0 else b10.amount end)b10,'
      '(case when b11.amount is null then 0 else b11.amount end)b11,'
      '(case when b12.amount is null then 0 else b12.amount end)b12 '
      'from  '
      
        '(select distinct  a.voucher_no  from t_petty_cash a INNER JOIN t' +
        '_petty_cash_det b ON a.voucher_no=b.voucher_no where trans_date ' +
        'between '#39'2025-01-02'#39' and '#39'2025-01-02'#39' and code_account='#39'1112'#39'  a' +
        'nd position='#39'K'#39')xx '
      
        'left join (select distinct a.actors_name,a.voucher_no,a.code_acc' +
        'ount,a.trans_date,a.description,a.order_no,b.code,c."name" from ' +
        '(SELECT a.voucher_no,a.actors_name,a.trans_date,a.description,b.' +
        'code_account,a.order_no,a.actors_code from t_petty_cash a INNER ' +
        'JOIN t_petty_cash_det b ON a.voucher_no=b.voucher_no) a,t_ak_acc' +
        'ount b,t_cost_actors c where a.code_account=b.code and a.actors_' +
        'code=c.code and b.type_id=2 and trans_date between '#39'2025-01-02'#39' ' +
        'and '#39'2025-01-02'#39' order by voucher_no)a on a.voucher_no=xx.vouche' +
        'r_no'
      '   '
      
        'left join (select voucher_no,a.code_account,a."position",sum(amo' +
        'unt)as total from (select a.voucher_no,b.code_account,b."positio' +
        'n",a.amount FROM t_petty_cash a INNER JOIN t_petty_cash_det b ON' +
        ' a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.code_account,' +
        'b."position") a,t_ak_account b where a.code_account=b.code and b' +
        '.type_id=2 and a."position"='#39'D'#39'  group by voucher_no,a.code_acco' +
        'unt,a."position" order by voucher_no)b on a.voucher_no=b.voucher' +
        '_no'
      '   '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'SBG'#39')b1 on a.voucher_no=b1.voucher_' +
        'no'
      '     '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'ASR'#39')b2 on a.voucher_no=b2.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'JMT'#39')b3 on a.voucher_no=b3.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'PD2'#39')b4 on a.voucher_no=b4.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'ATK'#39')b5 on a.voucher_no=b5.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'LNA'#39')b6 on a.voucher_no=b6.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'PTM'#39')b7 on a.voucher_no=b7.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'MRA'#39')b8 on a.voucher_no=b8.voucher_' +
        'no'
      '     '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'PDK'#39')b9 on a.voucher_no=b9.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'PDP'#39')b10 on a.voucher_no=b10.vouche' +
        'r_no'
      '    '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'KPK'#39')b11 on a.voucher_no=b11.vouche' +
        'r_no'
      '    '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'HMS'#39')b12 on a.voucher_no=b12.vouche' +
        'r_no    '
      
        'where a.voucher_no is not null order by a.trans_date,a.voucher_n' +
        'o,a.order_no;')
    Left = 624
    Top = 32
  end
  object dxBarManager1: TdxBarManager
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Style = []
    Categories.Strings = (
      'Default')
    Categories.ItemsVisibles = (
      2)
    Categories.Visibles = (
      True)
    PopupMenuLinks = <>
    UseSystemFont = True
    Left = 828
    Top = 32
    PixelsPerInch = 96
    object dxBarManager1Bar1: TdxBar
      Caption = 'Action'
      CaptionButtons = <>
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 805
      FloatTop = 8
      FloatClientWidth = 175
      FloatClientHeight = 225
      ItemLinks = <
        item
          UserDefine = [udWidth]
          UserWidth = 103
          Visible = True
          ItemName = 'txtkdacckredit1'
        end
        item
          UserDefine = [udWidth]
          UserWidth = 104
          Visible = True
          ItemName = 'DTPick1'
        end
        item
          BeginGroup = True
          UserDefine = [udWidth]
          UserWidth = 103
          Visible = True
          ItemName = 'dxBarEdit3'
        end
        item
          Visible = True
          ItemName = 'DTPick2'
        end
        item
          Visible = True
          ItemName = 'DxRefresh'
        end
        item
          BeginGroup = True
          Visible = True
          ItemName = 'dxPrint'
        end>
      OneOnRow = True
      Row = 0
      UseOwnFont = False
      Visible = True
      WholeRow = False
    end
    object dxBUpdate: TdxBarButton
      Caption = 'Update  '
      Category = 0
      Visible = ivNever
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFCF7ECFFEDC36AFFF1D08BFFFFFFFFFFFFFFFFFF}
    end
    object dxBDelete: TdxBarButton
      Caption = 'Delete  '
      Category = 0
      Visible = ivNever
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFBFBFFFFF0C0CFFFFABABFFFFFFFFFFFFFFFFFFFF}
    end
    object dxBbaru: TdxBarLargeButton
      Caption = 'Baru  '
      Category = 0
      Visible = ivNever
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        FFFFFFFFFFFF}
    end
    object dxBarSubItem1: TdxBarSubItem
      Caption = 'Print  '
      Category = 0
      Enabled = False
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      ItemLinks = <
        item
          Visible = True
          ItemName = 'BProduksi'
        end
        item
          Visible = True
          ItemName = 'BNonProduksi'
        end>
    end
    object BProduksi: TdxBarButton
      Caption = 'Barang Produksi'
      Category = 0
      Hint = 'Barang Produksi'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object BNonProduksi: TdxBarButton
      Caption = 'Barang Non Produksi'
      Category = 0
      Hint = 'Barang Non Produksi'
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object BClosed: TdxBarButton
      Caption = 'CLose Kontrak    '
      Category = 0
      Enabled = False
      Visible = ivAlways
      Glyph.SourceDPI = 96
      Glyph.Data = {
        
        F7FFF7F7F7FF}
    end
    object dxPrint: TdxBarLargeButton
      Caption = 'Print'
      Category = 0
      Hint = 'Print'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        }
      OnClick = dxPrintClick
    end
    object DxRefresh: TdxBarLargeButton
      Caption = 'Cari'
      Category = 0
      Hint = 'Cari'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        2F673E0D0A09093C2F673E0D0A093C2F673E0D0A3C2F7376673E0D0A}
      OnClick = DxRefreshClick
    end
    object DtMulai: TcxBarEditItem
      Caption = 'Tanggal Awal   '
      Category = 0
      Hint = 'Tanggal Awal   '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object DtSelesai: TcxBarEditItem
      Caption = 'Tanggal Akhir  '
      Category = 0
      Hint = 'Tanggal Akhir  '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object CbSupplier: TcxBarEditItem
      Caption = 'Supplier'
      Category = 0
      Hint = 'Supplier'
      Visible = ivAlways
      Width = 150
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
    end
    object CbBarang: TcxBarEditItem
      Caption = 'Barang  '
      Category = 0
      Hint = 'Barang  '
      Visible = ivAlways
      PropertiesClassName = 'TcxButtonEditProperties'
      Properties.Buttons = <
        item
          Default = True
          Kind = bkEllipsis
        end>
    end
    object Cxppn: TcxBarEditItem
      Caption = 'PPN'
      Category = 0
      Hint = 'PPN'
      Visible = ivAlways
      ShowCaption = True
      Width = 0
      PropertiesClassName = 'TcxCheckBoxProperties'
      Properties.ImmediatePost = True
      InternalEditValue = False
    end
    object BtnClear: TdxBarLargeButton
      Caption = 'Clear'
      Category = 0
      Hint = 'Clear'
      Visible = ivAlways
      LargeGlyph.SourceDPI = 96
      LargeGlyph.Data = {
        
        B8F198F83F1812900DC3DC08450000000049454E44AE426082}
    end
    object txtkdacckredit1: TdxBarEdit
      Caption = 'Kode Akun         : '
      Category = 0
      Hint = 'Kode Akun         : '
      Visible = ivAlways
      Text = '1101.02'
    end
    object dxBarEdit3: TdxBarEdit
      Caption = 'Nama Akun    :   '
      Category = 0
      Hint = 'Nama Akun    :   '
      Visible = ivAlways
      Text = 'KAS KECIL'
    end
    object dxBarEdit2: TdxBarEdit
      Caption = 'Tanggal'
      Category = 0
      Hint = 'Tanggal'
      Visible = ivAlways
    end
    object dxBarEdit4: TdxBarEdit
      Category = 0
      Visible = ivNever
    end
    object cxBarEditItem1: TcxBarEditItem
      Category = 0
      Visible = ivNever
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object DTPick1: TcxBarEditItem
      Caption = 'Tanggal Awal     : '
      Category = 0
      Hint = 'Tanggal Awal     : '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
    object DTPick2: TcxBarEditItem
      Caption = 'Tanggal Akhir :   '
      Category = 0
      Hint = 'Tanggal Akhir :   '
      Visible = ivAlways
      PropertiesClassName = 'TcxDateEditProperties'
    end
  end
  object ActMenu: TActionManager
    Left = 791
    Top = 31
    StyleName = 'Platform Default'
    object ActBaru: TAction
      Caption = 'Baru  '
    end
    object ActUpdate: TAction
      Caption = 'Update  '
    end
    object ActRO: TAction
      Caption = 'Refresh  '
    end
    object ActDel: TAction
      Caption = 'Delete'
    end
    object ActPrint: TAction
      Caption = 'Print  '
    end
    object ActApp: TAction
      Caption = 'Approve  '
      Enabled = False
    end
    object ActReject: TAction
      Caption = 'Reject  '
      Enabled = False
    end
    object ActClose: TAction
      Caption = 'CLose PO    '
      Enabled = False
    end
  end
  object MemKasKecilADM: TMemTableEh
    Params = <>
    DataDriver = DataSetDriverEh1
    Left = 168
    Top = 216
  end
  object QKasKecilADM: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'select a.actors_name,a.voucher_no,a.description,a."name",b.total' +
        ','
      '(case when b1.amount is null then 0 else b1.amount end)b1,'
      '(case when b2.amount is null then 0 else b2.amount end)b2, '
      '(case when b3.amount is null then 0 else b3.amount end)b3,'
      '(case when b4.amount is null then 0 else b4.amount end)b4,'
      '(case when b5.amount is null then 0 else b5.amount end)b5, '
      '(case when b6.amount is null then 0 else b6.amount end)b6,'
      '(case when b7.amount is null then 0 else b7.amount end)b7,'
      '(case when b8.amount is null then 0 else b8.amount end)b8,'
      '(case when b9.amount is null then 0 else b9.amount end)b9,'
      '(case when b10.amount is null then 0 else b10.amount end)b10,'
      '(case when b11.amount is null then 0 else b11.amount end)b11,'
      '(case when b12.amount is null then 0 else b12.amount end)b12 '
      'from  '
      
        '(select distinct  a.voucher_no  from t_petty_cash a INNER JOIN t' +
        '_petty_cash_det b ON a.voucher_no=b.voucher_no where trans_date ' +
        'between '#39'2025-01-02'#39' and '#39'2025-01-02'#39' and code_account='#39'1112'#39'  a' +
        'nd position='#39'K'#39')xx '
      
        'left join (select distinct a.actors_name,a.voucher_no,a.code_acc' +
        'ount,a.trans_date,a.description,a.order_no,b.code,c."name" from ' +
        '(SELECT a.voucher_no,a.actors_name,a.trans_date,a.description,b.' +
        'code_account,a.order_no,a.actors_code from t_petty_cash a INNER ' +
        'JOIN t_petty_cash_det b ON a.voucher_no=b.voucher_no) a,t_ak_acc' +
        'ount b,t_cost_actors c where a.code_account=b.code and a.actors_' +
        'code=c.code and b.type_id=2 and trans_date between '#39'2025-01-02'#39' ' +
        'and '#39'2025-01-02'#39' order by voucher_no)a on a.voucher_no=xx.vouche' +
        'r_no'
      '   '
      
        'left join (select voucher_no,a.code_account,a."position",sum(amo' +
        'unt)as total from (select a.voucher_no,b.code_account,b."positio' +
        'n",a.amount FROM t_petty_cash a INNER JOIN t_petty_cash_det b ON' +
        ' a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.code_account,' +
        'b."position") a,t_ak_account b where a.code_account=b.code and b' +
        '.type_id=2 and a."position"='#39'D'#39'  group by voucher_no,a.code_acco' +
        'unt,a."position" order by voucher_no)b on a.voucher_no=b.voucher' +
        '_no'
      '   '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'SBG'#39')b1 on a.voucher_no=b1.voucher_' +
        'no'
      '     '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'ASR'#39')b2 on a.voucher_no=b2.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'JMT'#39')b3 on a.voucher_no=b3.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'PD2'#39')b4 on a.voucher_no=b4.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'ATK'#39')b5 on a.voucher_no=b5.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'LNA'#39')b6 on a.voucher_no=b6.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'PTM'#39')b7 on a.voucher_no=b7.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'MRA'#39')b8 on a.voucher_no=b8.voucher_' +
        'no'
      '     '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'PDK'#39')b9 on a.voucher_no=b9.voucher_' +
        'no'
      '      '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'PDP'#39')b10 on a.voucher_no=b10.vouche' +
        'r_no'
      '    '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'KPK'#39')b11 on a.voucher_no=b11.vouche' +
        'r_no'
      '    '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=2 and b.category_code='#39'HMS'#39')b12 on a.voucher_no=b12.vouche' +
        'r_no    '
      
        'where a.voucher_no is not null order by a.trans_date,a.voucher_n' +
        'o,a.order_no;')
    Left = 104
    Top = 208
  end
  object DSKasKecilADM: TDataSource
    AutoEdit = False
    DataSet = MemKasKecilADM
    Left = 104
    Top = 264
  end
  object frxDBDataset2: TfrxDBDataset
    UserName = 'frxDBDataset1'
    CloseDataSource = False
    FieldAliases.Strings = (
      'actors_name=actors_name'
      'voucher_no=voucher_no'
      'description=description'
      'name=name'
      'total=total'
      'b1=b1'
      'b2=b2'
      'b3=b3'
      'b4=b4'
      'b5=b5'
      'b6=b6'
      'b7=b7'
      'b8=b8'
      'b9=b9'
      'b10=b10'
      'b11=b11')
    DataSet = QCetak_KasKecil_BOP
    BCDToCurrency = False
    DataSetOptions = []
    Left = 672
    Top = 88
  end
  object QCetak_KasKecil_BOP: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'select a.actors_name,a.voucher_no,a.description,a."name",b.total' +
        ','
      '(case when b1.amount is null then 0 else b1.amount end)b1,'
      '(case when b2.amount is null then 0 else b2.amount end)b2,  '
      '(case when b3.amount is null then 0 else b3.amount end)b3,'
      '(case when b4.amount is null then 0 else b4.amount end)b4,'
      '(case when b5.amount is null then 0 else b5.amount end)b5, '
      '(case when b6.amount is null then 0 else b6.amount end)b6,'
      '(case when b7.amount is null then 0 else b7.amount end)b7,'
      '(case when b8.amount is null then 0 else b8.amount end)b8,'
      '(case when b9.amount is null then 0 else b9.amount end)b9,'
      '(case when b10.amount is null then 0 else b10.amount end)b10,'
      '(case when b11.amount is null then 0 else b11.amount end)b11 '
      'from '
      
        '(select  DISTINCT a.voucher_no from t_petty_cash a INNER JOIN t_' +
        'petty_cash_det b ON a.voucher_no=b.voucher_no where trans_date b' +
        'etween '#39'2025-01-01'#39' and '#39'2025-01-02'#39' and code_account='#39'1112'#39'  an' +
        'd position='#39'K'#39')xx '
      
        'left join  (select distinct a.actors_name,a.voucher_no,a.code_ac' +
        'count,a.trans_date,a.description,a.order_no,b.code,c."name" from' +
        ' (SELECT a.voucher_no,a.actors_name,a.trans_date,a.description,b' +
        '.code_account,a.order_no,a.actors_code from t_petty_cash a INNER' +
        ' JOIN t_petty_cash_det b ON a.voucher_no=b.voucher_no) a,t_ak_ac' +
        'count b,t_cost_actors c where a.code_account=b.code and a.actors' +
        '_code=c.code and b.type_id=3 and trans_date between '#39'2025-01-01'#39 +
        ' and '#39'2025-01-02'#39' order by voucher_no)a on a.voucher_no=xx.vouch' +
        'er_no  '
      
        'left join (select voucher_no,a.code_account,a."position",sum(amo' +
        'unt)as total from (select a.voucher_no,b.code_account,b."positio' +
        'n",a.amount FROM t_petty_cash a INNER JOIN t_petty_cash_det b ON' +
        ' a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.code_account,' +
        'b."position") a,t_ak_account b where a.code_account=b.code and b' +
        '.type_id=3 and a."position"='#39'D'#39'  group by voucher_no,a.code_acco' +
        'unt,a."position" order by voucher_no)b on a.voucher_no=b.voucher' +
        '_no '
      ' '
      
        'left join (select voucher_no,amount from(SELECT a.voucher_no,a.a' +
        'mount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cash' +
        '_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.code' +
        '_account)a,t_ak_account b where a.code_account=b.code and b.type' +
        '_id=3 and b.category_code='#39'SBG1'#39')b1 on a.voucher_no=b1.voucher_n' +
        'o '
      ' '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account)a,t_ak_account b where a.code_account=b.code and b.typ' +
        'e_id=3 and b.category_code='#39'PD1'#39')b2 on a.voucher_no=b2.voucher_n' +
        'o'
      '    '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account)a,t_ak_account b where a.code_account=b.code and b.typ' +
        'e_id=3 and b.category_code='#39'MRA1'#39')b3 on a.voucher_no=b3.voucher_' +
        'no '
      '   '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=3 and b.category_code='#39'PBP'#39')b4 on a.voucher_no=b4.voucher_' +
        'no'
      '    '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account)a,t_ak_account b where a.code_account=b.code and b.typ' +
        'e_id=3 and b.category_code='#39'ALP'#39')b5 on a.voucher_no=b5.voucher_n' +
        'o'
      '    '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account)a,t_ak_account b where a.code_account=b.code and b.typ' +
        'e_id=3 and b.category_code='#39'MRM'#39')b6 on a.voucher_no=b6.voucher_n' +
        'o '
      '    '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account)a,t_ak_account b where a.code_account=b.code and b.typ' +
        'e_id=3 and b.category_code='#39'MR1'#39')b7 on a.voucher_no=b7.voucher_n' +
        'o'
      '     '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account)a,t_ak_account b where a.code_account=b.code and b.typ' +
        'e_id=3 and b.category_code='#39'MG1'#39')b8 on a.voucher_no=b8.voucher_n' +
        'o '
      '    '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account)a,t_ak_account b where a.code_account=b.code and b.typ' +
        'e_id=3 and b.category_code='#39'BBM'#39')b9 on a.voucher_no=b9.voucher_n' +
        'o'
      '     '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account)a,t_ak_account b where a.code_account=b.code and b.typ' +
        'e_id=3 and b.category_code='#39'AKP'#39')b10 on a.voucher_no=b10.voucher' +
        '_no'
      '   '
      
        'left join (select voucher_no,amount from (SELECT a.voucher_no,a.' +
        'amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cas' +
        'h_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.cod' +
        'e_account) a,t_ak_account b where a.code_account=b.code and b.ty' +
        'pe_id=3 and b.category_code='#39'KPP'#39')b11 on a.voucher_no=b11.vouche' +
        'r_no   '
      
        'where a.voucher_no is not null order by a.trans_date,a.voucher_n' +
        'o,a.order_no;')
    Left = 624
    Top = 88
  end
  object frxReport2: TfrxReport
    Version = '2022.1.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick, pbCopy, pbSelection]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 44805.841268819400000000
    ReportOptions.LastChange = 46031.581273958300000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'procedure Memo21OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      ''
      'end;'
      ''
      'begin'
      ''
      'end.')
    Left = 542
    Top = 75
    Datasets = <
      item
        DataSet = frxDBDatasetPers
        DataSetName = 'frxDBDatasetPers'
      end
      item
        DataSet = frxDBDatasetADM2
        DataSetName = 'frxDBDatasetADM2'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 330.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 10000
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
      MirrorMode = []
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 171.456692920000000000
        Top = 18.897650000000000000
        Width = 1209.449600000000000000
        object Shape1: TfrxShapeView
          AllowVectorExport = True
          Top = 114.377952760000000000
          Width = 1209.448818900000000000
          Height = 57.078740160000000000
          Frame.Typ = []
        end
        object Memo50: TfrxMemoView
          AllowVectorExport = True
          Top = 12.094488190000000000
          Width = 1247.244094490000000000
          Height = 48.236240000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'IKHTISAR PENGGUNAAN DANA KAS KECIL '
            'BIAYA ADMINISTRASI DAN UMUM')
          ParentFont = False
        end
        object SysMemo8: TfrxSysMemoView
          AllowVectorExport = True
          Left = 1090.094930000000000000
          Top = 5.984230000000000000
          Width = 110.574830000000000000
          Height = 26.779530000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            'Hal. [PAGE#] dari [TOTALPAGES#]')
          ParentFont = False
        end
        object Memo1: TfrxMemoView
          AllowVectorExport = True
          Top = 12.204700000000000000
          Width = 354.488250000000000000
          Height = 18.897650000000000000
          DataField = 'company_name'
          DataSet = frxDBDatasetPers
          DataSetName = 'frxDBDatasetPers'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetPers."company_name"]')
          ParentFont = False
        end
        object Memo53: TfrxMemoView
          AllowVectorExport = True
          Top = 31.204700000000000000
          Width = 418.433070870000000000
          Height = 41.574830000000000000
          StretchMode = smMaxHeight
          DataField = 'address'
          DataSet = frxDBDatasetPers
          DataSetName = 'frxDBDatasetPers'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetPers."address"]')
          ParentFont = False
        end
        object Memo54: TfrxMemoView
          AllowVectorExport = True
          Top = 115.133858270000000000
          Width = 30.236220470000000000
          Height = 55.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'NO.')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo55: TfrxMemoView
          AllowVectorExport = True
          Left = 29.102362200000000000
          Top = 114.133858270000000000
          Width = 87.307086614173200000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'PELAKU')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memoperiode: TfrxMemoView
          AllowVectorExport = True
          Top = 70.102350000000000000
          Width = 1247.244094490000000000
          Height = 30.236240000000000000
          DisplayFormat.FormatStr = 'dd mmm yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Tanggal :')
          ParentFont = False
        end
        object Memo28: TfrxMemoView
          AllowVectorExport = True
          Left = 1151.244094488190000000
          Top = 112.133858270000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Keperluan Kantor')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo30: TfrxMemoView
          AllowVectorExport = True
          Left = 116.787401570000000000
          Top = 113.566929130000000000
          Width = 75.590551181102400000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'No. Bukti')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo32: TfrxMemoView
          AllowVectorExport = True
          Left = 192.000000000000000000
          Top = 113.566929130000000000
          Width = 112.629921259843000000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Keterangan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo31: TfrxMemoView
          AllowVectorExport = True
          Left = 304.629921260000000000
          Top = 116.133858270000000000
          Width = 56.692913390000000000
          Height = 54.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Total')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo33: TfrxMemoView
          AllowVectorExport = True
          Left = 360.566929133858000000
          Top = 117.133858270000000000
          Width = 56.692913385826800000
          Height = 53.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Gaji')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo34: TfrxMemoView
          AllowVectorExport = True
          Left = 416.125984251969000000
          Top = 115.133858270000000000
          Width = 56.692913385826800000
          Height = 55.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Sumbangan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo35: TfrxMemoView
          AllowVectorExport = True
          Left = 531.779527559055000000
          Top = 115.566929130000000000
          Width = 56.692913385826800000
          Height = 55.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'M/R Alat Kantor')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo36: TfrxMemoView
          AllowVectorExport = True
          Left = 588.472440944882000000
          Top = 114.566929130000000000
          Width = 56.692913385826800000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Alat Tulis dan Fotocopy')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo37: TfrxMemoView
          AllowVectorExport = True
          Left = 644.787401574803000000
          Top = 114.566929130000000000
          Width = 56.692913385826800000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Listrik dan Air')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo39: TfrxMemoView
          AllowVectorExport = True
          Left = 700.724409448819000000
          Top = 115.511811020000000000
          Width = 56.692913385826800000
          Height = 55.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'PTT dan Materai')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo41: TfrxMemoView
          AllowVectorExport = True
          Left = 757.795275590551000000
          Top = 114.511811020000000000
          Width = 56.692913385826800000
          Height = 56.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Perjalanan Dinas')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo42: TfrxMemoView
          AllowVectorExport = True
          Left = 814.488188980000000000
          Top = 114.511811020000000000
          Width = 56.692913385826800000
          Height = 55.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Pendidikan Karyawan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo43: TfrxMemoView
          AllowVectorExport = True
          Left = 870.803149606299000000
          Top = 114.511811020000000000
          Width = 56.692913385826800000
          Height = 56.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Pajak Daerah dan Perijinan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo47: TfrxMemoView
          AllowVectorExport = True
          Left = 474.330708661417000000
          Top = 114.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Asuransi')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo67: TfrxMemoView
          AllowVectorExport = True
          Left = 925.984251970000000000
          Top = 114.102350000000000000
          Width = 56.692913390000000000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'M/R Kendaraan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo70: TfrxMemoView
          AllowVectorExport = True
          Left = 981.921259842520000000
          Top = 114.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'ADM Bank')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo71: TfrxMemoView
          AllowVectorExport = True
          Left = 1038.992125984250000000
          Top = 113.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Jamuan Tamu')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo72: TfrxMemoView
          AllowVectorExport = True
          Left = 1094.929133858270000000
          Top = 113.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Managment Expense')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 24.692779130000000000
        Top = 249.448980000000000000
        Width = 1209.449600000000000000
        AllowSplit = True
        DataSet = frxDBDatasetADM2
        DataSetName = 'frxDBDatasetADM2'
        RowCount = 0
        object Memo10: TfrxMemoView
          AllowVectorExport = True
          Width = 30.236220470000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DataSetName = 'frxDBDatasetRekapKonfirmasi'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 30.102362200000000000
          Width = 87.307086610000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          Memo.UTF8W = (
            '[frxDBDatasetADM2."actors_name"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo12: TfrxMemoView
          AllowVectorExport = True
          Left = 531.779527560000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b4"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo19: TfrxMemoView
          AllowVectorExport = True
          Left = 304.629921260000000000
          Top = -0.333333340000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."total"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          AllowVectorExport = True
          Left = 116.787401570000000000
          Width = 75.590551180000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetADM2."voucher_no"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo25: TfrxMemoView
          AllowVectorExport = True
          Left = 700.724409450000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b7"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object SysMemo1: TfrxSysMemoView
          AllowVectorExport = True
          Width = 30.236220470000000000
          Height = 22.677180000000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[LINE#]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          AllowVectorExport = True
          Left = 588.472440940000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          OnBeforePrint = 'Memo21OnBeforePrint'
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b5"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo27: TfrxMemoView
          AllowVectorExport = True
          Left = 870.803149610000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b10"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo29: TfrxMemoView
          AllowVectorExport = True
          Left = 1151.244094490000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b12"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo44: TfrxMemoView
          AllowVectorExport = True
          Left = 192.000000000000000000
          Width = 112.629921260000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetADM2."description"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo45: TfrxMemoView
          AllowVectorExport = True
          Left = 360.566929130000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b1"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo46: TfrxMemoView
          AllowVectorExport = True
          Left = 416.125984250000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b2"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo48: TfrxMemoView
          AllowVectorExport = True
          Left = 644.787401574803000000
          Width = 56.692913385826800000
          Height = 24.566929130000000000
          OnBeforePrint = 'Memo21OnBeforePrint'
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b6"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo49: TfrxMemoView
          AllowVectorExport = True
          Left = 474.330708660000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b3"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo51: TfrxMemoView
          AllowVectorExport = True
          Left = 757.795275590000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b8"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo52: TfrxMemoView
          AllowVectorExport = True
          Left = 814.488188980000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b9"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo68: TfrxMemoView
          AllowVectorExport = True
          Left = 925.984251970000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b11"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo73: TfrxMemoView
          AllowVectorExport = True
          Left = 981.921259840000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b11"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo75: TfrxMemoView
          AllowVectorExport = True
          Left = 1038.992125980000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b11"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo77: TfrxMemoView
          AllowVectorExport = True
          Left = 1094.929133860000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b11"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 30.236220470000000000
        Top = 336.378170000000000000
        Width = 1209.449600000000000000
        object Memo16: TfrxMemoView
          AllowVectorExport = True
          Left = 1152.244094490000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b12">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          AllowVectorExport = True
          Width = 305.157480310000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            'Jumlah')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo57: TfrxMemoView
          AllowVectorExport = True
          Left = 361.566929130000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b1">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo58: TfrxMemoView
          AllowVectorExport = True
          Left = 417.125984250000000000
          Width = 55.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b2">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo60: TfrxMemoView
          AllowVectorExport = True
          Left = 530.779527560000000000
          Width = 57.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b4">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo61: TfrxMemoView
          AllowVectorExport = True
          Left = 588.472440940000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b5">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo62: TfrxMemoView
          AllowVectorExport = True
          Left = 644.787401570000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b6">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo63: TfrxMemoView
          AllowVectorExport = True
          Left = 700.724409450000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b7">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo64: TfrxMemoView
          AllowVectorExport = True
          Left = 756.795275590000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b8">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo65: TfrxMemoView
          AllowVectorExport = True
          Left = 815.154855650000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b9">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo66: TfrxMemoView
          AllowVectorExport = True
          Left = 870.803149610000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b10">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo59: TfrxMemoView
          AllowVectorExport = True
          Left = 473.330708660000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b3">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo69: TfrxMemoView
          AllowVectorExport = True
          Left = 925.984251970000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b11">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo56: TfrxMemoView
          AllowVectorExport = True
          Left = 304.629921260000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."total">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo74: TfrxMemoView
          AllowVectorExport = True
          Left = 981.921259840000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b11">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo76: TfrxMemoView
          AllowVectorExport = True
          Left = 1038.992125980000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b11">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo78: TfrxMemoView
          AllowVectorExport = True
          Left = 1094.929133860000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b11">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
    end
  end
  object frxReportADM2: TfrxReport
    Version = '2022.1.3'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick, pbCopy, pbSelection]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Default'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 44805.841268819400000000
    ReportOptions.LastChange = 46036.025171099540000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'procedure Memo21OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      ''
      'end;'
      ''
      'begin'
      ''
      'end.')
    Left = 638
    Top = 211
    Datasets = <
      item
        DataSet = frxDBDatasetPers
        DataSetName = 'frxDBDatasetPers'
      end
      item
        DataSet = frxDBDatasetADM2
        DataSetName = 'frxDBDatasetADM2'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      Orientation = poLandscape
      PaperWidth = 330.000000000000000000
      PaperHeight = 210.000000000000000000
      PaperSize = 10000
      LeftMargin = 5.000000000000000000
      RightMargin = 5.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
      MirrorMode = []
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 171.456692920000000000
        Top = 18.897650000000000000
        Width = 1209.449600000000000000
        object Shape1: TfrxShapeView
          AllowVectorExport = True
          Top = 114.377952760000000000
          Width = 1209.448818900000000000
          Height = 57.078740160000000000
          Frame.Typ = []
        end
        object Memo50: TfrxMemoView
          AllowVectorExport = True
          Top = 12.094488190000000000
          Width = 1247.244094490000000000
          Height = 48.236240000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'IKHTISAR PENGGUNAAN DANA KAS KECIL '
            'BIAYA ADMINISTRASI DAN UMUM')
          ParentFont = False
        end
        object SysMemo8: TfrxSysMemoView
          AllowVectorExport = True
          Left = 1090.094930000000000000
          Top = 5.984230000000000000
          Width = 110.574830000000000000
          Height = 26.779530000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            'Hal. [PAGE#] dari [TOTALPAGES#]')
          ParentFont = False
        end
        object Memo1: TfrxMemoView
          AllowVectorExport = True
          Top = 12.204700000000000000
          Width = 354.488250000000000000
          Height = 18.897650000000000000
          DataField = 'company_name'
          DataSet = frxDBDatasetPers
          DataSetName = 'frxDBDatasetPers'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetPers."company_name"]')
          ParentFont = False
        end
        object Memo53: TfrxMemoView
          AllowVectorExport = True
          Top = 31.204700000000000000
          Width = 418.433070870000000000
          Height = 41.574830000000000000
          StretchMode = smMaxHeight
          DataField = 'address'
          DataSet = frxDBDatasetPers
          DataSetName = 'frxDBDatasetPers'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          Memo.UTF8W = (
            '[frxDBDatasetPers."address"]')
          ParentFont = False
        end
        object Memo54: TfrxMemoView
          AllowVectorExport = True
          Top = 115.133858270000000000
          Width = 30.236220470000000000
          Height = 55.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'NO.')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo55: TfrxMemoView
          AllowVectorExport = True
          Left = 29.102362200000000000
          Top = 114.133858270000000000
          Width = 87.307086614173200000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'PELAKU')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memoperiode: TfrxMemoView
          AllowVectorExport = True
          Top = 70.102350000000000000
          Width = 1247.244094490000000000
          Height = 30.236240000000000000
          DisplayFormat.FormatStr = 'dd mmm yyyy'
          DisplayFormat.Kind = fkDateTime
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Tanggal :')
          ParentFont = False
        end
        object Memo28: TfrxMemoView
          AllowVectorExport = True
          Left = 1151.244094488190000000
          Top = 112.133858270000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Keperluan Kantor')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo30: TfrxMemoView
          AllowVectorExport = True
          Left = 116.787401570000000000
          Top = 113.566929130000000000
          Width = 75.590551181102400000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'No. Bukti')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo32: TfrxMemoView
          AllowVectorExport = True
          Left = 192.000000000000000000
          Top = 113.566929130000000000
          Width = 112.629921259843000000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Keterangan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo31: TfrxMemoView
          AllowVectorExport = True
          Left = 304.629921260000000000
          Top = 116.133858270000000000
          Width = 56.692913390000000000
          Height = 54.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Total')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo33: TfrxMemoView
          AllowVectorExport = True
          Left = 360.566929133858000000
          Top = 117.133858270000000000
          Width = 56.692913385826800000
          Height = 53.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Gaji')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo34: TfrxMemoView
          AllowVectorExport = True
          Left = 416.125984251969000000
          Top = 115.133858270000000000
          Width = 56.692913385826800000
          Height = 55.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Sumbangan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo35: TfrxMemoView
          AllowVectorExport = True
          Left = 531.779527559055000000
          Top = 115.566929130000000000
          Width = 56.692913385826800000
          Height = 55.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'M/R Alat Kantor')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo36: TfrxMemoView
          AllowVectorExport = True
          Left = 588.472440944882000000
          Top = 114.566929130000000000
          Width = 56.692913385826800000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Alat Tulis dan Fotocopy')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo37: TfrxMemoView
          AllowVectorExport = True
          Left = 644.787401574803000000
          Top = 114.566929130000000000
          Width = 56.692913385826800000
          Height = 56.267716540000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Listrik dan Air')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo39: TfrxMemoView
          AllowVectorExport = True
          Left = 700.724409448819000000
          Top = 115.511811020000000000
          Width = 56.692913385826800000
          Height = 55.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'PTT dan Materai')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo41: TfrxMemoView
          AllowVectorExport = True
          Left = 757.795275590551000000
          Top = 114.511811020000000000
          Width = 56.692913385826800000
          Height = 56.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Perjalanan Dinas')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo42: TfrxMemoView
          AllowVectorExport = True
          Left = 814.488188980000000000
          Top = 114.511811020000000000
          Width = 56.692913385826800000
          Height = 55.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Pendidikan Karyawan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo43: TfrxMemoView
          AllowVectorExport = True
          Left = 870.803149606299000000
          Top = 114.511811020000000000
          Width = 56.692913385826800000
          Height = 56.322834650000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Pajak Daerah dan Perijinan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo47: TfrxMemoView
          AllowVectorExport = True
          Left = 474.330708661417000000
          Top = 114.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Asuransi')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo67: TfrxMemoView
          AllowVectorExport = True
          Left = 925.984251970000000000
          Top = 114.102350000000000000
          Width = 56.692913390000000000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'M/R Kendaraan')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo70: TfrxMemoView
          AllowVectorExport = True
          Left = 981.921259842520000000
          Top = 114.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'ADM Bank')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo71: TfrxMemoView
          AllowVectorExport = True
          Left = 1038.992125984250000000
          Top = 113.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Jamuan Tamu')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo72: TfrxMemoView
          AllowVectorExport = True
          Left = 1094.929133858270000000
          Top = 113.102350000000000000
          Width = 56.692913385826800000
          Height = 56.700787400000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight]
          HAlign = haCenter
          Memo.UTF8W = (
            'Managment Expense')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 24.692779130000000000
        Top = 249.448980000000000000
        Width = 1209.449600000000000000
        AllowSplit = True
        DataSet = frxDBDatasetADM2
        DataSetName = 'frxDBDatasetADM2'
        RowCount = 0
        object Memo10: TfrxMemoView
          AllowVectorExport = True
          Width = 30.236220470000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DataSetName = 'frxDBDatasetRekapKonfirmasi'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 30.102362200000000000
          Width = 87.307086610000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          Memo.UTF8W = (
            '[frxDBDatasetADM2."actors_name"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo12: TfrxMemoView
          AllowVectorExport = True
          Left = 531.779527560000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b4"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo19: TfrxMemoView
          AllowVectorExport = True
          Left = 304.629921260000000000
          Top = -0.333333340000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."total"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo13: TfrxMemoView
          AllowVectorExport = True
          Left = 116.787401570000000000
          Width = 75.590551180000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftLeft, ftRight, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetADM2."voucher_no"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo25: TfrxMemoView
          AllowVectorExport = True
          Left = 700.724409450000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b7"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object SysMemo1: TfrxSysMemoView
          AllowVectorExport = True
          Width = 30.236220470000000000
          Height = 22.677180000000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            '[LINE#]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo21: TfrxMemoView
          AllowVectorExport = True
          Left = 588.472440940000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          OnBeforePrint = 'Memo21OnBeforePrint'
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b5"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo27: TfrxMemoView
          AllowVectorExport = True
          Left = 870.803149610000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b10"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo29: TfrxMemoView
          AllowVectorExport = True
          Left = 1151.244094490000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b15"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo44: TfrxMemoView
          AllowVectorExport = True
          Left = 192.000000000000000000
          Width = 112.629921260000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haCenter
          Memo.UTF8W = (
            '[frxDBDatasetADM2."description"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo45: TfrxMemoView
          AllowVectorExport = True
          Left = 360.566929130000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b1"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo46: TfrxMemoView
          AllowVectorExport = True
          Left = 416.125984250000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b2"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo48: TfrxMemoView
          AllowVectorExport = True
          Left = 644.787401570000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          OnBeforePrint = 'Memo21OnBeforePrint'
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b6"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo49: TfrxMemoView
          AllowVectorExport = True
          Left = 474.330708660000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b3"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo51: TfrxMemoView
          AllowVectorExport = True
          Left = 757.795275590000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b8"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo52: TfrxMemoView
          AllowVectorExport = True
          Left = 814.488188980000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b9"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo68: TfrxMemoView
          AllowVectorExport = True
          Left = 925.984251970000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b11"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo73: TfrxMemoView
          AllowVectorExport = True
          Left = 981.921259840000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b12"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo75: TfrxMemoView
          AllowVectorExport = True
          Left = 1038.992125980000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b13"]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo77: TfrxMemoView
          AllowVectorExport = True
          Left = 1094.929133860000000000
          Width = 56.692913390000000000
          Height = 24.566929130000000000
          StretchMode = smMaxHeight
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = [ftRight, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[frxDBDatasetADM2."b14"]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
      object ReportSummary1: TfrxReportSummary
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 30.236220470000000000
        Top = 336.378170000000000000
        Width = 1209.449600000000000000
        object Memo16: TfrxMemoView
          AllowVectorExport = True
          Left = 1152.244094490000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b15">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo17: TfrxMemoView
          AllowVectorExport = True
          Width = 305.157480310000000000
          Height = 30.236220470000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            'Jumlah')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo57: TfrxMemoView
          AllowVectorExport = True
          Left = 361.566929130000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b1">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo58: TfrxMemoView
          AllowVectorExport = True
          Left = 417.125984250000000000
          Width = 55.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b2">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo60: TfrxMemoView
          AllowVectorExport = True
          Left = 530.779527560000000000
          Width = 57.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b4">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo61: TfrxMemoView
          AllowVectorExport = True
          Left = 588.472440940000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b5">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo62: TfrxMemoView
          AllowVectorExport = True
          Left = 644.787401570000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b6">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo63: TfrxMemoView
          AllowVectorExport = True
          Left = 700.724409450000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b7">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo64: TfrxMemoView
          AllowVectorExport = True
          Left = 756.795275590000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b8">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo65: TfrxMemoView
          AllowVectorExport = True
          Left = 815.154855650000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b9">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo66: TfrxMemoView
          AllowVectorExport = True
          Left = 870.803149610000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b10">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo59: TfrxMemoView
          AllowVectorExport = True
          Left = 473.330708660000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b3">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo69: TfrxMemoView
          AllowVectorExport = True
          Left = 925.984251970000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b11">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo56: TfrxMemoView
          AllowVectorExport = True
          Left = 304.629921260000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftLeft, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."total">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo74: TfrxMemoView
          AllowVectorExport = True
          Left = 981.921259840000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b12">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo76: TfrxMemoView
          AllowVectorExport = True
          Left = 1038.992125980000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftRight, ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b13">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
        object Memo78: TfrxMemoView
          AllowVectorExport = True
          Left = 1094.929133860000000000
          Width = 56.692913390000000000
          Height = 30.236220470000000000
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = [ftTop, ftBottom]
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frxDBDatasetADM2."b14">,MasterData1,3)]')
          ParentFont = False
          VAlign = vaCenter
        end
      end
    end
  end
  object frxDBDatasetADM2: TfrxDBDataset
    UserName = 'frxDBDatasetADM2'
    CloseDataSource = False
    FieldAliases.Strings = (
      'actors_name=actors_name'
      'voucher_no=voucher_no'
      'description=description'
      'name=name'
      'total=total'
      'b1=b1'
      'b2=b2'
      'b3=b3'
      'b4=b4'
      'b5=b5'
      'b6=b6'
      'b7=b7'
      'b8=b8'
      'b9=b9'
      'b10=b10'
      'b11=b11'
      'b12=b12'
      'b13=b13'
      'b14=b14'
      'b15=b15')
    DataSet = QkasKecilADM2
    BCDToCurrency = False
    DataSetOptions = []
    Left = 576
    Top = 224
  end
  object QkasKecilADM2: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'select a.actors_name,a.voucher_no,a.description,a."name",b.total' +
        ', (case when b1.amount is null then 0 else b1.amount end)b1, (ca' +
        'se when b2.amount is null then 0 else b2.amount end)b2, (case wh' +
        'en b3.amount is null then 0 else b3.amount end)b3, (case when b4' +
        '.amount is null then 0 else b4.amount end)b4, (case when b5.amou' +
        'nt is null then 0 else b5.amount end)b5, (case when b6.amount is' +
        ' null then 0 else b6.amount end)b6, (case when b7.amount is null' +
        ' then 0 else b7.amount end)b7, (case when b8.amount is null then' +
        ' 0 else b8.amount end)b8, (case when b9.amount is null then 0 el' +
        'se b9.amount end)b9, (case when b10.amount is null then 0 else b' +
        '10.amount end)b10, (case when b11.amount is null then 0 else b11' +
        '.amount end)b11, (case when b12.amount is null then 0 else b12.a' +
        'mount end)b12,0::REAL AS b13 ,0::REAL AS b14 ,0::REAL AS b15 fro' +
        'm (select distinct  a.voucher_no  from t_petty_cash a INNER JOIN' +
        ' t_petty_cash_det b ON a.voucher_no=b.voucher_no where trans_dat' +
        'e between '#39'2025-10-01'#39' and '#39'2025-10-07'#39' and code_account='#39'1101.0' +
        '2'#39'  and position='#39'K'#39')xx left join (select distinct a.actors_name' +
        ',a.voucher_no,a.code_account,a.trans_date,a.description,a.order_' +
        'no,b.code,c."name" from (SELECT a.voucher_no,a.actors_name,a.tra' +
        'ns_date,a.description,b.code_account,a.order_no,a.actors_code fr' +
        'om t_petty_cash a INNER JOIN t_petty_cash_det b ON a.voucher_no=' +
        'b.voucher_no) a,t_ak_account b,t_cost_actors c where a.code_acco' +
        'unt=b.code and a.actors_code=c.code and b.type_id=2 and trans_da' +
        'te between '#39'2025-10-01'#39' and '#39'2025-10-07'#39' order by voucher_no)a o' +
        'n a.voucher_no=xx.voucher_no left join (select voucher_no,a.code' +
        '_account,a."position",sum(amount)as total from (select a.voucher' +
        '_no,b.code_account,b."position",a.amount FROM t_petty_cash a INN' +
        'ER JOIN t_petty_cash_det b ON a.voucher_no=b.voucher_no GROUP BY' +
        ' a.voucher_no,b.code_account,b."position") a,t_ak_account b wher' +
        'e a.code_account=b.code and b.type_id=2 and a."position"='#39'D'#39'  gr' +
        'oup by voucher_no,a.code_account,a."position" order by voucher_n' +
        'o)b on a.voucher_no=b.voucher_no left join (select voucher_no,am' +
        'ount from (SELECT a.voucher_no,a.amount,b.code_account FROM t_pe' +
        'tty_cash a INNER JOIN t_petty_cash_det b ON a.voucher_no=b.vouch' +
        'er_no GROUP BY a.voucher_no,b.code_account) a,t_ak_account b whe' +
        're a.code_account=b.code and b.type_id=2 and b.category_code='#39'SB' +
        'G'#39')b1 on a.voucher_no=b1.voucher_no left join (select voucher_no' +
        ',amount from (SELECT a.voucher_no,a.amount,b.code_account FROM t' +
        '_petty_cash a INNER JOIN t_petty_cash_det b ON a.voucher_no=b.vo' +
        'ucher_no GROUP BY a.voucher_no,b.code_account) a,t_ak_account b ' +
        'where a.code_account=b.code and b.type_id=2 and b.category_code=' +
        #39'ASR'#39')b2 on a.voucher_no=b2.voucher_no left join (select voucher' +
        '_no,amount from (SELECT a.voucher_no,a.amount,b.code_account FRO' +
        'M t_petty_cash a INNER JOIN t_petty_cash_det b ON a.voucher_no=b' +
        '.voucher_no GROUP BY a.voucher_no,b.code_account) a,t_ak_account' +
        ' b where a.code_account=b.code and b.type_id=2 and b.category_co' +
        'de='#39'JMT'#39')b3 on a.voucher_no=b3.voucher_no left join (select vouc' +
        'her_no,amount from (SELECT a.voucher_no,a.amount,b.code_account ' +
        'FROM t_petty_cash a INNER JOIN t_petty_cash_det b ON a.voucher_n' +
        'o=b.voucher_no GROUP BY a.voucher_no,b.code_account) a,t_ak_acco' +
        'unt b where a.code_account=b.code and b.type_id=2 and b.category' +
        '_code='#39'PD2'#39')b4 on a.voucher_no=b4.voucher_no left join (select v' +
        'oucher_no,amount from (SELECT a.voucher_no,a.amount,b.code_accou' +
        'nt FROM t_petty_cash a INNER JOIN t_petty_cash_det b ON a.vouche' +
        'r_no=b.voucher_no GROUP BY a.voucher_no,b.code_account) a,t_ak_a' +
        'ccount b where a.code_account=b.code and b.type_id=2 and b.categ' +
        'ory_code='#39'ATK'#39')b5 on a.voucher_no=b5.voucher_no left join (selec' +
        't voucher_no,amount from (SELECT a.voucher_no,a.amount,b.code_ac' +
        'count FROM t_petty_cash a INNER JOIN t_petty_cash_det b ON a.vou' +
        'cher_no=b.voucher_no GROUP BY a.voucher_no,b.code_account) a,t_a' +
        'k_account b where a.code_account=b.code and b.type_id=2 and b.ca' +
        'tegory_code='#39'LNA'#39')b6 on a.voucher_no=b6.voucher_no left join (se' +
        'lect voucher_no,amount from (SELECT a.voucher_no,a.amount,b.code' +
        '_account FROM t_petty_cash a INNER JOIN t_petty_cash_det b ON a.' +
        'voucher_no=b.voucher_no GROUP BY a.voucher_no,b.code_account) a,' +
        't_ak_account b where a.code_account=b.code and b.type_id=2 and b' +
        '.category_code='#39'PTM'#39')b7 on a.voucher_no=b7.voucher_no left join ' +
        '(select voucher_no,amount from (SELECT a.voucher_no,a.amount,b.c' +
        'ode_account FROM t_petty_cash a INNER JOIN t_petty_cash_det b ON' +
        ' a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.code_account)' +
        ' a,t_ak_account b where a.code_account=b.code and b.type_id=2 an' +
        'd b.category_code='#39'MRA'#39')b8 on a.voucher_no=b8.voucher_no left jo' +
        'in (select voucher_no,amount from (SELECT a.voucher_no,a.amount,' +
        'b.code_account FROM t_petty_cash a INNER JOIN t_petty_cash_det b' +
        ' ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.code_accou' +
        'nt) a,t_ak_account b where a.code_account=b.code and b.type_id=2' +
        ' and b.category_code='#39'PDK'#39')b9 on a.voucher_no=b9.voucher_no left' +
        ' join (select voucher_no,amount from (SELECT a.voucher_no,a.amou' +
        'nt,b.code_account FROM t_petty_cash a INNER JOIN t_petty_cash_de' +
        't b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.code_ac' +
        'count) a,t_ak_account b where a.code_account=b.code and b.type_i' +
        'd=2 and b.category_code='#39'PDP'#39')b10 on a.voucher_no=b10.voucher_no' +
        ' left join (select voucher_no,amount from (SELECT a.voucher_no,a' +
        '.amount,b.code_account FROM t_petty_cash a INNER JOIN t_petty_ca' +
        'sh_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no,b.co' +
        'de_account) a,t_ak_account b where a.code_account=b.code and b.t' +
        'ype_id=2 and b.category_code='#39'KPK'#39')b11 on a.voucher_no=b11.vouch' +
        'er_no left join (select voucher_no,amount from (SELECT a.voucher' +
        '_no,a.amount,b.code_account FROM t_petty_cash a INNER JOIN t_pet' +
        'ty_cash_det b ON a.voucher_no=b.voucher_no GROUP BY a.voucher_no' +
        ',b.code_account) a,t_ak_account b where a.code_account=b.code an' +
        'd b.type_id=2 and b.category_code='#39'HMS'#39')b12 on a.voucher_no=b12.' +
        'voucher_no where a.voucher_no is not null order by a.trans_date,' +
        'a.voucher_no,a.order_no;')
    Left = 496
    Top = 216
    object QkasKecilADM2actors_name: TStringField
      FieldName = 'actors_name'
      ReadOnly = True
      Size = 255
    end
    object QkasKecilADM2voucher_no: TStringField
      FieldName = 'voucher_no'
      ReadOnly = True
      Size = 100
    end
    object QkasKecilADM2description: TStringField
      FieldName = 'description'
      ReadOnly = True
      Size = 255
    end
    object QkasKecilADM2name: TStringField
      FieldName = 'name'
      ReadOnly = True
      Size = 255
    end
    object QkasKecilADM2total: TFloatField
      FieldName = 'total'
      ReadOnly = True
    end
    object QkasKecilADM2b1: TFloatField
      FieldName = 'b1'
      ReadOnly = True
    end
    object QkasKecilADM2b2: TFloatField
      FieldName = 'b2'
      ReadOnly = True
    end
    object QkasKecilADM2b3: TFloatField
      FieldName = 'b3'
      ReadOnly = True
    end
    object QkasKecilADM2b4: TFloatField
      FieldName = 'b4'
      ReadOnly = True
    end
    object QkasKecilADM2b5: TFloatField
      FieldName = 'b5'
      ReadOnly = True
    end
    object QkasKecilADM2b6: TFloatField
      FieldName = 'b6'
      ReadOnly = True
    end
    object QkasKecilADM2b7: TFloatField
      FieldName = 'b7'
      ReadOnly = True
    end
    object QkasKecilADM2b8: TFloatField
      FieldName = 'b8'
      ReadOnly = True
    end
    object QkasKecilADM2b9: TFloatField
      FieldName = 'b9'
      ReadOnly = True
    end
    object QkasKecilADM2b10: TFloatField
      FieldName = 'b10'
      ReadOnly = True
    end
    object QkasKecilADM2b11: TFloatField
      FieldName = 'b11'
      ReadOnly = True
    end
    object QkasKecilADM2b12: TFloatField
      FieldName = 'b12'
      ReadOnly = True
    end
    object QkasKecilADM2b13: TFloatField
      FieldName = 'b13'
      ReadOnly = True
    end
    object QkasKecilADM2b14: TFloatField
      FieldName = 'b14'
      ReadOnly = True
    end
    object QkasKecilADM2b15: TFloatField
      FieldName = 'b15'
      ReadOnly = True
    end
  end
end
