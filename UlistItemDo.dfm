object FListDo: TFListDo
  Left = 0
  Top = 0
  Caption = 'Form List DO'
  ClientHeight = 442
  ClientWidth = 628
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  Position = poDesktopCenter
  OnShow = FormShow
  TextHeight = 15
  object Panel1: TPanel
    Left = 0
    Top = 410
    Width = 628
    Height = 32
    Align = alBottom
    TabOrder = 0
    ExplicitTop = 409
    ExplicitWidth = 624
    object BBatal: TRzBitBtn
      Left = 552
      Top = 1
      Height = 30
      Align = alRight
      Caption = 'Cancel'
      TabOrder = 0
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 548
    end
    object BEdit: TRzBitBtn
      Left = 477
      Top = 1
      Height = 30
      Align = alRight
      Caption = 'OK'
      TabOrder = 1
      OnClick = BEditClick
      Glyph.Data = {
        
        E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8E8}
      NumGlyphs = 2
      ExplicitLeft = 473
    end
    object Cb_All: TCheckBox
      Left = 8
      Top = 6
      Width = 97
      Height = 17
      Caption = 'Pilih Semua'
      TabOrder = 2
      Visible = False
    end
  end
  object DBGrid_DO: TDBGridEh
    Left = 0
    Top = 57
    Width = 628
    Height = 353
    Align = alClient
    DataSource = DS_do
    DrawMemoText = True
    DynProps = <>
    IndicatorOptions = [gioShowRowIndicatorEh, gioShowRecNoEh, gioShowRowselCheckboxesEh]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
    OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghAutoSortMarking, dghMultiSortMarking, dghDialogFind, dghShowRecNo, dghColumnResize, dghColumnMove, dghExtendVertLines]
    SearchPanel.Enabled = True
    SearchPanel.FilterOnTyping = True
    TabOrder = 1
    TitleParams.MultiTitle = True
    Columns = <
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'notrans'
        Footers = <>
        Title.Caption = 'No. DO'
        Width = 150
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'date_trans'
        Footers = <>
        Title.Caption = 'Tanggal'
        Width = 100
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'type_do_name'
        Footers = <>
        Title.Caption = 'Jenis DO'
        Width = 120
      end
      item
        CellButtons = <>
        DynProps = <>
        EditButtons = <>
        FieldName = 'vendor_name'
        Footers = <>
        Title.Caption = 'Nama Vendor'
        Width = 200
      end
      item
        CellButtons = <>
        DisplayFormat = '#,##0.00'
        DynProps = <>
        EditButtons = <>
        FieldName = 'total_biaya'
        Footers = <>
        Title.Caption = 'Total Biaya'
        Width = 100
      end>
    object RowDetailData: TRowDetailPanelControlEh
    end
  end
  object RzPanel1: TRzPanel
    Left = 0
    Top = 0
    Width = 628
    Height = 57
    Align = alTop
    BorderSides = [sdLeft, sdTop, sdRight]
    Color = 15987699
    TabOrder = 2
    ExplicitWidth = 624
    object Label1: TLabel
      Left = 31
      Top = 18
      Width = 74
      Height = 15
      Caption = 'Tanggal DO   :'
    end
    object DT_tgl_do: TRzDateTimePicker
      Left = 120
      Top = 15
      Width = 121
      Height = 23
      Date = 46021.000000000000000000
      Format = ''
      Time = 0.621732118052023000
      TabOrder = 0
      OnChange = DT_tgl_doChange
    end
  end
  object Q_do: TUniQuery
    Connection = dm.Koneksi
    SQL.Strings = (
      
        'select a.notrans,a.date_trans,a.type_do_code,a.type_do_name,b.ve' +
        'ndor_code,b.vendor_name,a.status,(SELECT SUM(grand_total) from t' +
        '_delivery_order_cost WHERE notrans=a.notrans) total_biaya from t' +
        '_delivery_order a'
      'LEFT JOIN t_delivery_order_services b on b.notrans=a.notrans'
      
        'WHERE a.date_trans='#39'2025-12-26'#39' and a.deleted_at is NULL AND a.s' +
        'tatus=6')
    Left = 424
    Top = 176
    object Q_donotrans: TStringField
      FieldName = 'notrans'
      Size = 100
    end
    object Q_dodate_trans: TDateField
      FieldName = 'date_trans'
    end
    object Q_dotype_do_code: TStringField
      FieldName = 'type_do_code'
      Size = 100
    end
    object Q_dotype_do_name: TStringField
      FieldName = 'type_do_name'
      Size = 255
    end
    object Q_dovendor_code: TStringField
      FieldName = 'vendor_code'
      ReadOnly = True
      Size = 100
    end
    object Q_dovendor_name: TStringField
      FieldName = 'vendor_name'
      ReadOnly = True
      Size = 255
    end
    object Q_dostatus: TSmallintField
      FieldName = 'status'
    end
    object Q_dototal_biaya: TFloatField
      FieldName = 'total_biaya'
      ReadOnly = True
    end
  end
  object DS_do: TDataSource
    DataSet = Q_do
    Left = 480
    Top = 176
  end
end
