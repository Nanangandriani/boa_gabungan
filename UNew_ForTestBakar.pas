...
begin
  ...
  DtPMulai.Date:=Dm.Qtemp['prod_start_date'];
  DtPSelesai.Date:=dm.Qtemp['prod_end_date'];
  DtTest.Text:=dm.Qtemp['trial_date'];
end;
...
begin
  ...
  begin
    close;
    sql.Clear;
    sql.Text:='Select * from "warehouse".t_item_use_for where status=''1''';
    ExecSQL;
  end;
  Dm.Qtemp.First;
  while not dm.Qtemp.Eof do
